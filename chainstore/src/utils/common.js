...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('chainstore_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('chainstore_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('chainstore_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('chainstore_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'chainstore') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('chainstore_access_token', Cookies.get('access_token'))
  LocalStorage.set('chainstore_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('chainstore_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('chainstore_token_type', Cookies.get('token_type'))
  LocalStorage.set('chainstore_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'chainstore_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('chainstore_user_info', Cookies.get('user_info'))
  LocalStorage.set('chainstore_account_info', Cookies.get('account_info'))
