  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-sm q-pb-md">
      <div class="q-gutter-x-xs row">
        <q-btn color="primary" unelevated label="会员登记" :to="{ name: 'addMember' }" v-if="authorityMeta('add')" />
        ...
        <q-btn-dropdown color="red" label="批量操作" v-if="authorityMeta('batch')" :menu-offset="[10, 6]" unelevated>
          <q-list class="text-center">
            ...
            <q-item clickable v-if="authorityMeta('addAll')">
              <q-item-section @click="operateAddSomeMember">
                <q-item-label> 批量自定义生成会员 </q-item-label>
              </q-item-section>
            </q-item>
            <q-item clickable @click="showPopup" v-if="authorityMeta('editAll')" :disable="dataListSelections.length > 0 ? false : true">
              <q-item-section>
                <q-item-label
                  >批量操作会员{{ $q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_integralUnit
        ...
        <q-btn-dropdown color="green" label="导入导出" :menu-offset="[10, 6]" v-if="authorityMeta('importOrExport')" unelevated>
          <q-list class="text-center">
            <q-item clickable @click="batchImport" v-if="authorityMeta('import')">
              <q-item-section>
                <q-item-label> Xlxs 文件批量导入 </q-item-label>
              </q-item-section>
            </q-item>
      ...
      >
        <q-form ref="memberDataRef" class="q-gutter-md q-mt-md">
          <q-card-section class="row items-center q-pb-none">
            <div class="text-h6">
              批量操作会员{{ $q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}与{{
          ...
          <q-card-section class="q-pt-sm">
            <div class="col-12">
              <q-input
                ...
                :label="`${$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_integralUnit}`"
                outlined
                ...
                :rules="[
                  (val) =>
                    /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                    `${$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_integralUnit}为非负两位以内小数`,
                ]"
              />
            </div>
            <div class="col-12">
              <q-input
                :disable="parseInt(batchOperate) == 3"
                :label="`${$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit}`"
                outlined
                ...
                :rules="[
                  (val) =>
                    /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                    `${$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit}为非负两位以内小数`,
                ]"
              />
            </div>
...
import MemberAddress from '../address/MemberAddress.vue'
export default {
  name: 'MemberList',
  components: { AgMemberOperating, AddSomeMember, AgMembertag, MemberAddress },
  ...
  mounted () {
    if (
      this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig &&
      this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_regBindFormField &&
      this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_regBindFormField > 0
    ) {
      this.getFieldData(this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_regBindFormField)
    } else {
      this.restColumnData()
    }
  },
  methods: {
    getFieldData (formFieldId) {
      this.$store
        .dispatch('formfield/getColumnDefs', parseInt(formFieldId))
        .then((res) => {
          if (res.code == 200) {
            const columnDefsForm = res.data ? res.data : []
            this.$nextTick(() => {
              this.restColumnData(columnDefsForm)
            })
          } else {
            this.restColumnData()
          }
    ...
    restColumnData (column) {
      const columnData = [
        ...
        {
          headerName: '总' + this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit,
        ...
        {
          headerName: '总' + this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_integralUnit,
