      ...
      <div class="row q-gutter-xs">
        <div class="col-12 col-md-2">
          <q-select
            outlined
            dense
            v-model="reserveData.projectId"
            option-value="id"
            option-label="prp_name"
            :options="reserveDataOptions"
            label="预约项目"
            emit-value
            map-options
            lazy-rules
            ...
            <template v-slot:prepend>
              <q-icon name="event" class="cursor-pointer">
                <q-popup-proxy transition-show="scale" transition-hide="scale">
                  <q-date v-model="reserveData.prd_time_start" minimal mask="YYYY-MM-DD">
        ...
        <div class="col-md-2 col-12">
          <q-input outlined readonly dense label="结束日期" v-model="reserveData.prd_time_end">
            <template v-slot:prepend>
              <q-icon name="event" class="cursor-pointer">
                <q-popup-proxy transition-show="scale" transition-hide="scale">
                  <q-date v-model="reserveData.prd_time_end" minimal mask="YYYY-MM-DD">
                    <div class="row items-center justify-end">
                      <q-btn v-close-popup label="确定" color="primary" flat />
                    </div>
                  </q-date>
  ...
  methods: {
    getList () {
      ...
      const obj = {
        ...
        projectId: this.reserveData.projectId,
    ...
    getReserveDataList () {
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            this.reserveDataOptions = res.data ? res.data : []
          } else {
            this.$q.notify({
              message: '失败',
              icon: 'ion-close-circle-outline',
