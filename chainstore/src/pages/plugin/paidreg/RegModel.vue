            ...
            <q-item>
              <q-item-section>
                ...
                <q-item-label caption lines="2" class="q-gutter-xs">
                  ...
                  <q-badge color="pink" v-if="item.coupon && item.coupon.length > 0"> 卡券 </q-badge>
                  <q-badge color="indigo" v-if="item.count && item.count.length > 0"> 次卡 </q-badge>
                  <q-badge color="teal" v-if="item.timing && item.timing.length > 0"> 时卡 </q-badge>
                </q-item-label>
              </q-item-section>
              <q-item-section side>
                <span class="text-h6 text-deep-orange" v-if="parseInt(item.pp_type) == 2"
                  >{{ $q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.pp_selling_price }}</span
                >
                <span v-else> 免费 </span>
              </q-item-section>
            ...
            <q-card-section class="q-pa-sm">
              ...
              <q-list v-if="item.coupon && item.coupon.length > 0">
                <q-item class="bordered-orange-color q-mt-sm" :key="k + '-coupon'" v-for="(i, k) in item.coupon">
                  ...
                  <q-item-section side top>
                    <q-item-label caption class="text-primary">
                      <q-badge color="pink" v-if="parseInt(i.cou_type) == 1"> 折 扣 券 </q-badge>
                      <q-badge color="pink" v-if="parseInt(i.cou_type) == 2"> 代 金 券 </q-badge>
                      <q-badge color="pink" v-if="parseInt(i.cou_type) == 3"> 礼 品 券 </q-badge>
                      <q-badge color="pink" v-if="parseInt(i.cou_type) == 4"> 团 购 券 </q-badge>
                      <q-badge color="pink" v-if="parseInt(i.cou_type) == 5"> 优 惠 券 </q-badge>
                    </q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
              ...
              <q-list v-if="item.count && item.count.length > 0">
                <q-item class="bordered-orange-color q-mt-sm" :key="k + '-count'" v-for="(i, k) in item.count">
                  <q-item-section class="q-pa-xs">
                    ...
                    <q-item-label v-if="parseInt(i.grp_timeType) == 1"> 永久有效 </q-item-label>
                  ...
                  <q-item-section side top>
                    <q-badge color="red"> 计 次 项 </q-badge>
                  </q-item-section>
                </q-item>
              </q-list>
              ...
              <q-list v-if="item.timing && item.timing.length > 0">
                <q-item class="bordered-orange-color q-mt-sm" :key="k + '-tiem'" v-for="(i, k) in item.timing">
                  <q-item-section class="q-pa-xs">
                    ...
                    <q-item-label v-if="parseInt(i.gtg_time_type) == 1"> 永久有效 </q-item-label>
                  ...
                  <q-item-section side top>
                    <q-badge color="info"> 计 时 项 </q-badge>
                  </q-item-section>
                </q-item>
              </q-list>
            </q-card-section>
...
export default {
  ...
  created () {
    this.openPlugins = LocalStorage.getItem('chainstore_plugins_local').openPlugins
