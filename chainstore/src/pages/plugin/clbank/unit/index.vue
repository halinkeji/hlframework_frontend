<template>
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
       <edit-unit v-if="isPoppupStatus" @getMethods="getList" ref="editUnitRef"></edit-unit>
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <q-card flat class="q-pa-sm">
          <div class="row q-gutter-x-sm">
            <div class="col-12 col-md-2">
              <q-input clearable label="请输入材料名称" outlined v-model="searchQuery" dense />
            </div>

            <q-btn @click="getList()" color="primary" unelevated label="搜索" />
            <q-btn @click="clearnBut()" color="warning" unelevated label="重置" />
            <q-btn @click="setUnitData(0)" color="primary" unelevated label="新增" />
...
export default {
  ...
  methods: {
    getList () {
      ...
      this.$store
        .dispatch('clbank/getUnitDataList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    delAll () {
      this.$q
        .dialog({
          title: '是否确认',
          message: `确定删除 ${this.gridOptions.api.getSelectedRows().length} 条材料吗`,
          cancel: true
        })
        .onOk(() => {
          this.deleteRecord()
    ...
    deleteRecord () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('clbank/deleteData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
