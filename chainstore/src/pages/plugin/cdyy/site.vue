<template>
  <q-page class="fit">
        ...
        <q-card flat class="q-pa-sm">
          <div class="row q-gutter-x-sm">
            <div class="col-12 col-md-2">
              <q-input clearable label="请输入场地名称" outlined v-model="searchQuery" dense />
            </div>

            <q-btn @click="getList()" color="primary" unelevated label="搜索" />
            <q-btn @click="clearnBut()" color="warning" unelevated label="重置" />
            <q-btn @click="addSiteData(0)" color="primary" unelevated v-if="authorityMeta('add')" label="新增" />
            <q-btn
    ...
    <edit-site v-if="siteDialogStatus" @getMethods="getList" ref="editSiteRef"></edit-site>
  </q-page>
</template>
<script>
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          valueGetter: (p) => {
            return parseInt(p.data.cs_attribute) == 1 ? '全场' : '半场'
          }
        },
        {
          headerName: '状态',
  ...
  methods: {
    ...
    delAll () {
      this.$q
        .dialog({
          title: '是否确认',
          message: `确定删除 ${this.gridOptions.api.getSelectedRows().length} 条场地信息吗`,
          cancel: true
        })
        .onOk(() => {
          this.deleteRecord()
    ...
    deleteRecord () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('cdyy/delSiteData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
