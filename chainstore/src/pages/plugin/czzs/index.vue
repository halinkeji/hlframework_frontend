<template>
  <q-page>
    ...
    <div class="q-gutter-x-xs row q-pl-xs q-px-sm ">
      <q-input class="col-10 col-md-2 q-mt-sm" outlined v-model="inquireForm.invoiceCode" :label="`扣除|接收${$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit}单据号`" dense />
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '扣除' + this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit + '单据号',
          field: 'pcr_give_invoice_code',
          filter: false
        },
        {
          headerName: '接收' + this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit + '单据号',
