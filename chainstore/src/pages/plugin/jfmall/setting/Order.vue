    ...
    getItem () {
      this.$store.dispatch('jfSetting/getItem', null).then((res) => {
        if (parseInt(res.code) == 200) {
          ...
          this.orderSetting = res.data && res.data.js_order_set && res.data.js_order_set.deductRepertory
            ? res.data.js_order_set
            : {
                deductRepertory: 'order',
                deductWealth: 'order',
                cancelOrderDuration: 30,
    ...
    saveData () {
      this.$refs.addSettingRef.validate().then((success) => {
        if (success) {
          ...
          this.settingForm.js_order_set = this.orderSetting
          const obj = {
            ...this.settingForm
          }
          this.$store
