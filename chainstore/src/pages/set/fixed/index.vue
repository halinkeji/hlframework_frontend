    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-xs">
        <q-tabs @input="getFixedmoneyList" class="bg-white q-mt-sm" active-color="primary" indicator-color="primary" align="left" content-class="text-grey-9" narrow-indicator dense v-model="activeTab">
          <q-tab name="1" :label="`增加/扣除${$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_integralUnit}操作`" />
          <q-tab name="2" label="快速消费" />
          <q-tab name="3" label="会员充值" />
          <q-tab name="4" :label="`${$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit}扣费`" />
        </q-tabs>
        <q-space />
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          valueGetter: (p) => {
            if (p.data.fm_type == 1) {
              return '增加/扣除' + this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_integralUnit
            }
            if (p.data.fm_type == 2) {
              return '快速消费'
            }
            ...
            if (p.data.fm_type == 4) {
              return this.$q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit + '扣费'
            }
          },
          width: this.nowpx(0.25) + 'px'
        },
