  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2">
        ...
        会员{{ $q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}分析
      </div>
                ...
                <div class="q-px-sm col-12 q-pt-sm" v-for="(item, key) in totalCount" :key="key">
                  <q-card flat bordered>
                    <q-card-section align="center" class="q-py-lg">
                      <div class="text-h5">
                        {{ $q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ formatAmount(item.value) }}
                      </div>
                    </q-card-section>
                    <q-separator />
                    ...
                    <q-tr :props="props">
                      <q-td key="add" :props="props">
                        <span class="text-caption">
                          {{ $q.localStorage.getItem('chainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}</span
                        >
