<template>
 ...
 <div style="height:200px" class="con-example-images q-pa-xs">
            ...
            <q-card-actions align="center">
              <q-btn label="选择套餐" class="full-width" @click="addShoppingCart(item,item.id)" color="primary" unelevated />
            </q-card-actions>
          </q-card>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 添加至购物车
    addShoppingCart (item, packageId) {
      this.packagePopup = false
      this.$emit('dataList', item, packageId)
    }
  }
}
</script>
