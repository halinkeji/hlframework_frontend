    ...
    <q-header elevated class="bg-primary text-white" height-hint="98">
      <q-toolbar>
        ...
        <q-btn
          ...
          size="sm"
        />
        <!-- <q-avatar class="q-mb-sm" size="md">
          <q-img ratio="1" :src="`${$q.localStorage.getItem('chainstore_system_local').attach_url}` + store_info.logo">
              ...
              <q-item v-bind="scope.itemProps" v-on="scope.itemEvents" class="text-black">
                <q-item-section avatar>
                  <q-avatar>
                    ...
                    <q-img :src="`${$q.localStorage.getItem('chainstore_system_local').attach_url}` + scope.opt.logoData.path">
                      <template v-slot:error>
                        <div class="absolute-full flex flex-center">x</div>
                      </template>
                    </q-img>
...
export default {
  ...
  computed: {
    ...
    current_title () {
      if (LocalStorage.getItem('chainstore_apply_local')) {
        const systemLocal = LocalStorage.getItem('chainstore_apply_local')
  ...
  created () {
    if (LocalStorage.getItem('chainstore_apply_local')) {
      const copyright = LocalStorage.getItem('chainstore_apply_local').copyright
    ...
    this.getWxappQrcode()
    this.getMenuData()
    const user_info = LocalStorage.getItem('chainstore_user_info')
  ...
  methods: {
        ...
        this.$store
          ...
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$q.localStorage.set('chainstore_account_info', account_info)
              const storeLocal = this.$q.localStorage.getItem('chainstore_store_local')
              storeLocal.storeInfo = res.data
              this.$q.localStorage.set('chainstore_store_local', storeLocal)
    ...
    getMenuData () {
      const chainstore_left_menu_navigate_local = LocalStorage.getItem('chainstore_left_menu_navigate_local')

      this.linksData = chainstore_left_menu_navigate_local
    },
    logoutAccount () {
    ...
    updateMenuData () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            LocalStorage.set('chainstore_system_local', res.data.system_local)
            LocalStorage.set('chainstore_apply_local', res.data.apply_local)
            LocalStorage.set('chainstore_store_local', res.data.store_local)
            LocalStorage.set('chainstore_plugins_local', res.data.plugins_local)

            const allRouter = this.getFormatRoutes(res.data.router)
            LocalStorage.set('chainstore_router_local', res.data.router)
            routes[1].children = allRouter

            this.$router.addRoutes(routes)
            LocalStorage.set('chainstore_left_menu_navigate_local', res.data.menu)
