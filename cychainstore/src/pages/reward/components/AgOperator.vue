<template>
  <div>
    <q-btn color="secondary" class=" q-mx-sm" unelevated v-if="authorityMeta('join')" label="生成绩效汇总" @click="buttonShow" />
...
export default {
  ...
  methods: {
    ...
    deleteRecord () {
      this.$store
        .dispatch('reward/delData', [this.dataId])
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              message: '成功',
