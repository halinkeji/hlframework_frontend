    ...
    >
      <div class="fit column  no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2 row">
          <div class="text-subtitle2">{{ dp_name }}-结账</div>
          <q-space />
...
export default {
  ...
  methods: {
    ...
    getDeskSettleDetail () {
      this.$store
        .dispatch('consumeorder/getDeskSettleDetail', { currentDeskId: this.currentDeskId, memberId: this.memberId })
        .then((res) => {
    ...
    settleOrder () {
      ...
      this.$nextTick(() => {
        // 消费类型 钱 会员Id 需要返回的值
        this.$refs.checkoutRef.checkoutPageReceive('consumeorder', this.formatAmount(this.realPayMoney), this.memberId, params)
      })
    },
    ...
    paySuccessCallBack (v) {
      ...
      this.orderCheckout = false
      this.$store
        .dispatch('consumeorder/orderSetData', v)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              message: '成功',
