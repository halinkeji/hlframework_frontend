<template>
  <q-page class="fit row">
        ...
        <div class="col-shrink ">
          <q-card flat square bordered>
            <q-toolbar class="text-dark ">
              ...
              <q-btn :to="{ name: 'consumeOrder' }" class="text-subtitle2" dense  flat  >
                 <q-icon size="xs" name="las la-angle-left"></q-icon>返回
              </q-btn>
              <q-space />
              <q-toolbar-title class="text-subtitle2 text-right"> {{ deskData.dp_name }} / {{ deskData.dol_people_number }}人 </q-toolbar-title>
            </q-toolbar>
            <q-separator />
          </q-card>
        </div>
    ...
    <div class="col-shrink col-xs-2 col-sm-2 col-md-1 col-lg-1 col-xl-1 q-pa-xs bg-white column">
      ...
      <!-- <q-btn unelevated color="green" label="会员" @click="openReadingDialog" class="full-width text-h6 q-my-xs" v-if="!currentMemberInfo.id" />
      <q-btn unelevated color="negative" label="重置" @click="resetReadCard" class="full-width text-h6 q-my-xs" v-else /> -->
      <q-btn
        unelevated
        color="deep-orange"
        label="清空"
        ...
        class="full-width text-h6 q-my-xs"
      />
      <q-btn unelevated color="red" label="清台" @click="clearDesk" class="full-width text-h6 q-my-xs" />
      <q-btn unelevated color="green" label="对单" class="full-width text-h6 q-my-xs" @click="printer" />
      <q-btn unelevated color="deep-orange" label="下单" class="full-width text-h6 q-my-xs" @click="showPrincipal(currentDeskId)" />

      <q-btn
        v-if="parseInt(deskData.dp_status) == 3"
        unelevated
        color="red"
        label="结账"
    ...
    <principal ref="principalRef" v-if="principalPoppupStatus" @dataList="hidePrincipal" />
...
export default {
  name: 'addOrder',
  components: { Spec, Principal, Remarks, OrderSettle },
  data () {
    return {
      orderCheckout: false,
  ...
  created () {
    ...
    if (this.$route.query.id > 0) {
      ...
      this.linkData.dol_desk_id = this.$route.query.id
      this.carData.doc_desk_id = this.$route.query.id
      this.currentDeskId = this.$route.query.id
    }
    this.getList()
  ...
  methods: {
    init (deskId) {
      if (deskId > 0) {
        this.$store
          .dispatch('consumeorder/getDeskLinkData', { deskId })
          .then((res) => {
    ...
    getDishesTypeList () {
      this.$store
        .dispatch('consumeorder/getFoodTypeData')
        .then((res) => {
          if (res.code == 200) {
    ...
    setDeskCarData (params) {
      this.$store
        .dispatch('consumeorder/setDeskCarData', params)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              message: '添加购物车成功',
    ...
    delDishes (id) {
      const params = [id]
      this.$store.dispatch('consumeorder/delAllDeskCar', params).then((res) => {
        if (res.code == 200) {
          this.$q.notify({
            message: '成功',
            caption: '成功删除菜品!',
    ...
    clearCart () {
      this.$q
        .dialog({
          title: '提示',
          message: `确定清空 ${this.deskData.dp_name} 的购物车吗？`,
          cancel: true
        })
        .onOk(() => {
          const params = {
          ...
          this.$store.dispatch('consumeorder/clearCart', params).then((res) => {
            if (res.code == 200) {
              this.$q.notify({
                message: '清空购物车成功',
                icon: 'ion-checkmark-circle-outline',
    ...
    clearDesk () {
      this.$q
        .dialog({
          title: '提示',
          message: `确定清空 ${this.deskData.dp_name} 的桌位吗`,
          cancel: true
        })
        .onOk(() => {
          const params = {
          ...
          this.$store.dispatch('consumeorder/clearDesk', params).then((res) => {
            if (res.code == 200) {
              this.$q.notify({
                message: '清台成功',
                icon: 'ion-checkmark-circle-outline',
    ...
    showPrincipal (id) {
      ...
      if (this.carListData.length > 0) {
        this.principalPoppupStatus = true
        this.$nextTick(() => {
          this.$refs.principalRef.getDesk(id)
        })
      }
    },
    ...
    paySuccessCallBack () {
      this.orderSettleRefStatus = false
      this.$router.push({ name: 'consumeOrder' })
    },
    ...
    printer () {
      ...
      this.$store
        .dispatch('consumeorder/setPrinterCheck', carData)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              message: '打印成功',
