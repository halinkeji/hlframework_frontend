  ...
  <q-card flat>
    <q-card-section>
      ...
      <div class="row q-gutter-md">
        <q-btn unelevated color="indigo" v-if="authorityMeta('rfid')" label="绑定实体卡" size="12px" class="q-px-md" icon="las la-sim-card" @click="bindPhysicalCard" />
        <q-btn unelevated color="red-5" v-if="authorityMeta('changeCode')" label="换卡" size="12px" class="q-px-md" icon="lar la-credit-card" @click="changeNumber" />
        <q-btn unelevated color="blue-6" label="门店改绑" size="12px" class="q-px-md" icon="las la-pen" @click="changeBind" v-if="is_main_store && authorityMeta('changeStore')" />
        <q-btn unelevated color="red-5" v-if="authorityMeta('upgrade')" label="升级" size="12px" class="q-px-md" icon="las la-upload" @click="LevelChange" />
        <q-btn unelevated color="amber-8" v-if="authorityMeta('extension')" label="延期" size="12px" class="q-px-md" icon="las la-stopwatch" @click="delayChange" />
        <q-btn unelevated color="red-5" label="锁卡" size="12px" class="q-px-md" icon="las la-unlock" @click="lockCardPromptButton" v-if="memberInformation.mem_memberStatus == 1 && authorityMeta('locking')" />
        <q-btn unelevated color="blue-6" label="解锁" size="12px" class="q-px-md" icon="las la-pen" @click="handleCloseButton" v-if="memberInformation.mem_memberStatus == 2 && authorityMeta('locking')" />
          ...
          <div class="row q-mt-xs q-gutter-xs">
            <div class="col-md-3 col-12 ">
              <q-input
                ...
                disable
                standout
                :label="`总${$q.localStorage.getItem('cychainstore_store_local').storeBaseConfig.consume_base_balanceUnit}`"
                class="q-mt-md"
              >
              </q-input>
            </div>
            <div class="col-md-3 col-12">
              <q-input
                ...
                standout
                dense
                :label="`总${$q.localStorage.getItem('cychainstore_store_local').storeBaseConfig.consume_base_integralUnit}`"
                class="q-mt-md"
              >
              </q-input>
            </div>
