    ...
    >
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2 q-pa-sm row">
          新增{{ $q.localStorage.getItem('cychainstore_store_local').storeBaseConfig.consume_base_integralUnit }}折扣设置
          <q-space />
            ...
            <div class="row">
              ...
              <div class="col-6">
                <q-select
                  ...
                  dense
                  lazy-rules
                  :rules="[(val) => !!val || '是否在会员自助买单时开启']"
                  v-model="integralData.ic_is_member_use"
              ...
              <div class="row">
                ...
                <div class="col-6">
                  <q-input
                    ...
                    :label="`${$q.localStorage.getItem('cychainstore_store_local').storeBaseConfig.consume_base_integralUnit}换算比例`"
                    :error="false"
                    lazy-rules
                    :rules="[
                      (val) => (val > -1 && val <= 100) || '请输入正确的' + $q.localStorage.getItem('cychainstore_store_local').storeBaseConfig.consume_base_integralUnit + '换算比例',
                      (val) => /^[0-9]+([.]\d{1,2})?$/.test(val) || '请输入正确的' + $q.localStorage.getItem('cychainstore_store_local').storeBaseConfig.consume_base_integralUnit + '换算比例',
                    ]"
                  >
            ...
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  ...
                  position: 'top-right',
                  message: '成功',
                  caption: '新增' + this.$q.localStorage.getItem('cychainstore_store_local').storeBaseConfig.consume_base_integralUnit + '折扣设置成功',
                  color: 'green'
                })
    ...
    getItem () {
      this.$store.dispatch('integralset/getScene').then((res) => {
        if (res.code == 200) {
          ...
          const openPlugins = LocalStorage.getItem('cychainstore_plugins_local').openPlugins
