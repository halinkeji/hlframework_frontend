  ...
  <q-dialog @hide="hideNowPage" v-model="showDialog" persistent>
    ...
    >
      ...
      <div align="right" class="q-mt-xl">
        <q-btn unelevated color="primary" label="保存" @click="setData" class="q-px-xs" v-if="authorityMeta('saveTemplate')" />
        <q-btn unelevated label="关闭" class="q-ml-sm" color="grey" v-close-popup v-if="authorityMeta('backTemplate')" />
      </div>
    </q-card>
  </q-dialog>
</template>
