...
const currentRouterData = LocalStorage.getItem('cychainstore_router_local')
  ...
  Store.dispatch('user/roleInfo')
    .then(res => {
      if (res.data) {
        ...
        LocalStorage.set('cychainstore_plugins_local', res.data.plugins_local)
        ...
        // 写入左侧菜单
        LocalStorage.set('cychainstore_left_menu_navigate_local', menu)
        store().commit('settings/SET_MENUData', menu)

        // 写入业务菜单
        LocalStorage.set(
          'cychainstore_business_menu_navigate_local',
          businessMenu
        )
