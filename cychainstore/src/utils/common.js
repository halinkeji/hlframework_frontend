...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('cychainstore_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('cychainstore_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('cychainstore_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('cychainstore_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'cychainstore') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('cychainstore_access_token', Cookies.get('access_token'))
  LocalStorage.set('cychainstore_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('cychainstore_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('cychainstore_token_type', Cookies.get('token_type'))
  LocalStorage.set('cychainstore_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'cychainstore_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('cychainstore_user_info', Cookies.get('user_info'))
  LocalStorage.set('cychainstore_account_info', Cookies.get('account_info'))
