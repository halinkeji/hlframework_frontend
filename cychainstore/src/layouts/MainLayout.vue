                  ...
                  <q-expansion-item expand-icon-class="text-dark" expand-separator :label="i.name" v-if="i.children && i.children.length > 0">
                    <q-list separator class="text-dark full-width">
                      <q-item
                        ...
                        v-ripple
                        active-class="my-menu-link bg_deep text-white"
                        :to="{ path: value.to }"
                        :key="k"
          ...
          <div class="col text-right row">
            ...
            <div v-if="availableStoreList && availableStoreList.length > 1">
              <q-select
                ...
                option-value="id"
                option-label="name"
                options-selected-class="bg_shallow text-white"
                ...
                <template v-slot:option="scope">
                  <q-item v-bind="scope.itemProps" v-on="scope.itemEvents" class="text-black">
                    <q-item-section avatar>
                      <q-avatar>
                        <q-img :src="scope.opt.logo ? `${$q.localStorage.getItem('cychainstore_system_local').attach_url}` + scope.opt.logo : ''"
                    ...
                    <q-item-section>
                      <q-btn outline rounded color="bg_shallow" size="sm" label="进入" />
                    </q-item-section>
                  </q-item>
                </template>
              </q-select>
      ...
      <div class="row full-width full-height bg_shallow">
        <div class="col-12">
          <q-scroll-area class="fit">
            ...
            <q-tabs v-model="tab" vertical indicator-color="transparent" class=" text-white" active-bg-color="teal" active-class="text-dark bg_deep text-h6 ">
              <q-route-tab
                :name="item.name"
                :to="{ path: item.to }"
...
export default {
  ...
  computed: {
    ...
    mobileRegUrl () {
      ...
      if (account_info && system_local) {
        return system_local.systemInfo.site_url + '/m/cychainstore/?id=' + account_info.lib_id
      }
      return ''
    },
    current_title () {
      if (LocalStorage.getItem('cychainstore_apply_local')) {
        const systemLocal = LocalStorage.getItem('cychainstore_apply_local')
  ...
  created () {
    if (LocalStorage.getItem('cychainstore_apply_local')) {
      const copyright = LocalStorage.getItem('cychainstore_apply_local').copyright
    ...
    this.getWxappQrcode()
    this.getMenuData()
    const user_info = LocalStorage.getItem('cychainstore_user_info')
    this.userName = user_info ? user_info.username : '未知'

    const storeInfo = this.$q.localStorage.getItem('cychainstore_store_local')
  ...
  methods: {
    getBusinessMenuData () {
      this.businessMenuData = LocalStorage.getItem('cychainstore_business_menu_navigate_local')
    },
    nowTime () {
      const that = this
    ...
    getMenuData () {
      const cychainstore_left_menu_navigate_local = LocalStorage.getItem('cychainstore_left_menu_navigate_local')

      this.linksData = cychainstore_left_menu_navigate_local
    },
    logoutAccount () {
      this.$q
        .dialog({
    ...
    updateMenuData () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            LocalStorage.set('cychainstore_system_local', res.data.system_local)
            LocalStorage.set('cychainstore_apply_local', res.data.apply_local)
            LocalStorage.set('cychainstore_store_local', res.data.store_local)
            LocalStorage.set('cychainstoreplugins_local', res.data.plugins_local)

            const allRouter = this.getFormatRoutes(res.data.router)
            LocalStorage.set('cychainstore_router_local', res.data.router)
            routes[1].children = allRouter

            this.$router.addRoutes(routes)
            LocalStorage.set('cychainstore_left_menu_navigate_local', res.data.menu)
        ...
        this.$store
          ...
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$q.localStorage.set('cychainstore_account_info', account_info)

              const storeLocal = this.$q.localStorage.getItem('cychainstore_store_local')
              storeLocal.storeInfo = res.data

              this.$q.localStorage.set('cychainstore_store_local', storeLocal)
