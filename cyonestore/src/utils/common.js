...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('cyonestore_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('cyonestore_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('cyonestore_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('cyonestore_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'cyonestore') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('cyonestore_access_token', Cookies.get('access_token'))
  LocalStorage.set('cyonestore_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('cyonestore_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('cyonestore_token_type', Cookies.get('token_type'))
  LocalStorage.set('cyonestore_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'cyonestore_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('cyonestore_user_info', Cookies.get('user_info'))
  LocalStorage.set('cyonestore_account_info', Cookies.get('account_info'))
