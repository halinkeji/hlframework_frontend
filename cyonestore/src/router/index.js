...
const currentRouterData = LocalStorage.getItem('cyonestore_router_local')
  ...
  Store.dispatch('user/roleInfo')
    .then(res => {
      if (res.data) {
        ...
        // 写入左侧菜单
        LocalStorage.set('cyonestore_left_menu_navigate_local', menu)
        store().commit('settings/SET_MENUData', menu)

        // 写入业务菜单
        LocalStorage.set('cyonestore_business_menu_navigate_local', businessMenu)
