        ...
        <div class="col-shrink q-pa-sm full-height">
          <q-scroll-area class="fit full-height">
            <q-form ref="addTemplateRef">
              ...
              <q-select
                outlined
                v-model="memberData.pmm_level_id"
...
export default {
  ...
  methods: {
    getMemberInfo (val) {
      if (val) {
        if (val.id) {
          this.memberData.pmm_member_id = val.id
        }
      }
    },
    getConfig () {
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            if (res.data && res.data.pmc_form_id) {
              this.memberData.pmm_form_id = res.data.pmc_form_id
              this.$refs.templateComponentsRef.getList({
                formId: res.data.pmc_form_id
                // memberId: 1,
                // otherId: 1
              })
            }
    ...
    setWeights () {
      ...
      const findIndex = _.findIndex(this.levelOptions, function (o) {
        return parseInt(o.id) == parseInt(that.memberData.pmm_level_id)
      })
      if (findIndex > 0) {
        const levelItem = this.levelOptions[findIndex]
        if (levelItem && levelItem.id) {
          that.memberData.pmm_weights = levelItem.pml_weights
        }
      }
    },
    ...
    saveData () {
      this.$refs.addTemplateRef.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch('market/setMarketMember', obj)
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
