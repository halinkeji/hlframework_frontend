      ...
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <div class="row  q-gutter-x-sm">
          ...
          <q-btn
            ...
            @click="delAll()"
          />
          <q-btn class="col-12 col-md-1 q-mt-sm" unelevated color="grey" :to="{ name: 'bmtk' }" label="返回" />
        </div>
      </div>
...
export default {
  ...
  methods: {
    getList () {
      ...
      this.$store
        .dispatch('bmtk/getDetailList', obj)
        .then((res) => {
          if (res.code == 200) {
            if (res.data) {
    ...
    deleteRecordBatch () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('bmtk/sendStatus', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
