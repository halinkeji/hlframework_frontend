    ...
    >
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white q-px-xs q-pb-md row q-pa-xs">
          <div class="text-subtitle2">{{ operatingText }}用户</div>
        </div>
        ...
        <div class="col-shrink q-px-sm full-height">
          <q-form ref="userDataRef">
            ...
            <q-input
              ...
              label="用户账号"
              lazy-rules
              :disable="user_data.id ? true : false"
              :rules="[
                (val) => !!val || '用户名称为必填项',
                (val) => !/[\u4E00-\u9FA5]|[\uFE30-\uFFA0]/g.test(val) || '不可以有中文',
                (val) => !/\s/g.test(val) || '不可以有空格',
                (val) => !specialString.test(val) || '不可以有特殊字符',
              ]"
            />
