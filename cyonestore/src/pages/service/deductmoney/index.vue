  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2">
        ...
        {{ $q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_balanceUnit }}扣费
      </div>
              ...
              <q-card-section class="q-py-sm" v-if="fixedMoneyList.length > 0">
                <div class="row">
                  <div class="col-6 col-md-3 q-mt-md q-px-xs" v-for="item in fixedMoneyList">
                    <q-btn unelevated rounded color="green" class="q-px-md full-width" :disable="memberId == 0" @click="deductAmount = item.fm_parameter"> {{ $q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ item.fm_parameter }} </q-btn>
                  </div>
                </div>
              </q-card-section>
            </q-card>
          ...
          this.$store
            ...
            .then((res) => {
              if (res.code == 200) {
                ...
                this.$q.notify({
                  message: '失败',
                  caption: '扣除失败,' + this.$q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_balanceUnit + '不足',
