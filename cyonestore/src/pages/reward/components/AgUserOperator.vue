<template>
  <div>
    ...
    <q-btn color="info" unelevated class="q-mr-sm" label="生成二维码" @click="creatQrCodeM(dataId)" /> -->
  ...
  methods: {
    confirmDeleteRecord () {
      this.$q
        ...
        .onOk(() => {
          this.$store
            .dispatch('reward/delUserData', [this.dataId])
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
    ...
    creatQrCodeM (val) {
      ...
      const accountInfo = this.$q.localStorage.getItem('cyonestore_account_info')
      const systemLocal = this.$q.localStorage.getItem('cyonestore_system_local')
