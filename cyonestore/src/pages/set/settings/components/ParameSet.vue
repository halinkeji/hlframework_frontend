        ...
        <q-form ref="paramsForm" class="q-gutter-md row">
          <q-card flat class="bg-white q-pa-md col-6">
            <q-list separator bordered padding>
              ...
              <q-item>
                <q-item-section>
                  <q-item-label>积分消费是否需要密码</q-item-label>
                  <q-item-label caption>默认关闭，开启后收银台积分消费时需要密码进行验证</q-item-label>
                </q-item-section>
              ...
              <q-item>
                <q-item-section>
                  <q-item-label>储值消费是否需要密码</q-item-label>
                  <q-item-label caption>默认关闭，开启后收银台储值消费时需要密码进行验证</q-item-label>
                </q-item-section>
                <q-item-section side top>
                  <q-toggle
                    color="green"
              ...
              <q-item>
                <q-item-section>
                  <q-item-label>{{ $q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_integralUnit }}换算比例</q-item-label>
                  <q-item-label caption>默认：1 {{ $q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_moneyUnit }} = 1 {{ $q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_integralUnit }}。（示例：输入 3 ，则 1 {{ $q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_moneyUnit }} = 3 {{ $q.localStorage.getItem('cyonestore_store_local').storeBaseConfig.consume_base_integralUnit }}） </q-item-label>
                </q-item-section>
                <q-item-section side>
                  <q-input
                    outlined
              ...
              <q-item>
                <q-item-section>
                  <q-item-label>储值消费免密支付额度</q-item-label>
                  <q-item-label caption>默认‘0’，当时开启储值消费使用密码时，判断使用金额是否免密</q-item-label>
                </q-item-section>
                <q-item-section side>
                  <q-input
                    outlined
