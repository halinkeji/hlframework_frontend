<template>
  <div class="col-auto  self-start q-mx-lg ">
    <q-form @submit="onSubmit" class="q-gutter-md" :style="{ 'max-width': $q.screen.width * 0.6 + 'px' }">
      ...
      <q-input dense outlined v-model="corp.contact_url" readonly label="客户联系接收事件服务器URL">
      ...
      <q-input dense outlined v-model="corp.book_url" readonly label="通讯录接收事件服务器URL">
        <template v-slot:append>
      ...
      <div class="text-subtitle2"><q-icon name="las la-square-full" class="text-primary" />关联商户</div>
      <q-separator />
      <q-input dense outlined v-model="corp.relation_app_id" label="AppID" />
      <q-input dense outlined v-model="corp.relation_app_secret" label="AppSecret" />
        ...
        .then((res) => {
          if (res.code == 200) {
            if (res.data) {
              ...
              if (systemLocal && systemLocal.info) {
                local_url = LocalStorage.getItem('qwscrm_system_local').systemInfo.site_url
              }

              this.corp.contact_url = res.data.contact_url ? res.data.contact_url : local_url + '/v1/contact/contact-callback?CorpId=' + res.data.id

              this.corp.book_url = res.data.book_url ? res.data.book_url : local_url + '/v1/worker/worker-callback?CorpId=' + res.data.id
