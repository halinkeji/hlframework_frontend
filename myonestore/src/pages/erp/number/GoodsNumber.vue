<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-sm q-pb-md q-gutter-x-xs q-py-sm">
      <div class="row q-gutter-sm">
        <div class="col-12 col-md-4">
          <q-input clearable outlined v-model="searchNumber" label="商品名称/条码/助记码/简称" dense />
        </div>
        <div class="col-12 col-md-4">
          <q-select
            outlined
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '售价(' + this.$q.localStorage.getItem('myonestore_store_local').storeBaseConfig.consume_base_moneyUnit + ')',
          field: 'goo_price',
          filter: false,
          width: this.nowpx(0.1) + 'px',
          rowSpan: this.rowSpan,
  ...
  methods: {
    ...
    rowSpan (params) {
      ...
      if (cellValue && fields.includes(property)) {
        ...
        let countRowspan = 0
        if (prevRow && prevRow[property] === cellValue && prevRow.goods_id == row.goods_id && prevRow.goods_id == row.goods_id) {
          console.log(2)
          return countRowspan
        } else {
          countRowspan = 1
          // && prevRow && prevRow.goods_id && prevRow.goods_id == row.goods_id
          while (nextRow && nextRow[property] === cellValue && nextRow.goods_id == row.goods_id) {
