            ...
            <div class="q-pa-xs col-xs-12 col-sm-6 col-md-2">
              <q-card flat bordered @click="getSelectedGoodsData(props.row)">
                <q-img
                  ...
                  :ratio="1"
                  :src="$q.localStorage.getItem('myonestore_system_local').attach_url + props.row.coverImage.path"
                />
...
export default {
  ...
  methods: {
    ...
    getGoodsType () {
      this.$store
        .dispatch('goodstype/getAllData', { property: 2 })
        .then((res) => {
          if (res.code == 200) {
    ...
    getSelectedGoodsData (item) {
      ...
      if (item) {
        ...
        goodsItem.rtb_goods_id = item.id
