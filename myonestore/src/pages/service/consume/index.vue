  ...
  <div class="fit row no-wrap justify-between items-stretch content-stretch">
    ...
    <div class="col-shrink col-xs-4 col-sm-5 col-md-5 col-lg-5 col-xl-5">
      <left-layout
        ...
        :meiyeRewardData="meiyeRewardData"
        ref="shoppingCarRef"
      ...
      <div class="fit column no-wrap justify-between items-stretch content-stretch">
        <div class="col-shrink q-px-xs">
          ...
          <q-btn
            color="negative"
            class="full-width q-my-xs q-py-sm"
            @click="cancelConsumeUseCoupon"
            padding="none"
            v-if="couponInfo && couponInfo.id"
          >
            <span class="text-subtitle1 q-my-xs text-white"> 撤销卡券 </span>
          </q-btn>
        ...
        <div class="col-shrink q-px-xs" v-if="authorityMeta('settlement')">
          <q-btn
            class="full-width q-py-sm q-mb-xs bg_cinnabar"
            no-caps
            @click="rewardLabel()"
            :disable="shoppingCartsList && shoppingCartsList.length > 0 ? false : true"
          >
        ...
        <q-card-section>
          <q-card-section class="row">
            <div class="col-6 col-md-3 q-mt-md q-px-xs" v-for="(item, key) in priceDiscountList">
              <q-btn unelevated rounded color="green" @click="changeDiscount(item)" class="q-px-md full-width"> {{ item }}% </q-btn>
            </div>
          </q-card-section>
    ...
    <dialog-coupon-check
      ref="checkCouponDialogRef"
      v-if="checkCouponDialogStatus"
      @colseDialog="checkCouponDialogStatus = false"
      @setUseCoupon="getCouponCheck"
    />
  </div>
</template>
...
export default {
  ...
  computed: {},
  created () {
    if (this.$route.query.reserveId > 0) {
      this.getReserveUpOrder(this.$route.query.reserveId)
    }
  },
  watch: {},
  mounted () {
  ...
  methods: {
    ...
    getReserveUpOrder (reserveId) {
      const obj = {
        reserveId
      }
      this.$store
        .dispatch('reserve/reservePendingOrder', obj)
        .then((res) => {
          if (res.code == 200 && res.data) {
            this.solutionsHanging(res.data)
          }
        })
        .catch((error) => {})
    },
    rewardLabel () {
      ...
      this.$nextTick(() => {
        this.$refs.labelData.openStaffLabel(this.shoppingCartsList, 'consume')
      })
    },
    getRewardLabel (val) {
