  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md">
      <div class="col-md-2 col-12 q-pt-none">
        <q-input outlined bottom-slots v-model="text" dense class="q-py-none" label="请输入任务名称"> </q-input>
      </div>
      <q-btn unelevated color="primary" label="搜索" class="q-px-lg" />
      <q-btn unelevated color="amber" label="重置" class="q-px-lg" @click="text = ''" />
      <q-btn unelevated color="primary" label="添加群发任务" @click="medium = true" />
      <!-- 弹窗 -->
...
export default {
  ...
  data () {
    return {
      ...
      options: ['1', '2', '3'],
      modelOne: null,
      optionsOne: ['申请确认通知', '会员消费提醒', this.$q.localStorage.getItem('myonestore_store_local').storeBaseConfig.consume_base_integralUnit + '兑换成功通知'],
      modelT: null,
      optionsT: ['申请确认通知', '会员消费提醒', this.$q.localStorage.getItem('myonestore_store_local').storeBaseConfig.consume_base_integralUnit + '兑换成功通知'],
      modelFan: null,
