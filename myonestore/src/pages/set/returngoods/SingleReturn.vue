    ...
    >
      ...
      <q-form ref="singleReturnRef" class="q-gutter-md">
        <div class="q-my-lg">
          ...
          <q-input outlined dense stack-label class="q-pa-none q-mx-lg q-mb-lg" v-model="returnPoint" disable :label="`扣除${$q.localStorage.getItem('myonestore_store_local').storeBaseConfig.consume_base_integralUnit}`" />
            ...
            .onOk(() => {
              if (this.returnMode == 2) {
                if (parseInt(this.member_id) < 1) {
                  this.$q.notify({
                    message: '退货失败',
                    caption: '原因：非会员不能退款到' + this.$q.localStorage.getItem('myonestore_store_local').storeBaseConfig.consume_base_balanceUnit,
                    color: 'red',
