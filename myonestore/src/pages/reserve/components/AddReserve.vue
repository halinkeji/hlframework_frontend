  ...
  <q-dialog @hide="hideNowPage" v-model="showDialog">
    ...
    >
      <q-form ref="addReserveRef">
        ...
        <div class="q-mt-xs">
          <q-input outlined dense type="textarea" v-model="detailData.prd_remark" label="备注" />
        </div>
          ...
          <div v-for="(item, key) in reserveData.reserveTime">
            <q-radio v-model="detailData.prd_time_id == item.id ? item.id : detailData.prd_time_id" :val="item.id" @input="getReserve(item)">
              <span
                ...
                -
                {{ $q_date.formatDate(new Date(item.rpt_time_end * 1000), 'HH:mm') }}</span
              ></q-radio
            >
          </div>
        </div>
        <div class="q-mt-sm">
          ...
          <q-date v-model="reserveDate" minimal :events="reserveData.events" :options="reserveOptions" @input="judgmentDate" class="full-width" />
        </div>
