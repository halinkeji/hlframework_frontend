      ...
      <div class="row items-center " v-if="$q.screen.gt.md">
        <div class="col-12 col-md-4 q-pa-md">
          <q-input outlined @keyup.enter.native="cardReader" v-model="card_number" ref="readCardInput" :dense="dense" class="full-width" label="请输入卡号/身份证/手机号">
            <template v-slot:after>
              <q-btn unelevated label="读卡(D)" @click="cardReader()" class="q-px-md full-height bg_cinnabar text-white" />
            </template>
          </q-input>
        </div>
        <div class="col-12 col-md-7" v-if="userData.id">
          <div class="row q-pa-xs q-mt-sm">
            ...
            <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">
                总{{ $q.localStorage.getItem('myonestore_store_local').storeBaseConfig.consume_base_integralUnit }}：{{ userData.mem_memberPointsCount > 0 ? userData.mem_memberPointsCount : '0' }}
              </q-chip>
            </div>
            <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">
                总{{ $q.localStorage.getItem('myonestore_store_local').storeBaseConfig.consume_base_balanceUnit }}：{{ userData.mem_memberMoneyCount > 0 ? userData.mem_memberMoneyCount : '0' }}
              </q-chip>
            </div>
            <div class="col-6 col-md-3">
          ...
          <q-item>
            <q-item-section avatar>
              <q-avatar v-if="userData.coverImage && userData.coverImage.id" text-color="white">
                <img :src="$q.localStorage.getItem('myonestore_system_local').attach_url + userData.coverImage.path" />
              </q-avatar>
...
export default {
  ...
  mounted () {
    this.focusInput()
    this.openPlugins = this.$q.localStorage.getItem('myonestore_plugins_local').openPlugins
