    ...
    <q-dialog v-model="updatePasswordPopup">
      ...
      >
        ...
        <q-form autocorrect="off" ref="userFormRef" autocapitalize="off" autocomplete="off" spellcheck="false">
          ...
          <q-input outlined dense v-model="localUserInfo.mobile" label="手机号" disable lazy-rules bottom-slots />
          ...
          >
            ...
            <template v-slot:append>
              <q-btn flat @click="sendSmsCode" color="gery" :disable="smsSendDisableStatus" :label="smsSendStatus" />
            </template>
          </q-input>
...
export default {
  ...
  methods: {
    updatePassword () {
      const accountInfo = this.$q.localStorage.getItem('myonestore_account_info')
      this.user_data.id = accountInfo.user_id

      const userInfo = this.$q.localStorage.getItem('myonestore_user_info')
