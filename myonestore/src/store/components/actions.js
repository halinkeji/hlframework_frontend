...
let siteUrl = ''
if (LocalStorage.getItem('myonestore_system_local')) {
  const systemInfo = LocalStorage.getItem('myonestore_system_local').systemInfo
...
export function getDepotGoods ({ commit }, params) {
  return new Promise((resolve, reject) => {
    api
      .get('v3/erpnumber/depot-goods-number', { params })
      .then((response) => {
        resolve(response)
      })
      .catch((error) => {
...
export function fileUpload ({ commit }, item) {
  return new Promise((resolve, reject) => {
    const apiUrl = 'v3/in-out-file/upload-file'
    api
      .post(apiUrl, item)
      .then(response => {
        resolve(response)
