    ...
    <q-card flat bordered class="bg-white" style="width: 900px; max-width: 80vw;">
      <q-card-section class="q-pa-none q-px-md">
        <div class="row items-center no-wrap">
          <div class="col">
            <div class="text-subtitle2">导入商品档案</div>
          </div>

          <div class="col-auto">
            <q-btn color="grey-7" class="q-pa-none" round flat @click="closeUpload" v-close-popup icon="ion-close-circle-outline"> </q-btn>
          </div>
        </div>
      </q-card-section>
      ...
      <q-banner inline-actions rounded class="bg-grey-1 text-black">
        <template v-slot:action>
          <q-btn
            ...
            label="下载导入模板"
            icon="get_app"
            href="https://cdn.halin.net/download/xlsx/服装版-商品档案导入模板.xlsx"
            target="_blank"
          />
        </template>
      </q-banner>
...
export default {
  ...
  methods: {
    ...
    uploadedGoods() {
      ...
      this.$store
        .dispatch('clothingArchives/uploadArchives', obj)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              message: '成功',
