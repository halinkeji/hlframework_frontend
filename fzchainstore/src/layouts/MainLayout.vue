<template>
  <q-layout view="hHh Lpr lFf" class="shadow-2 fit rounded-borders">
              ...
              <q-menu fit>
                <q-card class="full-width" flat bordered>
                  ...
                  <q-card-section class="text-center q-pa-xs">
                    <q-btn rounded @click="downloadMobileRegCode" class="bg-primary text-white" label="下 载" />
                  </q-card-section>
                </q-card>
              </q-menu>
            </q-btn>
      ...
      <div class="row full-width full-height">
        <div :class="currentChildren && currentChildren.length > 0 ? 'col-5 text-left' : 'col-12'">
          <q-scroll-area class="fit">
            ...
            <q-tabs v-model="tab" vertical indicator-color="transparent" active-bg-color="primary" active-color="white" class="bg-primary text-white">
              <q-tab
                :name="item.name"
    ...
    <q-drawer side="right" v-model="drawerRight" bordered :width="150" :breakpoint="500" content-class="bg-grey-3 ">
      <q-scroll-area class="fit">
        <q-tabs v-model="nowStoreId" vertical active-class="bg-primary text-dark" class="bg-primary text-white">
          <q-tab
            :name="item.id"
            :key="key"
            v-for="(item, key) in availableStoreList"
            :icon="`img:${$q.localStorage.getItem('fzchainstore_store_local').attach_url}` + item.logo"
...
export default {
  ...
  computed: {
    ...
    storeEndTime () {
      ...
      if (system_local && system_local.storeUseEndTime) {
        const storeUseEndTime = system_local.storeUseEndTime
        return this.$q_date.formatDate(storeUseEndTime * 1000, 'YYYY年MM月DD日')
      }
      return '错误'
    },
    mobileRegUrl () {
      const account_info = this.$q.localStorage.getItem('fzchainstore_account_info')
      const system_local = this.$q.localStorage.getItem('fzchainstore_system_local')
      if (account_info && system_local) {
        return system_local.systemInfo.site_url + '/m/fzchainstore/?id=' + account_info.lib_id + '&StoreId=' + account_info.store_id
      }
      return ''
    },
    current_title () {
      if (LocalStorage.getItem('fzchainstore_apply_local')) {
        const systemLocal = LocalStorage.getItem('fzchainstore_apply_local')
  ...
  created () {
    if (LocalStorage.getItem('fzchainstore_apply_local')) {
      const copyright = LocalStorage.getItem('fzchainstore_apply_local').copyright
    ...
    this.getWxappQrcode()
    this.getMenuData()
    const user_info = LocalStorage.getItem('fzchainstore_user_info')
    this.userName = user_info ? user_info.username : '未知'

    const lib_local_info = this.$q.localStorage.getItem('fzchainstore_store_local')
    if (lib_local_info && lib_local_info.storeInfo) {
      this.store_info = lib_local_info.storeInfo
    }
    const account_info = LocalStorage.getItem('fzchainstore_account_info')
  ...
  methods: {
    ...
    getBusinessMenuData () {
      this.businessMenuData = LocalStorage.getItem('fzchainstore_business_menu_navigate_local')
    },
    nowTime () {
      const that = this
    ...
    getMenuData () {
      const fzchainstore_left_menu_navigate_local = LocalStorage.getItem('fzchainstore_left_menu_navigate_local')

      this.linksData = fzchainstore_left_menu_navigate_local
    },
    logoutAccount () {
      this.$q
        .dialog({
        ...
        this.$store
          ...
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$q.localStorage.set('fzchainstore_account_info', account_info)

              this.$q.localStorage.set('fzchainstore_store_local', {
                store: res.data
              })
    ...
    updateMenuData () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            LocalStorage.set('fzchainstore_system_local', res.data.system_local)
            LocalStorage.set('fzchainstore_apply_local', res.data.apply_local)
            LocalStorage.set('fzchainstore_store_local', res.data.store_local)
            LocalStorage.set('fzchainstore_plugins_local', res.data.plugins_local)

            const allRouter = this.getFormatRoutes(res.data.router)
            LocalStorage.set('fzchainstore_router_local', res.data.router)
            routes[1].children = allRouter

            this.$router.addRoutes(routes)
            LocalStorage.set('fzchainstore_left_menu_navigate_local', res.data.menu)
