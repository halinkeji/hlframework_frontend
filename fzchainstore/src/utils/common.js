...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('fzchainstore_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('fzchainstore_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('fzchainstore_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('fzchainstore_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'fzchainstore') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('fzchainstore_access_token', Cookies.get('access_token'))
  LocalStorage.set('fzchainstore_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('fzchainstore_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('fzchainstore_token_type', Cookies.get('token_type'))
  LocalStorage.set('fzchainstore_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'fzchainstore_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('fzchainstore_user_info', Cookies.get('user_info'))
  LocalStorage.set('fzchainstore_account_info', Cookies.get('account_info'))
