    ...
    <q-dialog @hide="hideNowPage" v-model="isSidebarActiveLocal" persistent>
      ...
      >
        <q-card-section class="row q-pa-sm items-center">
          <div class="text-subtitle2">{{ dataId > 0 ? '修改' : '新增' }}基础属性</div>
          <q-space />
        ...
        <q-card-actions align="center" class="q-pb-sm q-gutter-sm">
  ...
  methods: {
    init (id) {
      ...
      if (id > 0) {
        ...
        this.$store.dispatch('clothingProperty/getItemData', obj).then((res) => {
          if (res.code == 200) {
    ...
    getItemSort () {
      ...
      this.$store.dispatch('clothingProperty/getItem', obj).then((res) => {
        if (res.code == 200) {
