<template>
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-xs items-center">
        <q-tabs
          @input="getPropertyList"
          ...
          v-model="activeTab"
        >
          <q-tab name="brand" label="品牌" />
          <q-tab name="years" label="年份" />
          <q-tab name="season" label="季度" />
          <q-tab name="band" label="波段" />
          <q-tab name="series" label="系列" />
          <q-tab name="style" label="风格" />
          <q-tab name="fabric" label="面料类别" />
          <q-tab name="safetytype" label="安全类别" />
        </q-tabs>
        <q-space />
        ...
        <q-btn
          color="primary"
          unelevated
          @click="exportCsv(activeTab)"
          label="导出Csv文件"
          :disable="CreditOperateData.length == 0"
        />
      </div>
...
export default {
  ...
  methods: {
    ...
    getPropertyList () {
      ...
      this.$store
        .dispatch('clothingProperty/getList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    changeNumber (p) {
      ...
      if (!regu.test(data.cp_sort)) {
        ...
        this.$store
          .dispatch('clothingProperty/addData', obj)
          .then((res) => {
            if (res.code == 200) {
              this.$q.notify({
                message: '成功',
      ...
      if (this.dataListSelections.length > 0) {
        this.$q
          ...
          .onOk(() => {
            ...
            this.$store
              .dispatch('clothingProperty/delData', obj)
              .then((res) => {
                if (res.code == 200) {
                  this.$q.notify({
                    color: 'green',
