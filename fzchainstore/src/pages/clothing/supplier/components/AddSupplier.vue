      ...
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white q-px-xs q-pb-xs">
          <q-card-section class="row items-center q-pa-sm">
            <div class="text-subtitle2">{{ supplierFrom.id > 0 ? '修改' : '新增' }}供应商信息</div>
            <q-space />
...
export default {
  ...
  methods: {
    init (id) {
      ...
      if (id > 0) {
        this.dataId = id
        this.$store.dispatch('clothingSupplier/getItemData', this.dataId).then((res) => {
          if (res.code == 200) {
            if (res.data) {
    ...
    getBrandData () {
      this.$store.dispatch('clothingProperty/getPropertyList', { type: 'brand' }).then((res) => {
        if (res.code == 200) {
    ...
    submitData () {
      this.$refs.supplierDataRef.validate().then((success) => {
        if (success) {
          ...
          this.$store.dispatch('clothingSupplier/addData', obj).then((res) => {
            if (res.code == 200) {
              this.success = false
