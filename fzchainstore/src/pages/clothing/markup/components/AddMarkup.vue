...
export default {
  ...
  data () {
    return {
      typeOptions: this.$store.state.clothingMarkup.typeOptions,
      typeData: [],
      dataId: 0,
      popupActive: false,
      dataForm: {
  ...
  methods: {
    getType () {
      this.$store.dispatch('clothingMarkup/getTypeAll').then((res) => {
        if (res.code == 200) {
    ...
    init (id) {
      ...
      if (id > 0) {
        this.dataId = id
        this.$store
          .dispatch('clothingMarkup/getItemData', id)
          .then((res) => {
            if (res.code == 200) {
    ...
    clothingMarkupSumbit () {
      this.$refs.dataForm.validate().then((success) => {
        if (success) {
          ...
          this.$store.dispatch('clothingMarkup/addData', obj).then((res) => {
            if (res.code == 200) {
              this.$q.notify({
                message: '成功',
