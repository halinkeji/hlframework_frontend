      ...
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <div class="q-gutter-x-xs row items-center">
          <div class=" col-12 col-md-2">
            <q-input  outlined v-model="searchName" label="标签名称" dense />
          </div>
          ...
          <q-btn
            ...
            @click="delAll()"
          />
          <q-btn color="primary"  unelevated @click="exportCsv" label="导出Csv文件" :disable="clothingTagData.length == 0" />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getGoodsTagList () {
      ...
      this.$store
        .dispatch('clothingTag/getGoodsTagList', obj)
        .then((res) => {
          if (res.code == 200) {
            this.page.pageTotal = res.data ? res.data.page.totalCount : 0
            this.clothingTagData = res.data ? res.data.list : []
          }
        })
        .catch((error) => {
          this.$q.notify({
    ...
    deleteRecordBatch () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('clothingTag/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
