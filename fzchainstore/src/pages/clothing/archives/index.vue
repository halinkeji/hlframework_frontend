<template>
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class=" row q-gutter-sm q-pa-sm q-pb-md bg-white">
        ...
        <q-btn unelevated label="重置" class="q-mr-sm" color="warning" @click="clean" />
        <q-btn unelevated label="查询" color="primary" class="q-mr-sm" :loading="queryLoad" @click="getList">
        ...
        <q-btn color="primary" class="q-mr-sm" unelevated label="新增" :to="{ name: 'addClothingArchives' }" v-if="authorityMeta('add')" />
        <q-btn color="green" class="q-mr-md" unelevated label="Xlxs 文件批量导入" @click="batchImport()" />

        <q-btn color="primary" class="q-mr-sm" unelevated @click="countExport" label="导出Xls文件" :disable="archivesData.length == 0" />
        <q-btn @click="openCloudGoodsDialog" color="primary" unelevated>云商品库</q-btn>
        <q-btn color="primary" unelevated label="竖版展示" :to="{ name: 'clothingArchivesHeng' }" />
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          filter: false,
          valueGetter: (p) => {
            return p.data.type ? p.data.type.gt_name : ''
          },
          rowSpan: this.rowSpan,
          cellClassRules: {
            'cell-span-archives': function (p) {
  ...
  methods: {
    ...
    getList () {
      ...
      this.$store
        .dispatch('clothingArchives/getList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    rowSpan (params) {
      const fields = ['goo_name', 'cad_archives_id', 'bid']
    ...
    countExport () {
      this.$q
        ...
        .onOk((data) => {
          ...
          this.$store
            .dispatch('exports/getArchivesExport', obj)
            .then((res) => {
              if (res.code == 200) {
        ...
        .onOk((data) => {
          ...
          if (dataListSelections.length > 0) {
            const obj = dataListSelections.map((item) => {
              return item.cad_archives_id
            })
            this.printerGoods({ goodsData: obj, model: data })
          }
        })
        .onCancel(() => {})
    },
    printerGoods (goodsData) {
      this.$store.dispatch('cloudprinter/getGoodsLableTemplefz', goodsData).then((res) => {
        if (res.code == 200 && res.data && res.data.length > 0) {
          const goodsLabel = res.data
          _.forEach(goodsLabel, function (value, key) {
            setTimeout(() => {
              this.$refs.localPrinterRef.startPrinter(value)
            }, 50)
          })
        }
      })
