<template>
  <q-page class="fit">
      ...
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <div class="q-gutter-x-xs row ">
          <div class=" col-12 col-md-2 ">
            <q-input  outlined v-model="searchName" label="分组名称" dense />
          </div>

          <q-btn color="primary"  unelevated label="搜索" @click="getGroupList" />
    ...
    <add-group @getMethods="getGroupList" ref="addRef" v-if="adddGroupShow"></add-group>
  </q-page>
</template>
...
export default {
  ...
  methods: {
    ...
    getGroupList () {
      ...
      this.$store
        .dispatch('clothingGroup/getGroupList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    delAll () {
      this.$q
        .dialog({
          ...
          color: 'primary',
          title: '是否确认',
          message: `确定删除 ${this.dataListSelections.length} 条商品分组吗`,
          cancel: true,
          persistent: true
        })
        .onOk(() => {
    ...
    deleteRecordBatch () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('clothingGroup/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
