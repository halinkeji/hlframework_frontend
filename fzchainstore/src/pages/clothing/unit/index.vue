<template>
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-sm">
        <div class="col-12 col-md-2 ">
          <q-input outlined v-model="searchName" label="计量单位名称" dense />
        </div>

        <q-btn color="primary" unelevated label="搜索" @click="getClothingUnitList" />
        ...
        <q-btn
          ...
          @click="delAll()"
        />
        <q-btn color="primary" unelevated @click="exportCsv" label="导出Csv文件" :disable="clothingUnitData.length == 0" />
      </div>
    ...
    <add-unit @getMethods="getClothingUnitList" ref="addRef" v-if="adddUnitShow"></add-unit>
  </q-page>
</template>
...
export default {
  ...
  methods: {
    ...
    getClothingUnitList () {
      ...
      this.$store
        .dispatch('clothingUnit/getClothingUnitList', obj)
        .then((res) => {
          if (res.code == 200) {
            this.page.pageTotal = res.data ? res.data.page.totalCount : 0
            this.clothingUnitData = res.data ? res.data.list : []
          }
        })
        .catch((error) => {
          this.$q.notify({
    ...
    delAll () {
      this.$q
        .dialog({
          ...
          color: 'primary',
          title: '是否确认',
          message: `确定删除 ${this.dataListSelections.length} 条计量单位吗`,
          cancel: true,
          persistent: true
        })
        .onOk(() => {
    ...
    deleteRecordBatch () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('clothingUnit/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              icon: 'ion-checkmark-circle-outline',
              timeout: 500,
