      ...
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <div class="q-gutter-x-xs row ">
          <div class="col-12 col-md-2 ">
            <q-input  outlined v-model="searchName" label="规格组名称" dense />
          </div>

          <q-btn color="primary"  unelevated label="搜索" @click="getFormatList" />
          <q-btn color="warning"  unelevated label="重置" @click="clean" />
          <q-btn color="primary"  unelevated label="新增" v-if="authorityMeta('add')" :to="{ name: 'addClothingFormat' }" />
          <q-btn
            color="red"
            unelevated
...
export default {
  ...
  methods: {
    ...
    getFormatList () {
      ...
      this.$store
        .dispatch('clothingFormat/getList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    delAll () {
      this.$q
        .dialog({
          ...
          color: 'primary',
          title: '是否确认',
          message: `确定删除 ${this.dataListSelections.length} 条商品规格组吗`,
          cancel: true,
          persistent: true
        })
        .onOk(() => {
