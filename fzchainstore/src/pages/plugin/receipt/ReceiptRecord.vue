  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md">
      ...
      <div class="row q-gutter-xs q-my-sm">
        <q-tabs v-model="debtTab" dense class="text-grey" active-color="primary" indicator-color="primary" align="left" narrow-indicator>
          <q-tab name="not" label="未开票" />
          <q-tab name="opend" label="已开票" />
          <q-tab name="receiptRecord" label="开票记录" />
        </q-tabs>

        <q-btn @click="showPopup()" :disabled="dataListSelections.length > 0 ? false : true" color="positive" class="q-mt-sm col-12 col-md-1 q-mr-sm" unelevated label="开票" />
        <q-btn @click="refreshButton()" color="warning" :disabled="memberId > 0 ? false : true" class="q-mt-sm col-12 col-md-1 q-mr-sm" unelevated label="重置" />
        <q-input outlined dense v-model="total_amount" label="未开票总计金额" disable class="col-12 col-md-2"> </q-input>
      </div>
    </div>
    <div class="col-shrink full-height bg-grey-2" v-if="debtTab == 'not'">
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    ...
    <div class="col-shrink full-height bg-grey-2" v-if="debtTab == 'opend'">
      <open-record :memberId="memberId"></open-record>
    </div>
    <div class="col-shrink full-height bg-grey-2" v-if="debtTab == 'receiptRecord'">
      <rec-record :memberId="memberId"></rec-record>
    </div>
    <open-receipt ref="addRef" v-if="addPoppupStatus" @dataList="refreshButton"></open-receipt>
  </div>
</template>
<script>
import OpenReceipt from './components/OpenReceipt'
import OpenRecord from './components/OpenRecord'
import RecRecord from './components/RecRecord'
import AgTypeBg from './components/AgTypeBg'
import AgOperator from './components/AgOperator'
export default {
  name: 'ReceiptRecord',
  components: { OpenReceipt, OpenRecord, RecRecord, AgTypeBg, AgOperator },
  data () {
    return {
      debtTab: 'not',
      page: {
      ...
      columnDefs: [
        ...
        {
          headerName: this.$q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_balanceUnit + '支付',
        ...
        {
          headerName: this.$q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_integralUnit + '支付',
  ...
  methods: {
    ...
    rowSelectedAg (p) {
      ...
      if (this.dataListSelections.length > 0) {
        ...
        this.dataListSelections.forEach((item) => {
          amount += this.formatAmount(item.sc_realMoneyCount)
        })
    ...
    showPopup () {
      ...
      const mainInvoice = this.dataListSelections.map((item) => {
        return item.id
      })
      this.$nextTick(() => {
        this.$refs.addRef.init(this.total_amount, mainInvoice, this.memberId)
      })
    },
    // 重置按钮方法
    clearButton () {
    ...
    getDatalist () {
      ...
      this.$store
        .dispatch('receipt/getInvoiceList', obj)
        .then((res) => {
          if (res.code == 200) {
            if (res.data.list) {
