    ...
    >
      ...
      <q-form ref="mezsDataRef">
        ...
        <div class="row q-mt-md">
          <div class="col-6 q-pr-md">
            <q-input
              outlined
              dense
              v-model="mezsData.pmp_min_limit_amount"
              label="金额满最小值"
              :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的金额满最小值']"
            />
          </div>
          <div class="col-6 q-pr-sm">
            <q-input
              outlined
              dense
              v-model="mezsData.pmp_max_limit_amount"
              label="金额满最大值"
              :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的金额满最大值']"
            />
          </div>
        </div>
        <div class="row q-mt-xs">
          <div class="col-6 q-pr-md">
            <q-input
              outlined
              dense
              v-model="mezsData.pmp_give_balance"
              label="赠送储值"
              :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的赠送储值']"
            />
          </div>
          <div class="col-6 q-pr-sm">
            <q-input
              outlined
              dense
              v-model="mezsData.pmp_give_integral"
              label="赠送积分"
              :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的赠送积分']"
            />
          </div>
        </div>
        <div class="q-mt-xs">
          <q-chip outline square color="red" text-color="white">
            赠送储值 {{ mezsData.pmp_give_balance }} 元，赠送 {{ mezsData.pmp_give_integral }} 积分
          </q-chip>
        </div>
        ...
        <div align="right" class="q-mt-md q-mr-sm">
          <q-btn unelevated label="确定" :disable="addButtonDisabled" color="primary" @click="setData" v-if="authorityMeta('save')" />
