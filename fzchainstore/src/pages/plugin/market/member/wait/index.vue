  ...
  methods: {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            ...
            if (parseInt(this.configData.pmc_form_id) > 0) {
              this.getColumn(this.configData.pmc_form_id)
            } else {
              this.restColumnData()
            }
          }
    ...
    restColumnData () {
      ...
      const checkStatus = {
        ...
        width: this.nowpx(0.1) + 'px',
        valueGetter: (p) => {
          if (parseInt(p.data.pmm_weights) == -1) {
            return '待审核'
          } else if (parseInt(p.data.pmm_weights) == -2) {
            return '审核未通过'
          } else if (parseInt(p.data.pmm_weights) == -3) {
            return '已取消'
          } else if (parseInt(p.data.pmm_weights) >= 0) {
            return '审核通过'
          } else {
            return false
          }
      ...
      const addTime = {
        headerName: '申请时间',
        valueGetter: (p) => {
          return this.$q_date.formatDate(p.data.pmm_apply_time * 1000, 'YYYY-MM-DD HH:mm:ss')
        },
        width: this.nowpx(0.15) + 'px'
      }
    ...
    getList () {
      ...
      const params = {
        formId: this.configData.pmc_form_id,
