      ...
      if (data && data.length > 0) {
        ...
        for (let i = 0; i < data.length; i++) {
          if (list.indexOf(parseInt(data[i].id)) == -1) {
            ...
            person.ppg_goods_name = data[i].ca_name;
            person.ppg_code = data[i].ca_article_code;
            person.ppg_price = data[i].ca_price;
            specNewsDataBak.push(person);
          } else {
            this.$q.notify({
              color: 'red',
    ...
    getSelectedGoodsData(data, status = false) {
      ...
      if (data && data.length > 0) {
        ...
        for (let i = 0; i < data.length; i++) {
          if (list.indexOf(parseInt(data[i].id)) == -1) {
            const person = {};
            person.ppg_goods_id = data[i].cad_archives_id;
            person.ppg_goods_name = data[i].ca_name;
            person.ppg_sep_id = data[i].id;
            person.ppg_size_name = data[i].cad_size_name;
            person.ppg_color_name = data[i].cad_color_name;
            person.ppg_code = data[i].cad_skc_code;
            person.ppg_price = data[i].cad_price;
            specNewsDataBak.push(person);
          } else {
            this.$q.notify({
              color: 'red',
