  ...
  <q-dialog @hide="hideNowPage" v-model="addStoreSidebar" persistent>
    ...
    >
      <q-card-section class="row items-center q-pa-none ">
           <div class="text-h6">{{ id > 0 ? '修改' : '新增' }}签到规则</div>
        <q-space />
      ...
      <q-form ref="InfoDataRef">
        ...
        <div class="row q-my-xs">
          <div class="col-6">
            <q-input
              ...
              label="赠送积分"
              lazy-rules
              :rules="[(val) => /^[0-9]+(.[0-9]{1,2})?$/.test(val) || '请输入正确的积分']"
            />
          </div>
          <div class="col-6">
            <q-input
              ...
              dense
              class="q-px-sm"
              v-model="rulesData.cr_give_money"
              label="赠送金额"
              lazy-rules
              ...
              <q-item v-for="(item, key) in selectedCouponData" :key="key">
                ...
                <q-item-section>
                  ...
                  <q-item-label caption lines="2">
                    ...
                    <span v-else>
                      <span v-if="item.cou_startDay == 0"> 领取后当天生效,有效期{{ item.cou_endDay }} 天 </span>
                      <span v-else>
                        领取{{ item.cou_startDay == 0 ? '后当天生效' : item.cou_startDay + '天后生效' }} ,有效期{{ item.cou_endDay }}
                        天
                      </span>
                    </span>
                  </q-item-label>
        ...
        <div class="row q-mt-md">
          <q-space />
          <q-btn unelevated class="q-ml-sm q-mb-md" label="提交"  v-if="authorityMeta('save')" @click="saveData()" :disable="disableButton" color="primary" />
          <q-btn unelevated class="q-ml-sm q-mb-md" label="关闭" color="grey" @click="hideNowPage()" />
        </div>
      </q-form>
    </q-card>
  </q-dialog>
