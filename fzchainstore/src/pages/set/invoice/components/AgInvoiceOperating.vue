<template>
  <div v-if="!params.data.show_status">
    <div>
      ...
      <span
        v-if="
          (parseInt(params.data.sc_pay_status) == 1 ||
            parseInt(params.data.sc_pay_status) == 4) &&
          authorityMeta('consumeRemove')
        "
      >
        <q-btn
          class="q-ma-xs"
          ...
          round
          size="sm"
          v-if="params.data.sc_consumeType == 'consume'"
          @click="returnBut()"
        >
          <q-tooltip anchor="top middle" self="bottom middle" :offset="[10, 10]"
            >整单退货</q-tooltip
...
export default {
  ...
  methods: {
    ...
    exchangeBut () {
      this.params.context.exchangeBut(this.params.data.id)
    },
    revokeOrderBut () {
