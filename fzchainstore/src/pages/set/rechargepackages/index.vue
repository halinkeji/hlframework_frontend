<template>
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs "  :class="{ 'q-pb-lg':parseInt(activeName) == 0 ? true : false }">
        <q-tabs
          v-model="activeName"
        ...
        <div class="row q-px-sm q-gutter-x-sm" v-if="parseInt(activeName) == 0">
          <q-input class="col-12 col-md-2 q-mt-sm" clearable label="请输入套餐名称" outlined v-model="packageName" dense />
          <q-select
          ...
          <q-btn @click="getList()" color="primary" class="col-12 col-md-1 q-mt-sm" unelevated v-if="authorityMeta('search')" label="搜索" />
          <q-btn @click="clearButton" color="warning" class="col-12 col-md-1 q-mt-sm" unelevated v-if="authorityMeta('reset')" label="重置" />
          <q-btn
            :to="{ name: 'addRechargePackages' }"
            color="primary"
...
export default {
  ...
  components: { AgSwitch, Operating, ConsumptionReceipt },
  data () {
    return {
      activeName: '0',
      ...
      columnDefs: [
        ...
        {
          headerName: '赠送' + this.$q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_balanceUnit,
          field: 'rp_give_money',
          width: this.nowpx(0.1) + 'px'
        },
        {
        ...
        },
        {
          ...
          field: 'rp_start_time',
          valueGetter: (p) => {
            return this.$q_date.formatDate(p.data.rp_start_time * 1000, 'YYYY-MM-DD HH:mm')
          },
        ...
        },
        {
          ...
          field: 'rp_end_time',
          valueGetter: (p) => {
            return this.$q_date.formatDate(p.data.rp_end_time * 1000, 'YYYY-MM-DD HH:mm')
          },
          width: this.nowpx(0.12) + 'px'
        },
        {
