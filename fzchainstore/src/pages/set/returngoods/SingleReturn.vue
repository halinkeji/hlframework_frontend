      ...
      >
        <q-card-section class="row items-center q-pb-none">
          ...
          <q-btn
            ...
            dense
            v-close-popup
            @click="singleReturnDialog = false"
          />
        </q-card-section>
  ...
  methods: {
    init (data, member_id) {
      ...
      if (this.consumeData.cid_goodsOrService == 3) {
        ...
        this.totalNember =
          this.formatAmount(
            this.consumeData.cid_consumeNumber /
              this.consumeData.cid_package_number
          ) -
          this.formatAmount(
            this.consumeData.alreadyReturnNumber /
              this.consumeData.cid_package_number
          )
        this.rgd_return_quantity = this.totalNember
        this.rgd_return_amount = this.formatAmount(
          this.totalNember * this.consumeData.cid_sellingPrice
        )
      } else {
        this.goo_name = this.consumeData.cid_goo_name
        this.totalNember =
          this.formatAmount(this.consumeData.cid_consumeNumber) -
          this.formatAmount(this.consumeData.alreadyReturnNumber)
    ...
    changeNumber (val) {
      ...
      this.rgd_return_amount = this.formatAmount(
        this.consumeData.cid_sellingPrice * this.rgd_return_quantity
      )
    },
    setReturnData () {
