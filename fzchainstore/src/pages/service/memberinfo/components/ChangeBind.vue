<template>
  <div>
    <q-dialog v-model="changeBind">
      ...
      >
        <q-toolbar>
          <q-toolbar-title>店铺改绑</q-toolbar-title>
          <q-btn flat round dense icon="close" v-close-popup @click="closeButton" />
        </q-toolbar>
        <q-card-section>
          <div class="row">
            <div class="col">
              <q-select filled label="绑定店铺" v-model="memberInformation.store_id" :options="libInStoreList" option-value="id" option-label="name" emit-value map-options />
            </div>
          </div>
