    ...
    <q-dialog v-model="weighComponentsDialog" persistent>
      ...
      >
        <q-form ref="consumeFormRef">
          <q-toolbar>
            <q-toolbar-title class="text-center"> 请将{{ otherValue.chud_goo_name }}放到秤上</q-toolbar-title>
          </q-toolbar>
          <q-card-section>
            <div class="text-h4">{{ otherValue.chud_goo_price }}</div>
            <div class="text-caption text-grey">单价（{{ $q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_moneyUnit }}/千克）</div>
          </q-card-section>
...
export default {
  ...
  methods: {
    ...
    init (val, type) {
      ...
      this.otherValue = val
      this.otherType = type
      if (val && val.ccb_consumeNumber) {
        this.old_consumeNumber = this.otherValue.ccb_consumeNumber
      }
      // this.$nextTick(() => {
      setTimeout(() => {
        if (that.$refs.consumeNumberRef) {
          that.$refs.consumeNumberRef.focus()
          const systemLocal = LocalStorage.getItem('fzchainstore_plugins_local')
    ...
    confirmWeighResult () {
      this.$refs.consumeFormRef.validate().then((success) => {
        if (success) {
          ...
          this.otherValue.ccb_consumeNumber = this.ccb_consumeNumber
          this.$emit('confirmWeighResult', 3, this.ccb_consumeNumber)
        }
      })
    },
    ...
    computeAmountSubtotal () {
      if (this.otherValue) {
        this.amountSubtotal = this.formatAmount(this.formatAmount(this.ccb_consumeNumber) * this.formatAmount(this.otherValue.chud_goo_price))
      }
    },
    // 格式化保留小数点后两位数据
    formatAmount (value) {
      ...
      try {
        ...
        this.serial_port.on('data', (data) => {
          ...
          switch (2) {
            case '1':
              // 大华计算格式
              that.ccb_consumeNumber = this.$q_decimal(num / 1000000000).toFixed(2)
              break
            case '2':
              // 顶尖计算格式
              that.ccb_consumeNumber = this.$q_decimal(num / 1000).toFixed(2)
              break
            default:
              // 其他通用计算格式
              that.ccb_consumeNumber = this.$q_decimal(num / 1000).toFixed(2)
        ...
        console.log('this.ccb_consumeNumber', that.ccb_consumeNumber)
      ...
      if (systemLocal && systemLocal.eWeighing && parseInt(systemLocal.eWeighing.psc_status) == 1) {
        this.serial_port.close(function (error) {
          if (error) {
            ...
            console.log('关闭串口成功')
            console.log('最后称重结果：', that.ccb_consumeNumber)
          }
        })
