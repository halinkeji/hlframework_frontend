                ...
                >
                  <q-card-section class="q-pa-xs">
                    ...
                    <q-item-label :class="{ 'text-white': parseInt(currentGoodsSpecInfo.id) == parseInt(props.row.id) }" class="text-dark row">
                      {{ $q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ props.row.sl_sellingPrice }}
  ...
  methods: {
    confirmWeighResult () {
      this.$refs.consumeFormRef.validate().then((success) => {
        if (success) {
          ...
          this.currentGoodsSpecInfo.ccb_consumeNumber = this.ccb_consumeNumber
    ...
    computeAmountSubtotal () {
      if (this.currentGoodsSpecInfo) {
        this.amountSubtotal = this.formatAmount(
          this.formatAmount(this.ccb_consumeNumber) * this.formatAmount(this.currentGoodsSpecInfo.sl_sellingPrice)
        )
      }
    },
    ...
    getWeighing () {
      ...
      try {
        ...
        console.log('this.ccb_consumeNumber', that.formatAmount(that.ccb_consumeNumber))
