<template>
  <div class="flex flex-wrap">
    ...
    <q-dialog v-model="specificationPopup">
      <q-card
        :style="{
          width: parseInt(this.$q.screen.width * (['xs', 'sm', 'md'].indexOf($q.screen.name) > -1 ? 0.8 : 0.7)) + 'px',
          'max-width': parseInt(this.$q.screen.width * (['xs', 'sm', 'md'].indexOf($q.screen.name) > -1 ? 0.8 : 0.7)) + 'px',
        }"
      >
        <q-toolbar>
          <q-toolbar-title>
            {{ $q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_integralUnit }}操作
          </q-toolbar-title>
        ...
        <q-card-section>
          <div class="col-shrink bg-white full-height bg-grey-2 q-pt-xs">
            <div class="row fit bg-white justify-around">
              ...
              <q-form ref="submitFormRef" class="col-12 col-md-12 q-px-xs q-pt-md">
                <q-card flat class="q-pa-md">
                  ...
                  >
                    <template v-slot:prepend>
                      <div class="q-pl-none text-subtitle1">增加/扣除</div>
                    </template>
                  </q-input>
                  ...
                  <q-card-section class="q-py-sm" v-if="fixedMoneyList.length > 0">
                    <div class="row">
                      <div class="col-6 col-md-2 q-mt-md q-px-xs" v-for="item in fixedMoneyList">
                        <q-btn
                          unelevated
                          rounded
                          color="green"
                          ...
                          :disable="memberId == 0"
                          @click="pointInput = item.fm_parameter"
                        >
                          {{ item.fm_parameter
          ...
          <div class="col-shrink bg-white q-pb-md q-gutter-md text-center q-mt-xs">
            <q-btn
              ...
              :disable="!(pointInput && memberId) && !buttonDisabled"
              @click="pointChangeBut(1)"
            />
            <q-btn
              unelevated
              color="blue-6"
