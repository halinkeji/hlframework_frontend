  ...
  <div class="fit row no-wrap justify-between items-stretch content-stretch">
    ...
    <div class="col-shrink col-xs-10 col-sm-11 col-md-11 col-lg-11 col-xl-11">
      <left-layout
        ...
        ref="shoppingCarRef"
        :ccb_hangup_id="pendingOrderInfo && pendingOrderInfo.id ? pendingOrderInfo.id : 0"
      />
    </div>
    <!-- center -->
          ...
          <div class="col-shrink q-px-xs">
            ...
            <q-btn v-if="authorityMeta('hangup')" color="white" class="full-width q-my-xs q-py-sm" @click="openPendingDialog" padding="none">
              <span class="text-subtitle1 q-my-xs text-black">
                挂单(h)
                <q-badge color="red" rounded floating>{{ hangUpTotal }}</q-badge>
              </span>
            </q-btn>
            <q-btn v-if="authorityMeta('hangup')" color="white" class="full-width q-my-xs q-py-sm" @click="openPending" padding="none">
              <span class="text-subtitle1 q-my-xs text-black">
                取单
                <!-- <q-badge color="red" rounded floating>{{ hangUpTotal }}</q-badge> -->
              </span>
            ...
            <q-btn color="positive" class="full-width q-my-xs q-py-sm" @click="showGoodsPage">
              <span class="text-white text-subtitle1">商品库</span>
            </q-btn>
            <q-btn color="secondary" class="full-width q-my-xs q-py-sm" @click="showExchange">
              <span class="text-white text-subtitle1">退换货</span>
            </q-btn>
            <q-btn
              color="primary"
              class="full-width q-my-xs q-py-sm"
...
export default {
  ...
  mounted () {
    ...
    keyboardJS.bind('numadd', (e) => {
      if (this.shoppingCartsInfo && this.shoppingCartsInfo.id && this.shoppingCartsInfo.goods) {
  ...
  methods: {
    ...
    showGoodsPage () {
      this.$nextTick(() => {
        this.$refs.shoppingCarRef.selectBut()
      })
    },
    ...
    openPending () {
      this.pendingOrderStatus = true
      this.$nextTick(() => {
        this.$refs.pendingOrderRef.getHangUpList()
      })
    },
    // 重置会员卡
    resetMemberInfo () {
