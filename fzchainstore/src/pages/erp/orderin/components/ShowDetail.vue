      ...
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white q-px-xs q-pb-md q-gutter-x-xs ">
          <div class="row justify-center">
            <h6 class="no-margin">入库单据信息</h6>
            <q-space />
            <q-btn unelevated flat round dense icon="close" v-close-popup />
          </div>
          <div class="row">
            <div class="col-12 col-md-2 q-pa-xs">
              <q-input dense disable v-model="erpInFormData.cep_contract_number" label="入库单据号" />
            </div>
            <!-- <div class="col-12 col-md-1 q-pa-xs">
              <q-input disable dense v-model="erpInFormData.cep_buyer_name" label="入库员" />
            </div> -->

            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="erpInFormData.cep_total_purchase_amount" label="入库实付款" />
            </div>
            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="erpInFormData.cep_total_purchase_num" label="发货总数量" />
            </div>
            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="erpInFormData.cep_note" label="入库备注" />
            </div>
          </div>
        </div>
          ...
          <q-form ref="erpInRef">
            <div class="row ">
              <div class="col-12 q-px-xs">
                <q-input type="textarea" outlined v-model="erpInFormData.cep_note" label="备注" dense :rows="2" />
              </div>
            </div>
            <div class="row q-gutter-sm q-mt-xs q-pb-lg">
              <div class="col-auto">
                账单小计:<span class="text-primary  text-h6"> {{ erpInFormData.cep_total_purchase_amount }}</span>
              </div>
              <div class="col-auto">
                总数量:<span class="text-primary text-h6">{{ erpInFormData.cep_total_shipments_quantity }}</span>
              </div>
              <q-space />
              <div class="col-auto text-h5" v-if="erpInFormData.cep_status > 0">
                单据状态:
                ...
                <span v-if="erpInFormData.cep_status == 6" class="">确认入库</span>
              </div>
            </div>
          </q-form>
        </div>
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '折后单价',
          field: 'eod_discount_after_amount',
          filter: false,
          valueGetter: (p) => {
            return p.data.eod_archives_id > 0 ? (p.data.eod_discount_after_amount > 0 ? p.data.eod_discount_after_amount : p.data.eod_price) : ''
          },
          width: this.nowpx(0.06) + 'px'
        },
        {
          headerName: '采购金额',
          field: 'eod_cash_amount',
          filter: false,
          width: this.nowpx(0.06) + 'px'
        },
        {
          headerName: '到货数量',
          field: 'eod_shipments_quantity',
          filter: false,
          width: this.nowpx(0.06) + 'px'
        },
        {
          headerName: '到货金额',
          field: 'eod_shipments_amount',
          filter: false,
          width: this.nowpx(0.06) + 'px'
        },
        {
          headerName: '残次数量',
          field: 'eod_defect_quantity',
          filter: false,
          width: this.nowpx(0.06) + 'px'
        },
        {
          headerName: '残次金额',
          field: 'eod_defect_amount',
          filter: false,
          width: this.nowpx(0.06) + 'px'
        },
        {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            ...
            this.goodsInData = res.data.orderin.length > 0 ? res.data.orderin : []
