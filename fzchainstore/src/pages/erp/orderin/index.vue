  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-py-md">
      <div class="q-gutter-xs row">
        ...
        <q-btn color="primary" unelevated label="新增" v-if="authorityMeta('orderAdd')" @click="chooseInvoice(6)" />
...
export default {
  ...
  methods: {
    ...
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        const obj = {
          out_order_id: val[0].id
        }
        this.$store
          .dispatch('erpOrderin/createOrderInData', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addErpOrderin', query: { id: res.data.id } })
              })
            } else {
              this.$q.notify({
                message: '生成失败',
    ...
    getList () {
      ...
      this.$store
        .dispatch('erpOrderin/getDataList', obj)
        .then((res) => {
          if (res.code == 200) {
