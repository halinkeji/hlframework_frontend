<template>
  <div v-if="!params.data.show_status">
    <q-btn
      ...
      :to="{ name: 'addStoreReturnin', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.cep_status == 3 && authorityMeta('storeClose')"
      :to="{ name: 'addStoreReturnin', query: { id: params.data.id } }"
    >
  ...
  methods: {
    ...
    saveDataOperate (text, cepStatus) {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            store_return_in_depot_order_id: this.params.data.id,
            cepStatus
          }
          this.$store
