...
export default {
  ...
  methods: {
    ...
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        const obj = {
          purchase_return_order_id: val[0].id
        }
        this.$store
          .dispatch('erpPurchaseOut/createOutDepotOrder', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addPurchaseReturnOut', query: { id: res.data.id } })
              })
            } else {
              this.$q.notify({
                message: '生成失败',
    ...
    getList () {
      ...
      this.$store
        .dispatch('erpPurchaseOut/getDataList', obj)
        .then((res) => {
          if (res.code == 200) {
