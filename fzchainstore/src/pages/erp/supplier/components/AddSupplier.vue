  ...
  <div>
    <q-dialog @hide="hideNowPage" v-model="isSidebarActiveLocal" persistent>
      ...
      >
        ...
        <q-form ref="supplierDataRef" class="q-gutter-md">
          <q-card-section class="q-pt-lg">
            <div class="row">
              <div class="col-md-6 col-12 q-pr-sm q-mt-xs ">
                <q-input
                  label="名称"
                  outlined
                  v-model="supplierFrom.gs_supplierName"
                  dense
                  lazy-rules
                  :rules="[val => !!val || '名称是必填的']"
                />
              </div>
              <div class="col-md-6 col-12 q-pr-sm q-mt-xs ">
                <q-select
            ...
            <div class="row">
              ...
              <div class="col-md-6 col-12 q-pr-sm q-mt-xs q-mt-sm">
                <q-input
                  label="联系地址"
                  outlined
                  v-model="supplierFrom.gs_supplierAddress"
                  dense
                  :error="false"
                />
              </div>
            ...
            <q-input
              ...
              label="备注"
              outlined
              v-model="supplierFrom.gs_supplierNote"
              dense
              type="textarea"
            />
          </q-card-section>
        ...
        this.$store.dispatch('supplier/getItemData', this.dataId).then(res => {
          if (res.code == 200) {
            if (res.data) {
              ...
              this.supplierFrom.gs_supplierTypeId = parseInt(
                this.supplierFrom.gs_supplierTypeId
              )
    ...
    submitData () {
      this.$refs.supplierDataRef.validate().then(success => {
        if (success) {
          ...
          const address =
            this.dataFrom.province +
            '-' +
            this.dataFrom.city +
            '-' +
            this.dataFrom.district +
            '-' +
            this.supplierFrom.gs_supplierAddress
          if (!this.success) {
