  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-sm q-pb-md">
      <div class="q-gutter-x-xs row  ">
        <q-input class="col-12 col-md-2 q-mt-sm" label="分类名称" outlined v-model="searchProvider" dense />
        <q-btn color="primary" class=" q-mt-sm" unelevated v-if="authorityMeta('searchType')" label="搜索" @click="getGoodsSupplierTypeList" />
        <q-btn color="warning" class=" q-mt-sm" unelevated label="重置" v-if="authorityMeta('resetType')" @click="clean" />
  ...
  methods: {
    ...
    delAll () {
      ...
      this.$q
        .dialog({
          title: '是否确认',
          message: `确认删除${dataListSelections.length}条供应商分类`,
          cancel: true
        })
        .onOk(() => {
