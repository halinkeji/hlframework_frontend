      ...
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white q-px-xs q-pb-md q-gutter-x-xs ">
          <div class="row justify-center">
            <h6 class="no-margin">出库单据信息</h6>
            <q-space />
            <q-btn unelevated flat round dense icon="close" v-close-popup />
          </div>
          <div class="row">
            <div class="col-auto" v-if="invoiceData.cep_if_pack == '1'">
              <q-btn
          ...
          <div class="row">
            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="invoiceData.cep_contract_number" label="出库单据号" />
            </div>
            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="invoiceData.cep_depot_name" label="出库仓库" />
            </div>
            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="invoiceData.cep_total_purchase_num" label="出库数量" />
            </div>
            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="invoiceData.cep_total_purchase_amount" label="出库实付款" />
            </div>

            <div class="col-12 col-md-2 q-pa-xs">
              <q-input disable dense v-model="invoiceData.cep_note" label="出库备注" />
            </div>
          </div>
        </div>
          ...
          <q-form ref="erpInRef">
            <div class="row ">
              <div class="col-12 q-px-xs">
                <q-input type="textarea" outlined v-model="invoiceData.cep_note" label="备注" dense :rows="2" />
              </div>
            </div>
            <div class="row q-gutter-sm q-mt-xs q-pb-lg">
              ...
              <div class="col-auto text-h5" v-if="invoiceData.cep_status > 0">
                ...
                <span v-if="invoiceData.cep_status == 6" class="">已分配</span>
                <span v-if="invoiceData.cep_status == 7" class="">已配货</span>
              </div>
            </div>
          </q-form>
        </div>
...
export default {
  ...
  methods: {
    getItem (val) {
      ...
      this.$store
        .dispatch('erpDirectOut/getGoodsBuf', obj)
        .then((res) => {
          if (res.code == 200) {
            this.invoiceData = res.data
            this.goodsInData = res.data.out.length > 0 ? res.data.out : []
