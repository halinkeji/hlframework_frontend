<template>
  <div v-if="!params.data.show_status">
    <q-btn
      ...
      :to="{ name: 'addErpDirectAllocation', query: { id: params.data.id } }"
    >
    ...
    <q-btn class=" q-ma-xs" v-if="authorityMeta('log')" unelevated color="info" icon="assignment" round size="sm" @click="showLog">
    ...
    saveDataOperate (text, cepStatus) {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            cepStatus,
            direct_allocation_order_id: this.params.data.id
          }
          this.$store
