<template>
  <div v-if="!params.data.show_status">
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 1 && authorityMeta('orderConfirm')"
      :to="{ name: 'addErpDirectIn', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 2 && authorityMeta('orderSave')"
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 2 && authorityMeta('orderAudit')"
      :to="{ name: 'addErpDirectIn', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 5 && authorityMeta('orderHangup')"
      :to="{ name: 'addErpDirectIn', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.cep_status < 2 && authorityMeta('orderClose')"
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.cep_status == 3 && authorityMeta('orderStorage')"
      :to="{ name: 'addErpDirectIn', query: { id: params.data.id } }"
    >
      <q-tooltip anchor="top middle" self="bottom middle" :offset="[10, 10]">入库</q-tooltip>
    </q-btn>

    <q-btn class="q-ma-xs" color="info" v-if="authorityMeta('orderLog')" unelevated icon="assignment" round size="sm" @click="showLog">
  ...
  methods: {
    ...
    saveDataOperate (text, cepStatus) {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            direct_in_depot_order_id: this.params.data.id,
