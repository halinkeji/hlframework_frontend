<template>
  ...
  <div>
    <q-btn
      ...
      :to="{ name: 'addErpTransfer', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.cep_status == 1 && authorityMeta('close')"
    ...
    <q-btn class="q-ma-xs" color="info" unelevated v-if="authorityMeta('log')" icon="assignment" round size="sm" @click="showLog">
  ...
  methods: {
    ...
    saveDataOperate (text, cepStatus) {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            transfer_order_id: this.params.data.id,
            cepStatus
          }
          this.$store
