<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <!-- <div class="col-shrink bg-white q-px-xs q-pb-md">
      <q-btn color="primary" class=" q-mt-sm q-mb-md " unelevated @click="exportCsv" label="导出Csv文件" :disable="orderData.length == 0" />
    </div> -->
    ...
    <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-sm">
      ...
      <div class="col-2">
        <q-input outlined @clear="getOrderData" clearable v-model="orderNumber" label="单据号" dense />
      </div>

      <q-btn color="primary" unelevated @click="exportCsv" label="导出Csv文件" :disable="orderData.length == 0" />
    </div>
...
<script>
import AgInput from 'pages/erp/ordergoods/components/AgInput'
export default {
  name: 'OrderView',
  ...
  methods: {
    ...
    getOrderData () {
      ...
      this.$store.dispatch('erpNumber/getOrder', obj).then((res) => {
        if (res.code == 200) {
          this.page.totalCount = res.data ? res.data.page.totalCount : 0
          this.orderData = res.data ? res.data.list : []
          if (this.orderData && this.orderData.length > 0) {
            this.setTopTabel(this.orderData)
          }
        }
      })
    },
