<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-sm">
      ...
      <div class="col-2">
        <q-input outlined @clear="gteOutData" clearable v-model="orderNumber" label="单据号" dense />
      </div>
      <q-btn color="primary"  unelevated @click="exportCsv" label="导出Csv文件" :disable="outData.length == 0" />
    </div>
...
export default {
  ...
  methods: {
    ...
    gteOutData () {
      ...
      this.$store.dispatch('erpNumber/getItems', obj).then((res) => {
        if (res.code == 200) {
          this.page.totalCount = res.data ? res.data.page.totalCount : 0
          this.outData = res.data ? res.data.list : []
          if (res.data) {
            this.setTopTabel(this.outData)
          }
        }
      })
    },
