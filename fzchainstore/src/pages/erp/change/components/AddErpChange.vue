    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <q-form ref="erpInRef" class="q-gutter-xs row">
          ...
          <q-btn color="primary" unelevated v-if="authorityMeta('select') && changeFormData.cep_status < 2" label="选择商品" @click="selectBut" />
          ...
          <q-btn
            color="green"
            unelevated
            v-if="authorityMeta('code') && changeFormData.cep_status < 2"
            label="条码添加"
...
export default {
  ...
  methods: {
    ...
    countMethor (p) {
      ...
      this.$store
        .dispatch('erpchange/editButtfer', data)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
