  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-py-md">
      <div class="q-gutter-xs row">
        ...
        <q-btn color="primary" unelevated label="生成入库单" @click="chooseInvoice(1)" v-if="authorityMeta('inAdd')"  />
...
export default {
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '原始单据编号',
          field: 'purchaseOrderData.cep_contract_number',
  ...
  methods: {
    ...
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        const obj = {
          purchase_order_id: val[0].id
        }
        this.$store
          .dispatch('erpin/createPurchaseInDepotOrder', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addErpIn', query: { id: res.data.id } })
              })
            } else {
              this.$q.notify({
                message: '生成失败',
    ...
    countExport () {
      ...
      this.$store
        .dispatch('exports/getInExport', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    showLogData (val) {
      ...
      this.$nextTick(() => {
        this.$refs.showDataRef.init(val, 3)
      })
    },
    authorityMeta (key) {
      if (this.$route.meta) {
