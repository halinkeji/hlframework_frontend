      ...
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        ...
        <div class="col-shrink q-pa-sm full-height bg-white">
          <q-form ref="orderFormRef">
            <q-input outlined v-model="wo_pickup_code" @keyup.enter="getItem"  label="自堤单号">
...
export default {
  ...
  methods: {
    ...
    getItem () {
      this.$store
        .dispatch('wxMallOrder/checkOrderDetail', {
          wo_pickup_code: this.wo_pickup_code
        })
        .then((res) => {
    ...
    updateState () {
      this.$refs.orderFormRef.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch('wxMallOrder/setGetOrder', obj)
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
