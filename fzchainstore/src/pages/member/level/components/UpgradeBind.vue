<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      ...
      <q-scroll-area class="fit full-height">
        <q-card flat bordered class="q-my-xs">
          <q-card-section class="row items-center q-py-xs">
            ...
            <q-space />
            <q-btn label="添加次卡" unelevated @click="showGroupCountList" color="primary" />
          </q-card-section>

          <q-separator />

          <q-card-section flat class="row" v-if="selectedCountData.length > 0">
            <div class="q-pa-xs col-3" v-for="(item, key) in selectedCountData" :key="key">
              <q-list bordered separator>
                <q-item>
                  <q-item-section>
                    ...
                    <q-item-label>
                      价格：
                      <span class="text-orange"> {{ $q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} </span>
                ...
                <q-expansion-item expand-separator icon="payment" label="详情">
                  <q-list v-if="item.rechargeDetail">
                    <q-item v-for="(i, k) in item.rechargeDetail" :key="k">
                      <q-item-section side>
                        <q-item-label> {{ i.grc_count }} 次 </q-item-label>
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>
                          {{ i.grc_goodsName }}
        ...
        <q-card flat bordered class="q-my-xs">
          ...
          <q-card-section flat class="row" v-if="selectedGoodsTimingData.length > 0">
            <div class="q-px-xs col-3" v-for="(item, key) in selectedGoodsTimingData" :key="key">
              <q-list bordered separator>
                <q-item>
                  <q-item-section>
                ...
                <q-item>
                  <q-item-section>
                    ...
                    <q-item-label caption lines="3" v-if="parseInt(item.cou_type) == 6">
                      兑换 <span class="text-h6 text-deep-orange text-text-weight-bolder"> {{ item.cou_give_integral }} </span> {{ $q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_integralUnit }}
                    </q-item-label>
                    <q-item-label caption lines="3" v-if="parseInt(item.cou_type) == 7">
                      兑换 <span class="text-h6 text-deep-orange text-text-weight-bolder"> {{ item.cou_give_balance }} </span> {{ $q.localStorage.getItem('fzchainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}
                    </q-item-label>
                  </q-item-section>
    ...
    <hl-select-time-group ref="selectGoodsTimeRef" v-if="selectGoodsPoppupStatus" @dataList="setGoodsTimingData" />
