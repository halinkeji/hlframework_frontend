<template>
  <q-dialog @hide="hideNowPage" v-model="storeDayStatus" persistent>
    <q-card
      :style="{
    ...
    >
      <q-card-section class="row items-center q-pb-none">
        <div class="text-subtitle1">店铺日目标设置</div>
        <q-space />
                ...
                <div class="full-width cursor-pointer">
                  <div v-if="storeIndicatorsDay.length > 0">
                    <template v-for="(item, key) in storeIndicatorsDay">
                      <div
                        v-if="timestamp.day == item.sid_time_day && timestamp.month == item.sid_time_month && timestamp.year == item.sid_time_year"
                      >
                        <div class="q-mx-md text-center">
                          <!-- <q-btn square size="sm" color="grey-11" unelevated class="full-width text-center text-blue-grey-10 text-body1">{{
                            item.sid_consume_target_amount
                          }}</q-btn> -->
                          <q-input
                            dense
                            input-class="text-center"
                            outlined
                            v-model="item.sid_consume_target_amount"
...
<script>
export default {
  name: 'StoreDayCalendar',
  components: {},
  data() {
    return {
      storeDayStatus: false,
      saveStopData: false,
      resourceData: [],
      calendarDate: '',
      storeDayData: [],
  ...
  computed: {
    noTargetTamount() {
      ...
      that.storeIndicatorsDay.forEach(function (item) {
        amount += that.formatAmount(item.sid_consume_target_amount);
      });
  ...
  methods: {
    ...
    getCurrentReserveDetail() {
      this.$store
        .dispatch('rewardIndicators/getStoreDayData', {
    ...
    saveData() {
      ...
      this.$store
        .dispatch('rewardIndicators/setStoreDayData', obj)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              message: '成功',
              caption: `${this.dataId > 0 ? '修改' : '新增'}` + '店铺日指标成功！',
              icon: 'ion-checkmark-circle-outline',
              color: 'green',
              timeout: 500,
              position: 'top-right',
