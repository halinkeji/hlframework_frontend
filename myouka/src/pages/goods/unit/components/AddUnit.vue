    ...
    <q-dialog @hide="hideNowPage" v-model="popupActive" persistent>
      ...
      >
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">{{ dataId > 0 ? '修改' : '新增' }}计量单位</div>
        ...
        <q-card-section>
          <q-form ref="dataForm">
            <q-input outlined v-model="dataForm.gu_sort" dense label="排序" class="q-pa-sm q-my-sm" :rules="[(val) => /^\d+$/.test(val) || '请输入正确的排序']" />
            <div class="q-my-sm">
              <q-select class="q-pa-sm" outlined option-value="value" option-label="label" emit-value map-options v-model="dataForm.gu_goodsOrService" :options="options" dense label="属性" />
            </div>
            <div class="q-my-sm" v-if="dataForm.gu_goodsOrService == 1">
              <q-select
                class="q-pa-sm"
                outlined
                option-value="value"
