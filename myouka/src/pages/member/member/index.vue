    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-sm">
        ...
        >
          ...
          <q-tab name="3" label="未激活会员" v-if="authorityMeta('inactivated') && is_main_store"> </q-tab>
        </q-tabs>
      </div>

      <div class="col-shrink full-height bg-grey-2">
        <div v-if="activeName == '0'" class="fit">
          <member-list ref="memberListRef"  :memberCardLevelDatas="memberCardLevelDatas"></member-list>
        </div>
...
export default {
  ...
  mounted () {
    if (
      this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig &&
      this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_regBindFormField &&
      this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_regBindFormField > 0
    ) {
      this.getFieldData(this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_regBindFormField)
    }
  },
  created () {
    this.getLevel()
  ...
  methods: {
    changeListTab () {
      switch (this.activeName) {
        case '0':
          this.$nextTick(() => {
            this.$refs.memberListRef.restColumnData(this.columnDefsForm)
          })
    ...
    getFieldData (formFieldId) {
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            this.columnDefsForm = res.data ? res.data : []
            this.$nextTick(() => {
              this.changeListTab()
            })
          } else {
