<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-sm q-pb-lg">
        ...
        <q-btn-dropdown class="q-mt-sm q-ml-xs" color="red" label="批量操作" v-if="authorityMeta('batch')" :menu-offset="[10, 6]" unelevated>
          <q-list class="text-center">
            ...
            <q-item clickable @click="showPopup" v-if="authorityMeta('editAll')" :disable="dataListSelections.length > 0 ? false : true">
              <q-item-section>
                <q-item-label
                  >批量操作会员{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit
      ...
      <div class="q-gutter-x-xs row items-center q-mt-sm">
           <div class="col-12 col-md-3">
        <q-input
          clearable
          label="可搜索:车牌号/会员卡号/手机号/身份证号/真实姓名/昵称"
          outlined
          v-model="searchQuery"
          dense
        />
      ...
      >
        <q-form ref="memberDataRef" class="q-gutter-md q-mt-md">
          <q-card-section class="row items-center q-pb-none">
            <div class="text-h6">
              批量操作会员{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }}与{{
          ...
          <q-card-section class="q-pt-sm">
            <div class="col-12">
              <q-input
                ...
                :label="`${$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit}`"
                outlined
                ...
                :rules="[
                  (val) =>
                    /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                    `${$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit}为非负两位以内小数`,
                ]"
              />
            </div>
            <div class="col-12">
              <q-input
                :disable="parseInt(batchOperate) == 3"
                :label="`${$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit}`"
                outlined
                ...
                :rules="[
                  (val) =>
                    /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                    `${$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit}为非负两位以内小数`,
                ]"
              />
            </div>
