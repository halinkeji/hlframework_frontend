      ...
      <q-card
        :style="{
          width:
            parseInt(
              this.$q.screen.width *
                (['xs', 'sm', 'md'].indexOf($q.screen.name) > -1 ? 0.8 : 0.6)
            ) + 'px',
          'max-width':
            parseInt(
              this.$q.screen.width *
      ...
      >
        <q-toolbar>
          ...
          <q-btn
            ...
            dense
            icon="close"
            @click="specificationPopup = false"
          />
        </q-toolbar>
            ...
            <template v-slot:item="props">
              <div class="q-pa-xs col-xs-12 col-sm-6 col-md-2">
                <q-card @click="confirmWeighResult(props.row)" flat bordered>
                  <q-img
                  :src="currentGoodsInfo.coverImage && currentGoodsInfo.coverImage.id ? $q.localStorage.getItem('myouka_system_local').attach_url + currentGoodsInfo.coverImage.path : ''"
                    :ratio="1"
                  class="full-width"
                      >
                 <template v-if="!currentGoodsInfo.coverImage">
                  ...
                  <q-card-section class="q-pa-xs" style="font-size:10px">
                    <div class=" text-dark">
                      条码: {{ props.row.sl_barCode }}
                    </div>
                    <q-item-label class="text-deep-orange  row">
                      {{ $q.localStorage.getItem("myouka_store_local").storeBaseConfig.consume_base_moneySymbol }}
                      {{ props.row.sl_sellingPrice }} / {{ props.row.sl_title }}
                    </q-item-label>

                    <q-item-label class="text-primary row">
                      库存:{{ props.row.sl_inventory }}
                    </q-item-label>
                  </q-card-section>
                </q-card>
              </div>
