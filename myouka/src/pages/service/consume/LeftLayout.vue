        ...
        <q-list bordered separator class="q-mx-xs">
          ...
          >
            ...
            <q-item-section no-wrap>
              <q-item-label lines="1" class="row"> <span class="text-orange" v-if="item.chud_goodsOrService == 3">[套]</span> </q-item-label>
              <q-item-label caption class="text-body2 text-dark" lines="2">{{ item.chud_goo_name }}</q-item-label>
              <q-item-label caption class="text-body2 text-dark"
                >现价:{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.chud_selling_price
            ...
            <q-item-section side>
              <q-item-label caption class="text-italic text-body2"
                >原售价:{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}
              ...
              <q-item-label class="text-h5 text-negative"
                >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.chud_totalPrice }}</q-item-label
              >
            </q-item-section>
          </q-item>
        </q-list>
        ...
        <q-list>
          ...
          <q-item clickable v-ripple>
            ...
            <q-item-section class="text-subtitle1 text-weight-bold text-negative" v-if="totalServeFeeAmount">
              税金
              {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ totalServeFeeAmount }}
            </q-item-section>
            ...
            <q-item-section side top>
              <q-item-label class="text-h4 text-negative text-weight-bolder"
                >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ totalAmount }}</q-item-label
              >
            </q-item-section>
          </q-item>
        </q-list>
