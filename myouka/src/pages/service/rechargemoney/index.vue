                    ...
                    <q-list dense>
                      ...
                      <q-item>
                        ...
                        <q-item-section>
                          ...
                          <q-item-label caption>赠送{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }}</q-item-label>
                        </q-item-section>
                      </q-item>
                      ...
                      <q-item>
                        ...
                        <q-item-section>
                          ...
                          <q-item-label caption>赠送{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}</q-item-label>
                        </q-item-section>
                      </q-item>
                      ...
                      <q-btn
                        ...
                        :label="parseInt(packageData_id) == parseInt(item.id) ? '取消' : `${$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol}${item.rp_money}`"
          ...
          <q-card flat class="col-12 col-md-5 q-pr-sm q-mt-sm ">
            ...
            <q-card-section>
              ...
              <q-input
                ...
                class="q-mt-md text-h6"
                :rules="[(val) => (!/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/.test(val) && !!val ? false : true || '请输入正确的'+this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit)]"
              >
                <template v-slot:prepend>
                  <span class="text-subtitle1">赠送</span>
                </template>
                <template v-slot:append>
                  <div class="q-pl-none">
                    <span class="text-subtitle1">{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}</span>
                  </div>
                </template>
              </q-input>
            </q-card-section>
            ...
            <q-card-section>
              <div class="row q-my-lg">
                <div class="col-6 col-md-3 q-mt-md q-px-xs" v-for="item in fixedMoneyList">
                  <q-btn @click="rechargeAmount = item.fm_parameter" unelevated rounded color="green" :disable="memberId == 0" class="q-px-md full-width">{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ item.fm_parameter }}</q-btn>
                </div>
              </div>
            </q-card-section>
          </q-card>
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'rp_give_points', label: '赠送' + this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit, field: 'rp_give_points' }
      ],
      packageData: []
    }
  },
