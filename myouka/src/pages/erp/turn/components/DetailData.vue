    ...
    <q-card class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      ...
      <div class="col-shrink bg-white q-pb-md  text-center q-gutter-sm q-px-md">
        <q-btn
          ...
          class="q-px-xl"
          v-if="
            parseInt(data.gii_status) == 2 && parseInt(data.gii_inStoreId) == parseInt($q.localStorage.getItem('myouka_account_info').store_id)
          "
          ...
          .then((res) => {
            if (res.code == 200) {
              ...
              if (
                ...
                parseInt(this.data.gii_inStoreId) == parseInt(this.$q.localStorage.getItem('myouka_account_info').store_id)
              ) {
