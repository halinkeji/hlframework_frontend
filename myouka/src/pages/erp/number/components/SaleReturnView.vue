<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-py-sm">
      <q-btn color="primary"  unelevated @click="exportCsv" label="导出Csv文件" :disable="saleReturnData.length == 0" />
    </div>
...
export default {
  ...
  methods: {
    ...
    getSaleReturnData () {
      ...
      this.$store.dispatch('erpnumber/getSale', obj).then((res) => {
        if (res.code == 200) {
          this.page.totalCount = res.data ? res.data.page.totalCount : 0
          this.saleReturnData = res.data ? res.data.list : []
          if (res.data) {
            this.setTopTabel(this.saleReturnData)
          }
        }
      })
    },
    ...
    rowSpan (params) {
      ...
      const data = this.saleReturnData
