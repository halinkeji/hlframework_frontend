        ...
        <q-item-section side class="q-pr-xl">
          <q-btn dense size="sm" flat round color="primary" icon="las la-eye">
            <q-menu>
              ...
              <!-- <q-list style="min-width: 800px">
                <q-item class="full-width" :key="item.id + k" v-for="(i, k) in params.data.backDetail">
                  <q-item-section>
                    <q-item-label>{{ parseInt(i.oid_debt_mode) == 1 ? '以油还油' : '现金' }}</q-item-label>
                  </q-item-section>
                  <q-item-section>
                    <q-item-label class="text-subtitle1 text-weight-bold text-primary">
                      {{ i.oid_ton_count }}<span class="text-caption">吨</span>
                    </q-item-label>
                  </q-item-section>
                  <q-item-section>
                    <q-item-label class="text-subtitle1 text-weight-bold text-primary">
                      {{ i.oid_liter_count }}<span class="text-caption">升 </span></q-item-label
                    >
                  </q-item-section>
                  <q-item-section>
                    <q-item-label class="text-subtitle1 text-weight-bold text-primary">
                      {{ i.oid_total_price }}<span class="text-caption">元</span>
                    </q-item-label>
                  </q-item-section>

                  <q-item-section v-if="parseInt(i.oid_debt_mode) == 2">
                    <q-item-label>
                      盈亏 <span class="text-subtitle1 text-weight-bold text-primary">{{ i.oid_debt_profit_amount }}</span>
