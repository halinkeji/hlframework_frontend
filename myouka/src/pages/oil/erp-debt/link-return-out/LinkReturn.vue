<template>
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      ...
      <!-- <div class="col-shrink bg-white q-px-xs q-py-sm">
        <q-form ref="LendDataRef" class="row">
          <div class="col-12 col-md-3">
            <q-btn color="orange" @click="openOilRecord" unelevated label="选择借入单据" size="md" />
          </div>
        </q-form>
      </div> -->
                ...
                <div v-if="parseInt(item.retrun_type) == 1">
                  <div v-if="parseInt(item.oid_unit) == 1">
                    <q-input
                      dense
                      outlined
                      @input="changeRepaymentCount(item, item.totalTonCount, mainkey)"
                      v-model="item.totalTonCount"
                      label="请输入本次还油吨数"
                    />
                  </div>
                  <div v-if="parseInt(item.oid_unit) == 2">
                    <q-input
                      dense
                      outlined
                      @input="changeRepaymentCount(item, item.totalLiterCount, mainkey)"
                      v-model="item.totalLiterCount"
                      label="请输入本次还油升数"
                    />
                  </div>
                </div>
      ...
      <oil-record ref="oilopenRef" type="borrow_out" v-if="openPoppupStatus" @getRecord="getSelectedTypeOilData" />
    </div>
  </q-page>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    submitData () {
      this.$q
        ...
        .onOk(() => {
          ...
          this.$store.dispatch('turnoil/setLinkTurnReturn', obj).then((res) => {
            if (res.code == 200) {
              this.$q.notify({
                message: '成功',
                caption: '还回成功！',
    ...
    changeRepaymentCount (item, oilCount, key) {
      if (parseInt(item.oid_unit) == 1) {
        if (this.formatAmount(oilCount) > this.formatAmount(item.oid_last_ton_count)) {
          item.totalTonCount = 0
          this.$q.notify({
            message: '失败',
            caption: '归还吨数不能大于借油吨数',
      ...
      if (parseInt(item.oid_unit) == 2) {
        if (this.formatAmount(oilCount) > this.formatAmount(item.oid_last_liter_count)) {
          item.totalLiterCount = 0
          this.$q.notify({
            message: '失败',
            caption: '归还升数不能大于借油升数',
      ...
      if (parseInt(item.oid_unit) == 1) {
        item.totalLiterCount = this.calculateLiter(item.oid_density, item.totalTonCount)
        item.repaymentAmount = this.formatAmount(item.totalTonCount * item.oid_price)
      } else if (parseInt(item.oid_unit) == 2) {
        item.totalTonCount = this.calculateTon(item.oid_density, item.totalLiterCount)
        item.repaymentAmount = this.formatAmount(item.totalLiterCount * item.oid_price)
      }
    },
