  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
          ...
          <q-item >
            <q-item-section>
              <q-select
                outlined
                dense
                v-model="invoiceData.poi_link_company_id"
      ...
      <div class="col-shrink bg-white q-mt-xs q-py-sm text-center q-gutter-sm q-pb-md">
        ...
        <q-btn unelevated label="返回" class="q-px-xl" color="grey" :to="{ name: 'oilDebt' }" />
      </div>
    </div>
  </q-page>
</template>
...
import AddAg from './components/AddAg.vue'
export default {
  name: 'AddOpenoil',
  components: {
    AgSelect,
  ...
  methods: {
    ...
    setData () {
      this.$refs.unitDataRef.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch('turnoil/setLinkLendBack', obj)
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
    ...
    editActivedEvent (p) {
      ...
      if (parseInt(oid_unit) == 1) {
        row.oid_liter_count = parseInt((oid_tonCount / oid_density) * 1000)
        row.oid_total_price = this.formatAmount(oid_tonCount * oid_price)
      } else {
        row.oid_ton_count = this.formatAmountThree((oid_literCount * oid_density) / 1000)
        row.oid_total_price = this.formatAmount(oid_literCount * oid_price)
      }
      row.oid_last_ton_count = row.oid_ton_count
