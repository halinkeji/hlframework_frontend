<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <out-detail ref="turnDetailRef" v-if="outDetailStatus" @dataList="getDataList" />
    <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-xs">
      ...
      <q-btn unelevated color="primary" label="调出" :to="{ name: 'turnLend' }" />
    </div>
    <div class="col-shrink q-px-xs full-height bg-grey-2">
      <ag-grid-vue
        ref="agGridTable"
        class="ag-theme-balham ag-grid-table erp-in-table hl_header_center hl_content_center full-height oil-erp-turn-back-table"
...
export default {
      ...
      columnDefs: [
        {
          ...
          cellClassRules: {
            'cell-span-erp-turn-back': function (p) {
              if (p.value) {
                return true
              }
            }
        ...
        },
        {
          ...
          rowSpan: this.rowSpan,
          valueGetter: (p) => {
            return p.data.poi_invoice_type_text
          },
          cellClassRules: {
            'cell-span-erp-turn-back': function (p) {
              if (p.value) {
  ...
  methods: {
    getLendList () {
      ...
      this.$store
        .dispatch('turnoil/getTurnData', obj)
        .then((res) => {
          if (res.code == 200) {
            this.oilLendData = res.data ? res.data.list : [] // 三元运算赋值
...
<style>
.oil-erp-turn-back-table .cell-span-erp-turn-back {
  background: white;
