  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2">
        ...
        会员消费{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }}报表
      </div>
        ...
        <q-scroll-area class="fit full-height">
          <div class="bg-white q-px-md row">
            <div class="col-9 q-py-sm" v-if="is_main_store">
              <q-select label="查询油站" use-chips @input="setDateArea(selectDate)" clearable v-model="selectStoreData" option-value="id" option-label="name" multiple emit-value map-options dense outlined :options="libInStoreList" />
            </div>
              ...
              <div class="q-px-sm col-2 q-pt-sm" v-for="(item, key) in chartsData.totalData" :key="key">
                <q-card flat bordered>
                  <q-card-section align="center" class="q-py-lg">
                    <div class="text-h5">{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ item.amount }}</div>
                  </q-card-section>
                  <q-separator />
          ...
          <div class="q-mt-sm">
            <div class="q-mt-sm q-pa-md bg-white">
              <div class="full-width text-weight-bold q-mb-md">
                消费{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }}TOP排行榜
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          headerName: '金额',
          valueGetter: (p) => {
            return this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol + p.data.mcr_moneyCount
          },
  ...
  methods: {
    ...
    drawLine () {
      ...
      memberGiveCharts.setOption({
        ...
        series: [
          {
            name: '消费' + this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit,
            data: this.chartsData.consumeBalanceData ? this.chartsData.consumeBalanceData : [],
            type: 'line',
    ...
    getchartsData () {
      ...
      this.$store
        .dispatch('charts/getChartsConsumeBalanceData', params)
        .then((res) => {
          if (res.code == 200) {
    ...
    exportBut () {
      ...
      this.gridOptions.api.exportDataAsCsv({
        fileName: date + '-消费' + this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit + 'TOP排行榜'
      })
    }
  }
}
