    ...
    <q-dialog @hide="hideNowPage" v-model="viewDetails" persistent full-height>
      ...
      >
        <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
          <div class="row col-shrink bg-white q-px-xs q-py-sm text-subtitle2 q-mb-xs">
            <span class="row text-h6 q-px-md"> 整单退货 </span>
            <q-space />
            ...
            <q-scroll-area class="fit full-height">
              <q-card-section>
                <q-item-label class="row q-gutter-y-md">
                  ...
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }}支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_rechargePaymentCount) }}</span
                    >
                  </div>

                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_pointPaymentCount) }}</span
                    >
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    银行卡支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_blankPaymentCount) }}</span
                    >
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    (刷卡)微信支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_wechatPaymentCount) }}</span
                    >
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    (刷卡)支付宝支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_alipayPaymentCount) }}</span
                    >
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    个人微信支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_wechatPersonalPaymentCount) }}</span
                    >
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    个人支付宝支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_alipayPersonalPaymentCount) }}</span
                    >
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    现金支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_cashPaymentCount) }}</span
                    >
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    其他支付:<span class="text-body1"
                      >{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                      }}{{ this.formatAmount(invoiceData.sc_otherPaymentCount) }}</span
                    >
                  </div>

                  <div class="col col-md-3 col-sm-6 col-xs-12">
                    操作员:<span class="text-body1">{{ invoiceData.user ? invoiceData.user.username : '' }}</span>
                  </div>
                  <div class="col col-md-3 col-sm-6 col-xs-12">备注:{{ invoiceData.sc_note }}</div>
                </q-item-label>
              </q-card-section>
                  ...
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      ...
                      <q-td
                        ...
                        :props="props"
                      >
                        <span v-if="props.row.cid_goodsOrService == 2">服务不能单品退货</span>
                        <span v-else>
                          <q-btn
                            color="primary"
                            text-color="white"
              ...
              <q-card class="q-pa-sm" flat>
                <q-card-section class="row q-pa-none">
                  <span class="row text-h6"> 退货详情 </span>
                  <q-space />
                </q-card-section>

                <div class="row q-col-gutter-sm">
                  <div class="col-md-3 col-12">
                    <q-input outlined dense stack-label v-model="sc_return_amount" disable label="退货金额" />
                  </div>
                  <div class="col-md-3 col-12">
                    <q-input
                      outlined
                      ...
                      v-model="returnPoint"
                      disable
                      :label="`扣除${$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit}`"
                    />
                  </div>
                  <div class="col-md-3 col-12">
                    <q-input outlined dense stack-label disable v-model="returnTime" label="退货时间" />
                  </div>
                  <div class="col-md-3 col-12">
                    <q-select
                      dense
          ...
          <div class="col-shrink bg-white q-mt-xs text-center q-gutter-sm q-py-md" v-if="returnCount < 1">
            <q-checkbox dense v-model="printInvoiceStatus" label="打印小票" color="blue-8" />
            <q-btn color="primary" text-color="white" label="退货" class="q-px-xl" @click="setReturnData" unelevated />
            <q-btn color="grey" text-color="white" label="关闭" class="q-px-xl" @click="hideNowPage" v-close-popup unelevated />
          </div>
        </div>
      </q-card>
      <single-return ref="singleReturnRef" v-if="singleStatus" @getMethods="singleData"></single-return>
    </q-dialog>
  </div>
</template>
    ...
    setReturnData () {
      if (this.returnMode == 2) {
        if (this.invoiceData.member_id < 1) {
          this.$q.notify({
            message: '退货失败',
            caption: '原因：非会员不能退款到' + this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit,
            color: 'red',
