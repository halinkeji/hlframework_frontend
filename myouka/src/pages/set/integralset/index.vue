  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit + '系数',
  ...
  methods: {
    ...
    delAll () {
      this.$q
        .dialog({
          title: '是否确认',
          message: `确定删除 ${this.gridOptions.api.getSelectedRows().length} 条${this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit}设置吗`,
          cancel: true
        })
        .onOk(() => {
          this.deleteRecord()
