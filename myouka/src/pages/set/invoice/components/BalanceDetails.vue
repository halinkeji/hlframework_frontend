    ...
    <div class="col-shrink bg-white q-px-xs q-pb-md">
      <div class="row">
        <div class="col-12 col-md-5 q-mr-xs" v-if="is_main_store">
          <q-select label="查询油站" use-chips v-model="selectStoreData" option-value="id" clearable option-label="name" multiple emit-value map-options dense outlined :options="libInStoreList" />
        </div>
        <q-input class="col-12 col-md-3 q-mr-xs" label="请输入单据号" clearable outlined v-model="searchFrom.searchSerm" dense />
        <q-input v-if="!memberId" class="col-12 col-md-3 q-mr-xs" clearable label="请输入会员卡号|电话号|姓名" outlined v-model="searchFrom.memberText" dense />
...
export default {
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '变更' + this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit,
        ...
        {
          headerName: this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit + '来源',
        ...
        {
          headerName: this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit + '状态',
  ...
  methods: {
    ...
    exportBut () {
      ...
      this.gridOptions.api.exportDataAsCsv({ fileName: date + '-' + this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit + '记录' })
    },
    pinnedComputed (val) {
      const nowWidth = this.$q.screen.width
      let pinned = val
