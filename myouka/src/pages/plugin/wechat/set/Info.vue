            ...
            <div class="row q-mt-sm" v-if="dataFrom.id > 0">
              <div class="col-12 col-md-8 q-ml-md">
               ...
               <div class="col-12 col-md-8 q-ml-md q-mt-sm">
                <q-input outlined dense v-model="nearbyUrl" label="附近油站" disable>
                  <template v-slot:after>
                    <q-btn unelevated color="primary" label="复制" @click="copyClipboard(nearbyUrl)" />
                  </template>
                </q-input>
              </div>
...
export default {
  ...
  computed: {
    ...
    nearbyUrl () {
      ...
      const system_local = this.$q.localStorage.getItem('myouka_system_local')
      if (account_info && system_local) {
        const url = system_local.systemInfo.site_url + '/m/myouka/nearby?id=' + account_info.lib_id
        // if (account_info.lib_id != account_info.store_id) {
        //   url += '&StoreId=' + account_info.store_id
        // }
        return url
    ...
    manageEntranceUrl () {
      ...
      const system_local = this.$q.localStorage.getItem('myouka_system_local')
      if (account_info && system_local) {
        return system_local.systemInfo.site_url + '/m/myouka/manage/index?id=' + account_info.lib_id
      }
      return ''
    }
  },
  ...
  methods: {
    ...
    getNoticeUrl () {
      ...
      const system_local = this.$q.localStorage.getItem('myouka_system_local')
      if (account_info && system_local) {
        this.notice_url = system_local.systemInfo.site_url + '/myouka/wechat/default/wechat-event?id=' + account_info.lib_id
      }
    }
  }
}
