          ...
          <template v-slot:body="props">
            <q-tr :props="props">
              ...
              <q-td key="expand" class="text-center">
                <q-btn
                  size="sm"
                  :disable="props.row.child && props.row.child.length > 0 ? false : true"
                  color="primary"
                  round
                  unelevated
                  dense
              ...
              <q-td key="wt_sort" class="text-center" :props="props">
                {{ props.row.wt_sort }}
                <q-popup-edit v-model="props.row.wt_sort" label-set="确定" buttons @save="changeNumber(props.row)" title="编辑排序">
                  <q-input v-model="props.row.wt_sort" dense autofocus counter />
                </q-popup-edit>
              </q-td>

              <q-td key="wt_status" class="text-center">
                <q-toggle
                v-model="props.row.wt_status"
                color="green"
                true-value="1"
                false-value="0"
                @input="changeTypeStatus(props.row.id,props.row.wt_status)"
              />
              </q-td>

              <q-td key="wt_name" class="text-center">
                {{ props.row.wt_name }}
              </q-td>

              <q-td key="wt_top" class="text-center">
                {{ topObject[props.row.wt_top] }}
              </q-td>

              <q-td key="wt_cover_image" class="text-center">
                <q-img
                  :src="
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'wt_name', align: 'center', label: '分类名称', field: 'wt_name' },
        { name: 'wt_top', align: 'center', label: '分类属性', field: 'wt_top' },
        {
          name: 'wt_cover_image',
          align: 'center',
          label: '封面图',
          field: 'wt_cover_image'
        },
        {
          name: 'id',
          align: 'center',
  ...
  methods: {
    getList () {
      ...
      this.$store
        .dispatch('wxMallType/getData', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    changeNumber (row) {
      const data = row
      const regu = /^\+?[0-9][0-9]*$/
      data.wt_sort = data.wt_sort.replace(/\s+/g, '')
      if (!data.wt_sort) {
        return false
      }
      if (!regu.test(data.wt_sort)) {
        this.$q.notify({
          message: '错误',
          caption: '请输入正整数',
          color: 'red',
      ...
      const obj = {
        wt_sort: data.wt_sort,
        id: data.id
      }
      this.$store.dispatch('wxMallType/setData', obj).then((res) => {
        if (res.code == 200) {
          this.$q.notify({
            message: '成功',
            caption: '修改排序成功',
    ...
    deleteRecord (obj) {
      this.$store.dispatch('wxMallType/delData', obj).then((res) => {
        if (res.code == 200) {
          this.$q.notify({
            color: 'green',
            icon: 'ion-checkmark-circle-outline',
    ...
    deleteOneData (row) {
      this.$q
        .dialog({
          ...
          color: 'primary',
          title: '是否确认',
          message: `确定删除标题为 "${row.wt_name}" 的分类吗`,
          cancel: true
        })
        .onOk(() => {
