<template>
  <q-page class="fit">
      ...
      <div class="col-shrink bg-white q-px-xs q-py-sm">
        <div class="row q-gutter-x-sm">
          <div class="col-12 col-md-2">
            <q-input clearable label="请输入促销方案" outlined v-model="searchQuery" dense />
          </div>
    ...
    <add-promotion v-if="isPoppupStatus" @getMethods="getList" ref="addRef"></add-promotion>
  </q-page>
</template>
<script>
...
export default {
  ...
  methods: {
    getList () {
      ...
      this.$store
        .dispatch('wxMallPromotion/getData', obj)
        .then((res) => {
          if (res.code == 200) {
            if (res.data) {
    ...
    delAll () {
      this.$q
        .dialog({
          title: '是否确认',
          message: `确定删除 ${this.gridOptions.api.getSelectedRows().length} 条促销规则吗`,
          cancel: true
        })
        .onOk(() => {
          this.deleteRecord()
    ...
    deleteRecord () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('wxMallPromotion/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
