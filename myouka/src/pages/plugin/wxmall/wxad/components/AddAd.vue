      ...
      >
        ...
        <q-form ref="adDataRef" class="q-gutter-md">
          ...
          <div class="row q-px-sm">
            ...
            <q-input outlined dense  v-model="adData.wa_title" label="广告标题" lazy-rules :rules="[(val) => !!val || '广告标题不能为空']" />
               <q-select outlined option-value="value" option-label="label" emit-value map-options v-model="adData.wa_position" :options="position" dense label="广告位置" :error="false" />

              <q-select outlined option-value="value" option-label="label" emit-value map-options v-model="adData.wa_type" :options="adType" dense label="请选择广告类型" :error="false" />
              <div>
                <q-input v-if="adData.wa_type == '2'" outlined dense v-model="adData.wa_url" label="广告链接" lazy-rules :rules="[(val) => !!val || '广告链接不能为空']" />
              </div>
              <div class="row q-mb-md" v-if="adData.wa_type == '1'">
            ...
            <div class="col-12 col-md-4 q-px-sm text-center">
              ...
              <hl-upload
                  ...
                  @input="(val)=>{ $nextTick(()=>{ adData.wa_cover_image = val })}"
                  currentTitle="广告图"
          ...
          <div class="row">
            <q-input outlined dense class="col-12 q-px-sm" v-model="adData.wa_description" type="textarea" label="简介" />
          </div>
        </q-form>
...
export default {
  ...
  methods: {
    getItem (id) {
      ...
      if (id > 0) {
        this.$store.dispatch('wxMallAd/getItem', id).then((res) => {
          if (res.code == 200) {
            this.adData = res.data ? res.data : {}
            this.wxGoodsData = res.data && parseInt(res.data.wa_type) == 1 && res.data.goods ? res.data.goods : {}
          }
        })
      }
    },
    saveData () {
      this.$refs.adDataRef.validate().then((success) => {
        if (success) {
          if (this.adData.wa_type == 1 && !this.wxGoodsData.id) {
            this.$q.notify({
              message: '失败',
    ...
    getImageData (v) {
      ...
      _.forEach(v, function (value, key) {
        that.adData.wa_cover_image = value.path
      })
    },
    closeUpload (v) {
      this.uploadImgStatus = false
