<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md q-gutter-x-xs row">
      ...
      <q-input class="col-12 col-md-2" label="可搜索:合伙人姓名/合伙人手机号/会员卡号/会员姓名" outlined v-model="searchQuery" dense />
      <q-select
        outlined
        dense
        label="选择合伙人级别"
      ...
      <div class="col-12 col-md-3">
        <q-btn unelevated color="primary" label="搜索" v-if="authorityMeta('search')" @click="getList" />
        <q-btn unelevated color="grey-6" label="重置" v-if="authorityMeta('reset')" class="q-mx-sm" @click="clean" />
    ...
    <add-memberp ref="addRef" v-if="isDialogStatus" @getMethods="getList"></add-memberp>
  </div>
</template>
<script>
import Operator from './AgOperator'
import AddMemberp from './AddMemberp.vue'
export default {
  name: 'MemberLists',
  components: {
    AddMemberp,
