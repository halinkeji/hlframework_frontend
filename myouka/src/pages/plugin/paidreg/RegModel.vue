<template>
  <div class="q-pa-sm fit" v-if="currentPluginStatus">
    ...
    <q-scroll-area class="full-height">
      <div class="row q-col-gutter-xs">
        <div class="col-12 " v-for="(item, key) in packageList" :key="key">
          <q-card  flat bordered @click="checkPackage(item)" class="cursor-pointer ">
            <q-item >
              <q-item-section >
                <q-item-label>{{ item.pp_name }}
              ...
              <q-item-section side>
            <span class="text-h6 text-deep-orange" v-if="parseInt(item.pp_type) == 2">{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.pp_selling_price }}</span>
                <span v-else>
                  免费
                </span>
                  ...
                  <q-item  class="bordered-orange-color q-mt-sm" :key="k + '-coupon'" v-for="(i, k) in item.coupon">
                    <q-item-section class="q-pa-xs">
                      ...
                      <q-item-label v-if="i.cou_dateType == 1">
                        {{ $q_date.formatDate(i.cou_startTime * 1000, 'YYYY-MM-DD') + ' 至 ' + $q_date.formatDate(i.cou_endTime * 1000, 'YYYY-MM-DD') + ' 有效' }}
                      </q-item-label>
                      <q-item-label v-else-if="i.cou_dateType == 2">
                        <q-item-label v-if="i.cou_startDay == 0">
                          {{ '领取后当天生效,有效期' + i.cou_endDay + '天' }}
                        </q-item-label>
                        <q-item-label v-else>
                          {{ '领取' + (i.cou_startDay == 0 ? '后当天生效' : i.cou_startDay + '天后生效') + ',有效期' + i.cou_endDay + '天' }}
                        </q-item-label>
                      </q-item-label>
                    </q-item-section>
                  ...
                  <q-item  class="bordered-orange-color q-mt-sm" :key="k + '-count'" v-for="(i, k) in item.count">
                    <q-item-section class="q-pa-xs">
                      ...
                      <q-item-label v-else-if="parseInt(i.grp_timeType) == 3">
                        {{ $q_date.formatDate(i.grp_startTime * 1000, 'YYYY-MM-DD HH:mm:ss') }}至{{ $q_date.formatDate(i.grp_endTime * 1000, 'YYYY-MM-DD HH:mm:ss') }}
                      </q-item-label>
                      <q-item-label v-else-if="parseInt(i.grp_timeType) == 2">
                        领取后{{ i.grp_startTime }}<span v-if="parseInt(i.grp_endTime) == 1">年 </span><span v-else-if="parseInt(i.grp_endTime) == 2">季度 </span><span v-else-if="parseInt(i.grp_endTime) == 3">月 </span>
                  ...
                  <q-item  class="bordered-orange-color q-mt-sm" :key="k + '-tiem'" v-for="(i, k) in item.timing">
                    <q-item-section class="q-pa-xs">
                      ...
                      <q-item-label v-else-if="parseInt(i.gtg_time_type) == 3">
                        {{ $q_date.formatDate(i.gtg_time_start * 1000, 'YYYY-MM-DD HH:mm:ss') }}至{{ $q_date.formatDate(i.gtg_time_end * 1000, 'YYYY-MM-DD HH:mm:ss') }}
                      </q-item-label>
                      <q-item-label v-else-if="parseInt(i.gtg_time_type) == 2">
                        领取后{{ i.gtg_time_start }}<span v-if="parseInt(i.grp_endTime) == 1">年 </span><span v-else-if="parseInt(i.gtg_time_end) == 2">季度 </span><span v-else-if="parseInt(i.gtg_time_end) == 3">月 </span>
...
export default {
  ...
  data () {
    return {
      ...
      currentPluginStatus: false
    }
  },
  created () {
    this.openPlugins = LocalStorage.getItem('myouka_plugins_local').openPlugins
    if (this.openPlugins.indexOf('Paidreg') > -1) {
  ...
  methods: {
    ...
    getList (storeId) {
      if (this.currentPluginStatus) {
        const params = {
          status: 1,
          regStroreId: storeId
        }
        this.$store
          .dispatch('paidreg/getListAllData', params)
          .then((res) => {
            if (res.code == 200) {
