<template>
  <q-dialog v-model="checkoutPopupStatus" @hide="closeCheckoutPopup">
                  ...
                  <q-item class="text-center">
                    <q-item-section avatar>
                      ...
                      <q-avatar v-if="memberInfo.coverImage && memberInfo.coverImage.id" text-color="white">
                        <img :src="$q.localStorage.getItem('myouka_system_local').attach_url + memberInfo.coverImage.path" />
                      </q-avatar>
                ...
                <div class="row q-col-gutter-xs q-mt-xs">
                  <div class="col-md-6 col-12" v-if="zfkMainInfo.useBalance > 0">
                    <q-item class="bg-white q-my-xs rounded-borders q-pa-sm">
                      <q-item-section>
                        <q-item-label>
                          主卡{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }}:
                          <span class="text-subtitle1 text-weight-bold text-red">{{ zfkMainInfo.useBalance }}</span>
                          {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneyUnit }}
                        </q-item-label>
                        <q-item-label caption
                          >开启使用主卡{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }}</q-item-label
                        >
                      </q-item-section>
                  ...
                  <div class="col-md-6 col-12" v-if="zfkMainInfo.useIntegral > 0">
                    <q-item class="bg-white q-my-xs rounded-borders q-pa-sm">
                      <q-item-section>
                        <q-item-label>
                          主卡{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}:
                          <span class="text-subtitle1 text-weight-bold text-red">{{ zfkMainInfo.useIntegral }}</span>
                        </q-item-label>
                        <q-item-label caption
                          >开启使用主卡{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}</q-item-label
                        >
                      </q-item-section>
                      <q-item-section side top>
                  ...
                  <q-card-section class="row q-pa-xs">
                    <div class="col-3 text-center" v-for="(v, k) in paymentMethodData">
                      <div @click="selectPaymentMethod(v, k)">
                        <q-img
                          :src="v.iconImage && v.iconImage.id ? $q.localStorage.getItem('myouka_system_local').attach_url + v.iconImage.path : ''"
                          ratio="1"
                          width="60px"
                          :class="{
                ...
                <q-card flat square class="full-width text-h6">
                  <div class="row justify-between q-py-xs">
                    ...
                    <div class="col-4 text-right q-pr-md">
                      {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_handleMoneyCount }}
                    </div>
                  </div>
                  ...
                  <div class="row justify-between q-py-xs" v-if="sc_rulePaymentCount">
                    ...
                    <div class="col-4 text-red text-right q-pr-md">
                      -{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_rulePaymentCount }}
                    </div>
                  </div>
                  <div class="row justify-between q-py-xs" v-if="hyryxPaymentCount">
                    <div class="col-4 q-pl-md">会员日优惠</div>
                    <div class="col-4 text-red text-right q-pr-md">
                      -{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ hyryxPaymentCount }}
                    </div>
                  </div>
                  ...
                  <div class="row justify-between q-py-xs" v-if="hyryxGiveIntegral">
                    ...
                    <div class="col-4 text-primary text-right q-pr-md">
                      {{ hyryxGiveIntegral }}
                      <span class="text-body1">{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}</span>
                    </div>
                  </div>
                  ...
                  <div class="row justify-between q-py-xs" v-if="sc_couponPaymentCount">
                    ...
                    <div class="col-4 text-red text-right q-pr-md">
                      -{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_couponPaymentCount }}
                    </div>
                  </div>

                  <div class="row justify-between q-py-xs" v-if="otherCouponInfo && otherCouponInfo.record_id">
                    <div class="col-4 q-pl-md">使用优惠券</div>
                    <div class="col-4 text-red text-right q-pr-md">
                      {{ otherCouponInfo.coupon_title }}
                    </div>
                  </div>
                  ...
                  <div class="row justify-between q-py-xs" v-if="sc_rounding_amount">
                    ...
                    <div class="col-4 text-red text-right q-pr-md">
                      -{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_rounding_amount }}
                    </div>
                  </div>
                  <div class="row justify-between q-py-xs" v-if="sc_change_amount">
                    <div class="col-4 q-pl-md">现金找零</div>
                    <div class="col-4 text-red text-right q-pr-md">
                      {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_change_amount }}
                    </div>
                  </div>
                  ...
                  <div class="row justify-between q-py-xs" v-if="useGroupBalanceValue">
                    <div class="col-4 q-pl-md">油卡储值支付</div>
                    <div class="col-4 text-primary text-right q-pr-md">
                      {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ useGroupBalanceValue }}
                    </div>
                  </div>
                  ...
                  <div class="row justify-between q-py-xs">
                    ...
                    <div class="col-4 text-right q-pr-md">
                      {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_realMoneyCount }}
                    </div>
                  </div>
                </q-card>

                <q-card flat square class="full-width text-h6 q-pa-sm q-mt-xs">
                  <div class="q-py-sm" v-if="groupBalanceKey && memberGroupBalance > 0">
                    <q-item-label caption>卡项储值</q-item-label>
                    <q-item >
                      <q-item-section>
                        <q-item-label class="text-h6 text-weight-bold">{{ memberGroupBalance }}  <span class="text-caption">储值</span>  </q-item-label>
                        <q-item-label caption> 可支付本次消费金额的 {{ oilMoneyCashRatio }} % </q-item-label>
                      </q-item-section>

                      <q-item-section side top>
                        <q-checkbox @input="openGroupBalanceDeduction()" v-model="useGroupBalanceSwitch" />
                      </q-item-section>
                    </q-item>
                  </div>
                </q-card>
                          ...
                          <q-item-section>
                            ...
                            <q-item-label caption>
                              折扣: <span class="text-subtitle1 text-red">{{ formatAmount(scope.opt.ic_discount_ratio) }}</span> 折 ，{{
                                $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit
                              }}比例: 1{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneyUnit }} =
                  ...
                  <div class="row justify-between">
                    ...
                    >
                      ...
                      <div class="text-red text-body2" v-if="v.key == 'rechargePay'">
                        <!-- 总{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_balanceUnit }} :{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}
                        {{ memberInfo.mem_memberMoneyCount }}  -->
                        获赠:{{ useMoneyGivePoint }}{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}
                      </div>
                      <div class="text-red text-body2" v-if="v.key == 'pointPay'">
                        <!-- 已用{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }}：{{ useDiscountPoint }}， -->
                        可用:{{ totalAvailableAmount
                        }}{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }} ({{
                          $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol
                        }}
                        1 = {{ pay_config.pointUseCashSRatio }}
                        {{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }})
                        <!-- (总{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_integralUnit }} ： {{ memberInfo.mem_memberPointsCount }}) -->
                      </div>
                      <div class="q-mt-sm text-body2" v-if="v.key == 'cashPay'">
                        <div class="text-red text-body2">
                          实收:{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}
                          {{ formatAmount(sc_real_payment_cash) }}，
                          <span v-if="formatAmount(formatAmount(sc_real_payment_cash) - formatAmount(v.value)) > 0">
                            找零:{{ $q.localStorage.getItem('myouka_store_local').storeBaseConfig.consume_base_moneySymbol }}
    ...
    <q-dialog v-model="passwordVerifyDialog" persistent>
      <q-card
        :style="{
        ...
        flat
      >
        <q-card-section>验证会员</q-card-section>
        <q-separator />
        <q-card-section class="q-pt-none">
        ...
        >
          ...
          >
            <div class="row">
              <div class="col-12 q-mt-sm" v-if="verifyType == 'password'">
                <q-input
              ...
              <div class="col-12 q-mt-sm" v-if="verifyType == 'code'">
                 <q-input
                  ref="currentPasswordInput"
                  dense
                  outlined
                  bottom-slots
                  v-model="memberInfo.mem_memberMobile"
                  disable
                >
                 <template v-slot:after>
                  <q-btn unelevated  color="positive"  @click="sendSmsCode(memberInfo.mem_memberMobile)" :disable="!smsSendDisableStatus ? false : true" :label="smsSendStatus" />
                </template>
                </q-input>
        ...
        <q-card-actions align="around">
          <q-btn unelevated color="primary" @click="confirmVerifyPassword" class="q-px-xl">确认</q-btn>
...
export default {
  ...
  data () {
    return {
      ...
      oilMoneyCountSwitch: false, // 储值卡抵现开关
      oilMoneyCashRatio: 0, // 余额抵扣金额比率
      memberGroupBalance: 0, // 当前油品的储值总额
      useGroupBalanceSwitch: false, // 使用储值卡开关
      useGroupBalanceValue: 0, // 本次使用储值卡余额
      groupBalanceGivePointRatio: 0, // 使用储值支付赠送比例
      groupBalanceGivePointSwitch: 0, // 使用储值支付赠送开关
      groupBalanceGivePoint: 0, // 使用储值支付赠送积分
      groupBalanceKey: '',
      otherCouponInfo: {
        record_id: 0,
        coupon_id: 0,
      ...
      basePayConfig: {},
      passwordVerifyDialog: false,
      currentPasswordValue: '',
      isCurrentPasswordVerifyStatus: false,
      isCurrentPauseData: null,
      verifyType: 'password',
  ...
  computed: {},
  methods: {
    checkoutPageReceive (consumeType, money, member_id, return_param = {}, oilId = null, couponInfo = null) {
      ...
      if (couponInfo && couponInfo.record_id) {
        this.useCouponId = couponInfo.record_id
        this.sc_couponPaymentCount = couponInfo.amount
        this.otherCouponInfo = couponInfo
      }

      this.storeBaseConfig = this.$q.localStorage.getItem('myouka_store_local').storeBaseConfig

      this.$nextTick(() => {
        this.getChectoutSettle(consumeType, member_id, return_param, oilId)
      })
    },

    getChectoutSettle (consumeType = null, member_id = null, return_param = null, oilId = null) {
      const obj = {
        member_id,
        consumeType,
        return_param,
      ...
      this.$store
        .dispatch('components/checkout', obj)
        .then((res) => {
          if (res.code == 200) {
            ...
            if (res.data.other_settle_config && res.data.other_settle_config.userInfoList) {
              ...
              if (this.userInfoList && this.userInfoList.length > 0) {
                this.personnel_id = this.userInfoList[0].id
              }
            }
            ...
            if (res.data.basePayConfig) {
              this.basePayConfig = res.data.basePayConfig
            }

            // console.log('memberBalance', res.data.other_settle_config.memberGroupBalance, res.data.other_settle_config.groupBalanceKey)

            if (
              res.data.other_settle_config &&
              res.data.other_settle_config.groupBalanceKey &&
              res.data.other_settle_config.memberGroupBalance &&
              res.data.other_settle_config.memberGroupBalance > 0
            ) {
              this.memberGroupBalance = res.data.other_settle_config.memberGroupBalance

              this.groupBalanceKey = res.data.other_settle_config.groupBalanceKey
            }

            // this.groupBalanceGivePointSwitch = this.pay_config.groupBalanceGivePointSwitch
            // this.groupBalanceGivePointRatio = this.pay_config.groupBalanceGivePointRatio
            this.oilMoneyCashRatio = this.pay_config.oilMoneyCashRatio
          }
        })
        .catch((error) => {})
    },
    ...
    selectPaymentMethod (v, k) {
      ...
      if (this.basePayConfig && this.basePayConfig.balancePayVerify && parseInt(this.basePayConfig.balancePayVerify) == 1 && !this.isCurrentPasswordVerifyStatus) {
        this.isCurrentPauseData = v
        ...
        this.passwordVerifyDialog = true
        return false
      }

      if (this.basePayConfig && this.basePayConfig.integralPayVerify && parseInt(this.basePayConfig.integralPayVerify) == 1 && !this.isCurrentPasswordVerifyStatus) {
        this.isCurrentPauseData = v
        const currentPasswordInput = this.$refs.currentPasswordInput
        setTimeout(() => {
          if (currentPasswordInput) {
    ...
    confirmVerifyPassword () {
      if (this.memberInfo.mem_memberPassword) {
        if (this.verifyType == 'password') {
          if (md5(this.currentPasswordValue) == this.memberInfo.mem_memberPassword) {
            this.passwordVerifyDialog = false
            this.isCurrentPasswordVerifyStatus = true

            this.selectPaymentMethod(this.isCurrentPauseData)
          ...
          if (this.smsRecordCode && this.currentVerifyCode) {
            if (this.smsRecordCode == md5(this.currentVerifyCode)) {
              const obj = {
                phoneNumber: this.currentPhoneNumber
              }
              this.$store
                .dispatch('smsMarket/getCachSmsCode', obj)
                .then((res) => {
                  if (res.code == 200 && res.data) {
                    if (res.data == this.currentVerifyCode) {
                      this.passwordVerifyDialog = false
                      this.isCurrentPasswordVerifyStatus = true
    ...
    sendSmsCode (regPhone = null) {
      if (!regPhone) {
        this.$q.notify({
          message: '失败',
          caption: '未检测到手机号，请使用密码验证',
          icon: 'ion-close-circle-outline',
          color: 'red',
          timeout: 500,
          position: 'center'
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200 && res.data) {
            ...
            this.currentPhoneNumber = regPhone
    ...
    cancelChangePassword () {
      this.passwordVerifyDialog = false
      this.isCurrentPasswordVerifyStatus = false
      this.currentPasswordValue = ''
      this.isCurrentPauseData = null
    ...
    countTotalPayMoney () {
      ...
      const useGroupBalanceValue = this.formatAmount(this.useGroupBalanceValue) // 本次使用储值卡余额
      _(obj).forEach(function (v) {
        totalMoney += that.formatAmount(v.value)
      })

      return this.formatAmount(
        totalMoney + sc_rulePaymentCount + sc_couponPaymentCount + sc_rounding_amount + hyryxPaymentCount + useGroupBalanceValue
      )
    },

    // 打开储值卡抵现开关
    openGroupBalanceDeduction () {
      const memberGroupBalance = this.memberGroupBalance // 会员可用余额
      const sc_realMoneyCount = this.sc_realMoneyCount // 本次所需要的

      if (this.useGroupBalanceSwitch) {
        const oilMoneyCashRatio = this.oilMoneyCashRatio

        // 本次消费金额 * 余额抵现比例 = 本次需要多少余额
        const needmemberMoneyCount = this.formatAmount(sc_realMoneyCount * (oilMoneyCashRatio * 0.01)) // 需要多少余额
        console.log('needmemberMoneyCount', sc_realMoneyCount, needmemberMoneyCount, oilMoneyCashRatio)
        let memberMoneyCountstr = 0

        // 当本次需要的余额 小于等于会员的余额时
        // 新生成一个变量 将本次需要多少余额  赋值
        if (needmemberMoneyCount <= memberGroupBalance) {
          if (needmemberMoneyCount > this.sc_realMoneyCount) {
            memberMoneyCountstr = this.formatAmount(this.sc_realMoneyCount)
          } else {
            memberMoneyCountstr = needmemberMoneyCount
          }
        } else {
          memberMoneyCountstr = this.formatAmount(memberGroupBalance)
        }

        if (parseInt(this.pay_config.groupBalanceGivePointSwitch) == 1 && this.pay_config.groupBalanceGivePointRatio) {
          this.groupBalanceGivePoint = this.formatAmount(this.pay_config.groupBalanceGivePointRatio * memberMoneyCountstr)
        }

        this.useGroupBalanceValue = memberMoneyCountstr
      } else {
        this.useGroupBalanceValue = this.formatAmount(0)
        this.groupBalanceGivePoint = 0
      }
    ...
    computeRealMoneyCount () {
      ...
      const useGroupBalanceValue = this.formatAmount(this.useGroupBalanceValue) // 本次使用储值卡余额
      this.sc_realMoneyCount = this.formatAmount(
        handleMoneyCount - rulePaymentCount - couponPaymentCount - roundingAmount - hyryxPaymentCount - useGroupBalanceValue
      )
    },
