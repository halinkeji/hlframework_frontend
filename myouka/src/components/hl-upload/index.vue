  ...
  <div class="q-mt-sm">
    ...
    <div class="row q-gutter-sm" v-if="currentAttachData && currentAttachData.length > 0 && multiple">
      <div v-for="(item, key) in currentAttachData">
        <div v-if="item && item.path">
          <q-img :src="$q.localStorage.getItem('myouka_system_local').attach_url + item.path" :width="`${size}px`" :height="`${size}px`">
    ...
    <div v-if="!multiple">
      <div v-if="currentAttachObject && currentAttachObject.path">
        <q-img
          :src="$q.localStorage.getItem('myouka_system_local').attach_url + currentAttachObject.path"
          :width="`${size}px`"
          :height="`${size}px`"
        >
