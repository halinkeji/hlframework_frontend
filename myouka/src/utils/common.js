...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('myouka_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('myouka_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('myouka_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('myouka_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'myouka') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('myouka_access_token', Cookies.get('access_token'))
  LocalStorage.set('myouka_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('myouka_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('myouka_token_type', Cookies.get('token_type'))
  LocalStorage.set('myouka_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'myouka_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('myouka_user_info', Cookies.get('user_info'))
  LocalStorage.set('myouka_account_info', Cookies.get('account_info'))
