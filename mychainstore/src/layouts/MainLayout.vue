<template>
  <q-layout view="hHh Lpr lFf" class="shadow-2 fit rounded-borders">
              ...
              <q-menu fit>
                <q-card class="full-width" flat bordered>
                  ...
                  <q-card-section class="text-center q-pa-xs">
                    <q-btn rounded @click="downloadMobileRegCode" class="bg_cinnabar text-white" label="下 载" />
                  </q-card-section>
                </q-card>
              </q-menu>
            </q-btn>
    ...
    <q-drawer side="right" v-model="drawerRight" bordered :width="150" :breakpoint="500" content-class="bg-grey-3 ">
      <q-scroll-area class="fit">
        <q-tabs v-model="nowStoreId" vertical active-class="bg_moon text-dark" class="bg_rouge text-white ">
          <q-tab
            :name="item.id"
            :key="key"
            v-for="(item, key) in availableStoreList"
            :icon="item.logoData ? `img:${$q.localStorage.getItem('mychainstore_system_local').attach_url}` + item.logoData.path : ''"
...
export default {
  ...
  computed: {
    ...
    mobileRegUrl () {
      ...
      if (account_info && system_local) {
        return system_local.systemInfo.site_url + '/m/mychainstore/?id=' + account_info.lib_id + '&StoreId=' + account_info.store_id
      }
      return ''
    },
    current_title () {
      if (LocalStorage.getItem('mychainstore_apply_local')) {
        const applyLocal = LocalStorage.getItem('mychainstore_apply_local')
  ...
  created () {
    if (LocalStorage.getItem('mychainstore_apply_local')) {
      const copyright = LocalStorage.getItem('mychainstore_apply_local').copyright
    ...
    this.getWxappQrcode()
    this.getMenuData()
    const user_info = LocalStorage.getItem('mychainstore_user_info')
    ...
    const account_info = LocalStorage.getItem('mychainstore_account_info')
  ...
  methods: {
    ...
    getBusinessMenuData () {
      this.businessMenuData = LocalStorage.getItem('mychainstore_business_menu_navigate_local')
    },
    nowTime () {
      const that = this
    ...
    getMenuData () {
      const mychainstore_left_menu_navigate_local = LocalStorage.getItem('mychainstore_left_menu_navigate_local')

      this.linksData = mychainstore_left_menu_navigate_local
    },
    logoutAccount () {
      this.$q
        .dialog({
        ...
        this.$store
          ...
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$q.localStorage.set('mychainstore_account_info', account_info)
              const storeLocal = this.$q.localStorage.getItem('mychainstore_store_local')
              storeLocal.storeInfo = res.data
              this.$q.localStorage.set('mychainstore_store_local', storeLocal)
    ...
    updateMenuData () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            LocalStorage.set('mychainstore_system_local', res.data.system_local)
            LocalStorage.set('mychainstore_apply_local', res.data.apply_local)
            LocalStorage.set('mychainstore_store_local', res.data.store_local)
            LocalStorage.set('mychainstore_plugins_local', res.data.plugins_local)
            const allRouter = this.getFormatRoutes(res.data.router)
            LocalStorage.set('mychainstore_router_local', res.data.router)
            routes[1].children = allRouter

            this.$router.addRoutes(routes)
            LocalStorage.set('mychainstore_left_menu_navigate_local', res.data.menu)
