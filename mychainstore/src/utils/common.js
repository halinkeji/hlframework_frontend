...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('mychainstore_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('mychainstore_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('mychainstore_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('mychainstore_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'mychainstore') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('mychainstore_access_token', Cookies.get('access_token'))
  LocalStorage.set('mychainstore_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('mychainstore_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('mychainstore_token_type', Cookies.get('token_type'))
  LocalStorage.set('mychainstore_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'mychainstore_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('mychainstore_user_info', Cookies.get('user_info'))
  LocalStorage.set('mychainstore_account_info', Cookies.get('account_info'))
