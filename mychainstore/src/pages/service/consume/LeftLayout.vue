        ...
        <q-list bordered separator class="q-mx-xs">
          ...
          >
            <q-item-section no-wrap>
              ...
              <q-item-label caption class="text-body2 text-dark" lines="1">{{ item.chud_goo_name }}</q-item-label>
              <q-item-label caption class="text-body2 text-dark">{{ $q.localStorage.getItem('mychainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.chud_selling_price }}</q-item-label>
            </q-item-section>
            <q-item-section side>
              <q-item-label>
                <span class="cursor-pointer text-dark text-subtitle1">{{ item.chud_consumeNumber }} 件</span>
              </q-item-label>
            </q-item-section>
            <q-item-section side>
              <q-item-label caption class="text-italic text-body2">原售价:{{ $q.localStorage.getItem('mychainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.chud_goo_price }}</q-item-label>
              <q-item-label class="text-h5 text-negative">{{ $q.localStorage.getItem('mychainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.chud_totalPrice }}</q-item-label>
            </q-item-section>
          </q-item>
        </q-list>
      </q-scroll-area>
        ...
        <q-list>
          ...
          <q-item clickable v-ripple>
            ...
            <q-item-section class="text-subtitle1 text-weight-bold text-negative" v-if="totalServeFeeAmount">
              服务费
              {{ $q.localStorage.getItem('mychainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ totalServeFeeAmount }}
            </q-item-section>
            ...
            <q-item-section side top>
              <q-item-label class="text-h4 text-negative text-weight-bolder">{{ $q.localStorage.getItem('mychainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ totalAmount }}</q-item-label>
            </q-item-section>
          </q-item>
        </q-list>
      </q-card>
...
export default {
  name: 'LeftLayout',
  props: ['member_id', 'consumehangup_id', 'currentMemberInfo', 'meiyeRewardData', 'couponInfo'],
  data () {
    return {
      goodsBarCode: '',
      shoppingCartsList: [],
