<template>
  <q-page class="fit row no-wrap justify-between items-stretch content-stretch">
            ...
            <div class="row full-height">
              <div class="col-3 q-pa-sm" v-for="(item, key) in deductCountList">
                <q-card flat class="my-card">
                  <div class="absolute-right q-pa-sm" v-if="isExpireDateStatus(item)">
                    <q-btn unelevated color="amber" class="z-max" @click="editCount(item)" label="延期" />
                  </div>
                  ...
                  <q-card-section class="q-pt-none">
                    <q-input
                      input-class="color_cinnabar"
                      :disable="isExpireDateStatus(item)"
                      @input="changeDeductCount(item)"
                      :hint="`剩余${item.mrc_count}次`"
                      v-model="item.mrc_deductcount"
                      label="本次使用"
                    >
                      <template v-slot:prepend>
                        <q-btn
                          ...
                          push
                          @click="
                            item.mrc_deductcount--;
                            totalCountData();
                          "
                          :disable="parseInt(item.mrc_deductcount) < 0"
                          size="sm"
                          round
                          icon="remove"
                        />
        ...
        <div class="col-shrink bg-white q-pb-md">
          <q-card flat class="q-my-sm">
            <div class="row q-px-sm q-mt-sm text-center">
              ...
              <div class="col-6 q-pa-xs">
                <span class="text-subtitle1 text-weight-bold">总次数
                  <span class="text-h5 text-red text-weight-bold">{{ totalCount }} </span> 次
                </span>
              </div>
            </div>
          </q-card>
    ...
    <div class="col-shrink col-xs-2 col-sm-1 col-md-1 col-lg-1 col-xl-1  column justify-end">
      ...
      <q-btn class="full-width q-my-xs q-py-sm " color="positive" padding="none" :disable="memberId == 0" @click="openDelayDialog">
        <span class="text-subtitle1 q-my-xs text-white">延期记录</span>
      </q-btn>
    ...
    <count-delay-record ref="delayRecordDialogRef" @closeDelayDialog="closeDelayDialog" v-if="delayRecordDialogStatus"></count-delay-record>

    <count-set-edit v-if="countSetEditStatus" ref="recordDelayEditRef" @setSuccess="getListData"></count-set-edit>
  </q-page>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      labelVisible: false,
      meiyeRewardData: [],
      delayRecordDialogStatus: false,
      countSetEditStatus: false
    }
  },
  ...
  methods: {
    openDelayDialog () {
      this.delayRecordDialogStatus = true
      this.$nextTick(() => {
        this.$refs.delayRecordDialogRef.init(this.memberId)
      })
    },
    // 打开读卡界面
    openReadingDialog () {
    ...
    editCount (data) {
      this.countSetEditStatus = true
      this.$nextTick(() => {
        this.$refs.recordDelayEditRef.openEditDialog(data, this.memberId, data.invoice_id)
      })
    },
    closeDelayDialog () {
      this.delayRecordDialogStatus = false
    },
    ...
    isExpireDateStatus (data) {
      if (parseInt(data.mrc_endTime) >= 2147457847) {
        return false
      } else {
        const currentTimeStamp = this.$q_date.formatDate(new Date(), 'X')
        if (currentTimeStamp < data.mrc_endTime) {
          return false
        }
      }
      return true
