  ...
  <q-dialog v-model="dialogShow" persistent>
    ...
    >
      <q-card-section class="row items-center q-pb-none">
        ...
        <q-space />
        <q-btn icon="close" flat round dense v-close-popup @close="closeDialog" />
      </q-card-section>
      <q-card-section>
      ...
      <q-card-actions align="right" class="q-pb-md">
        ...
        <q-btn label="取消" @click="closeDialog" class="text-right q-px-md" color="grey" unelevated />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
export default {
  ...
  computed: {},
  methods: {
    openEditDialog (data, memberId, invoiceId) {
      ...
      this.$refs.countDataRef.validate().then((success) => {
        if (success) {
          ...
          if (this.timeType == '3') {
            ...
            obj.endTime = this.$q_date.formatDate(new Date(this.endTime), 'X')
