...
export default {
  ...
  methods: {
    ...
    init (val) {
      this.change = true
      if (this.$q.localStorage.getItem('mychainstore_store_local').storeMemberConfig) {
        this.storeMemberConfig = this.$q.localStorage.getItem('mychainstore_store_local').storeMemberConfig
      }
      if (val && val.id) {
