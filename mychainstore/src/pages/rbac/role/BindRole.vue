  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink row bg-white q-px-xs q-py-md">
        ...
        <q-tabs v-model="tab" dense class="text-grey col-12" active-color="primary" indicator-color="primary" narrow-indicator :ripple="false" align="left">
        ...
        <q-scroll-area class="fit full-height">
          <q-tab-panels v-model="tab" animated>
            <q-tab-panel :name="item.type" v-for="(item, key) in clientMenu" :key="key">
              <card-checkbox @changeChildMetaData="changeChildMetaData" ref="changMenuDataRef" @changMenuData="changMenuData" :currentChildData="item.nowMenuData" />
            </q-tab-panel>
          </q-tab-panels>
        </q-scroll-area>
      </div>
      <div class="col-shrink bg-white q-pb-md text-center q-mt-xs q-gutter-sm">
        <q-btn unelevated color="primary" class="q-px-xl" @click="saveRoleData" label="保存" />
        <q-btn unelevated color="grey" class="q-px-xl" :to="{ name: 'userList' }" label="返回" />
      </div>
    </div>
  </q-page>
</template>
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            if (res.data) {
              if (res.data.info) {
                this.role_id = res.data.info.role_id.toString()
              }
