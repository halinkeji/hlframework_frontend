          ...
          <q-scroll-area class="fit full-height">
            <q-form ref="printerDataRef" class="q-gutter-md">
              ...
              <div v-if="printerData.cp_shared == 'wuba'">
               ...
               <div class="text-subtitle2 q-ml-md">操作员绑定</div>
              <div class="text-caption">设置后，打印机根据绑定操作员进行打印。默认不区分操作员</div>
              <q-separator />

              <q-checkbox v-model="userBindData" :val="item.id" :label="item.username" :key="'check' + key" v-for="(item, key) in userListData" />
    ...
    getItem () {
      this.$store.dispatch('cloudprinter/getItem', this.printerId).then((res) => {
        if (res.code == 200) {
          ...
          if (res.data && res.data.userBindData) {
            this.userBindData = res.data.userBindData.map(item => {
              return item.cub_user_id
            })
          }
        }
      })
      ...
      this.$refs.printerDataRef.validate().then((success) => {
        if (success) {
          ...
          const obj = {
            ...
            ...printerData,
            userBindData: this.userBindData
          }
          this.$store
