      ...
      <q-form ref="typeDataRef" class="q-gutter-md">
        <div class="row q-px-sm">
          <div class="col-12 col-md-8 q-pr-xl">
            ...
            <q-input outlined dense v-model="typeData.jt_name" label="分类名称" lazy-rules :rules="[(val) => !!val || '分类名称不能为空']" />
            ...
            <jfmall-type-tree
              ...
              :inputLabel="'jt_name'"
              :inputValue="'id'"
              :showValue="typeData.jt_pid"
              @resetValue="typeData.jt_pid = ''"
        ...
        <div class="row">
          <q-input outlined dense class="col-12 q-px-sm" v-model="typeData.jt_description" type="textarea" label="简介" />
        </div>
      </q-form>
...
<script>
import jfmallTypeTree from '../select-tree/index.vue'
export default {
  name: 'AddType',
  components: {
    jfmallTypeTree
  ...
  methods: {
    getTypeList () {
      this.$store.dispatch('jfType/getListData', { currentId: this.dataId }).then((res) => {
        if (res.code == 200) {
    ...
    setPid (item) {
      if (parseInt(item.jt_level) + 1 > 3) {
        ...
        this.$refs.jfmallTypeTreeRef.resetValueData()
        return false
      } else {
        this.typeData.jt_pid = item.id

        this.typeData.jt_level = parseInt(item.jt_level) + 1
      }
    },
    getItem (id) {
      this.dataId = id
      ...
      if (id > 0) {
        this.$store.dispatch('jfType/getItem', id).then((res) => {
          if (res.code == 200) {
