    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          headerName: '相差数量',
          valueGetter: (p) => {
            if (p.data.depot_id > 0) {
              return this.formatAmount(p.data.gic_number - p.data.gic_old_depot_number)
            } else {
              return this.formatAmount(p.data.gic_number - p.data.gic_oldNumber)
            }
          },
          width: this.nowpx(0.1) + 'px'
        },
