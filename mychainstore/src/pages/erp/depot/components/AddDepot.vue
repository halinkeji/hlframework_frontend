    ...
    <q-dialog @hide="hideNowPage" v-model="showDialog">
      ...
      >
        <q-form ref="depotDataRef">
          <span style="font-size: 18px">{{ depotData.id > 0 ? '修改仓库信息' : '新增仓库信息' }}</span>
          <div class="q-mt-lg">
            <q-input class="col-12 col-md-6 q-pr-sm" outlined dense v-model="depotData.ed_code" label="编号" lazy-rules :rules="[(val) => /^\d+$/.test(val) || '请输入正确的编号']">
              <template v-slot:after>
                <q-btn color="warning" class="full-width" unelevated label="一键生成编号" @click="math" />
              </template>
            </q-input>
            <q-input class="col-12 col-md-6 q-pr-sm q-mt-xs" outlined dense v-model="depotData.ed_name" label="仓库名称" lazy-rules :rules="[(val) => (val && val.length > 0) || '请填写名称']" />
            <q-input class="col-12 col-md-6 q-pr-sm q-mt-xs" outlined dense v-model="depotData.ed_place" label="仓库位置" lazy-rules :rules="[(val) => (val && val.length > 0) || '请填写仓库位置']" />
          ...
          <div  class="q-mt-lg text-center q-gutter-sm">
