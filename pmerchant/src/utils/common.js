...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('pmerchant_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('pmerchant_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('pmerchant_access_token')
  )
}
// 判断是否当前应用
export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('pmerchant_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'pmerchant') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('pmerchant_access_token', Cookies.get('access_token'))
  LocalStorage.set('pmerchant_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('pmerchant_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('pmerchant_token_type', Cookies.get('token_type'))
  LocalStorage.set('pmerchant_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'pmerchant_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('pmerchant_user_info', Cookies.get('user_info'))
  LocalStorage.set('pmerchant_account_info', Cookies.get('account_info'))
