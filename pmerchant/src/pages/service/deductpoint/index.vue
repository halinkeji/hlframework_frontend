  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2">
        ...
        {{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }}操作
      </div>
          ...
          <q-form ref="submitFormRef" class="col-12  col-md-4  q-px-xs  q-pt-md">
            <q-card flat class="q-pa-md">
              ...
              <q-card-section class="q-py-sm" v-if="fixedMoneyList.length > 0">
                <div class="row">
                  <div class="col-6 col-md-4 q-mt-md q-px-xs" v-for="item in fixedMoneyList">
                    <q-btn unelevated rounded color="green" class="q-px-md full-width" :disable="memberId == ''" @click="pointInput = item.fm_parameter">
                      {{ item.fm_parameter }}<span class="text-body1">{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }}</span>
                    </q-btn>
                  </div>
                </div>
              </q-card-section>
      ...
      <div class="col-shrink bg-white q-pb-md q-gutter-md text-center  q-mt-xs">
        <q-btn unelevated color="blue-6" class="q-px-xl" size="lg" label="增加" icon="las la-plus-circle" :disable="!(pointInput && memberId) && !buttonDisabled" @click="pointChangeBut(1)" />
        <q-btn unelevated color="blue-6" class="q-px-xl" size="lg" label="扣除" icon="las la-minus-circle" :disable="!(pointInput && memberId)  && !buttonDisabled" @click="pointChangeBut(2)" />
        <q-btn unelevated color="red-5" class="q-px-xl" size="lg" label="清除" icon="las la-trash-alt" @click="pointInput = ''" :disable="!pointInput" />
        <q-btn unelevated color="amber-8" class="q-px-xl" size="lg" label="重置" @click="refreshBut()" :disable="!pointInput && !memberId" icon="ion-refresh" />
      </div>
    </div>
  </q-page>
</template>
