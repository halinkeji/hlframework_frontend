...
export default {
  ...
  methods: {
    ...
    setCodeUrl () {
      const system_local = this.$q.localStorage.getItem('pmerchant_system_local')
      const account_info = this.$q.localStorage.getItem('pmerchant_account_info')
      if (system_local && account_info) {
        switch (this.bindClientType) {
          case 'wechat':
            this.dataUrl = system_local.systemInfo.site_url + '/m/pmerchant/manage-ouath?bind_code=' + this.bind_code + '&id=' + account_info.lib_id
            break
        }
      }
    },
