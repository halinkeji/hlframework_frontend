            ...
            <q-item-section class="text-subtitle1 text-weight-bold">
              <span>
                ...
                <span class="text-h6 text-negative"
                  >{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ totalAmount }}</span
                >
              </span>
            </q-item-section>
            <q-item-section class="text-subtitle1 text-weight-bold">
              <span>
                总计{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }}:
                <span class="text-h6 text-positive"> {{ totalUsePoint }} </span>
              </span>
            </q-item-section>
            <q-item-section class="text-subtitle1 text-weight-bold">
              <span>
                总计{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit }}:
    ...
    openCheckoutCounterPage () {
      if (this.totalUsePoint > this.currentMemberInfo.mem_memberPointsCount) {
        this.$q.notify({
          color: 'red',
          message: '操作失败',
          caption: `您的${this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}不足，请重新选择兑换商品，或充值`,
          icon: 'ion-close-circle-outline',
          position: 'top'
        })
        return false
        ...
        this.$q.notify({
          color: 'red',
          message: '操作失败',
          caption: `您的${this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit}不足，请重新选择兑换商品，或充值`,
          icon: 'ion-close-circle-outline',
          position: 'top'
        })
        return false
