    ...
    >
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white row q-px-xs q-pb-md">
          <div class="text-subtitle1">{{ dataId ? '修改' : '新增' }}{{ parseInt(lableData.jl_type) == 1 ? '标签' : '服务' }}</div>
          <q-space />
            ...
            <div class="row">
              ...
              <div class="col-12">
                <q-input
                  outlined
                  dense
                  v-model="lableData.jl_name"
                  :label="`${parseInt(lableData.jl_type) == 1 ? '标签' : '服务'}名称`"
                  maxlength="255"
                  lazy-rules
                  :rules="[(val) => (val && val.length > 0) || (parseInt(lableData.jl_type) == 1 ? '标签' : '服务' + '标题不能为空')]"
                />
                <q-btn-toggle
                  v-model="lableData.jl_status"
                  spread
              ...
              <div class="col-12 q-mt-md" v-if="parseInt(lableData.jl_type) == 2">
                <q-input outlined v-model="lableData.jl_description" dense type="textarea" rows="2" label="服务介绍" />
              </div>
              <div class="col-12 q-mt-md" v-if="parseInt(lableData.jl_type) == 1">
                <q-input
                  outlined
                  v-model="lableData.jl_remark"
                  dense
                  type="textarea"
                  rows="2"
                  :label="`${parseInt(lableData.jl_type) == 1 ? '标签' : '服务'}简介`"
                />
              </div>
              <div class="col-12 q-mt-md" v-else>
                <hl-editor
                  ...
                  @change="
                    (v) => {
                      lableData.jl_remark = v;
                    }
                  "
                  :content="lableData.jl_remark"
                  :titleText="`${parseInt(lableData.jl_type) == 1 ? '标签' : '服务'}简介`"
...
export default {
  ...
  methods: {
    init (id) {
      ...
      console.log('aaa', id)
      if (id > 0) {
        this.$store.dispatch('jfLable/getItem', id).then((res) => {
          if (res.code == 200) {
          ...
          this.$store
            ...
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
                  caption: `${this.dataId > 0 ? '修改' : '新增'}` + parseInt(this.lableData.jl_type) == 1 ? '标签' : '服务' + '成功',
                  color: 'green',
