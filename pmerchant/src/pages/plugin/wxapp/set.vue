          ...
          <q-form ref="dataFrom" class="row q-gutter-sm">
            ...
            <div class="col-12" >
              <q-banner inline-actions rounded class="text-white bg-positive">
                还没有微信小程序？快速代注册认证微信小程序。
                只需提供法人姓名、法人微信、企业名称、企业代码，采用法人人脸识别方式替代小额打款等认证流程。
            ...
            <div class="col-6">
              <q-input outlined dense readonly label="downloadFile合法域名" v-model="defaultData.downloadFile_url">
                <template v-slot:after>
                  <q-btn unelevated color="primary" @click="copyData(defaultData.downloadFile_url)" label="复制"></q-btn>
                </template>
              </q-input>
            </div>
...
export default {
  ...
  methods: {
    setConfig () {
      const systemLocal = this.$q.localStorage.getItem('pmerchant_system_local')
      const accountInfo = this.$q.localStorage.getItem('pmerchant_account_info')
      if (systemLocal.systemInfo && accountInfo.lib_id) {
        ...
        this.defaultData.serves_url = 'https://' + hostname + accountInfo.apply_id + '/wxapp/v1/message-push?id=' + accountInfo.lib_id
      }
    },
    getItemData () {
      this.$store
    ...
    createTokenString () {
      this.dataFrom.pwa_token = this.randomString(32)
    },
