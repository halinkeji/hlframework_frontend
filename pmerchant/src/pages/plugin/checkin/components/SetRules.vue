div<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-sm q-pb-md row q-gutter-x-sm">
      <q-select class="col-12 col-md-2" outlined option-value="value" option-label="label" emit-value map-options v-model="ruleType" :options="rulesType" dense label="请选择规则类型" />
      <q-btn @click="getList" color="primary" class="col-12 col-md-1 " unelevated label="搜索" />
      <q-btn @click="clearnButton" color="warning" class=" col-12 col-md-1 " unelevated label="重置" />
      <q-btn @click="addChechinRules(0)" color="primary" class=" col-12 col-md-1 " unelevated label="新增" v-if="authorityMeta('add')" />
      <q-btn :disable="dataListSelections.length > 0 ? false : true" @click="delAll" color="red" class="col-12 col-md-1 " unelevated v-if="authorityMeta('delAll')" label="批量删除" />
    </div>
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '赠送' + this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit,
          field: 'cr_give_points',
          width: this.nowpx(0.15) + 'px'
        },
        {
          headerName: '赠送' + this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit,
          field: 'cr_give_money',
          width: this.nowpx(0.15) + 'px'
        },
        {
