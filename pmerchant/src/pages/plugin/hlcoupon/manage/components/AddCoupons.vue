      ...
      <div class="col-shrink full-height q-mt-xs">
        <q-scroll-area class="fit full-height">
          ...
          <q-card flat bordered square class="rounded-borders q-mt-xs q-pa-sm">
            <div class="q-pa-xs">
              <div class="text-subtitle2">使用范围</div>
            </div>
            <q-separator />
            <div class="row q-pt-md">
              <div class="col-12">
                <q-option-group v-model="merchantForm.pc_limit_type" :options="limitOptions" color="primary" inline dense />
              </div>

              <div class="col-6 q-mt-sm" v-if="merchantForm.pc_limit_type == 'trade'">
                <q-select
                  outlined
                  v-model="merchantForm.pc_limit_json"
                  map-options
                  emit-value
                  dense
                  multiple
              ...
              <div class="col-6 q-mt-sm" v-if="merchantForm.pc_limit_type == 'store'">
                <q-select
                  outlined
                  v-model="merchantForm.pc_limit_json"
                  map-options
                  ...
                  option-label="name"
                  option-value="id"
                  :options="merchantListData"
                  label="门店"
                  :rules="[(val) => !!val || '请选择可使用门店']"
                />
              </div>
            </div>
          </q-card>
              ...
              <div class="row q-col-gutter-sm">
                ...
                <div class="col" v-if="dataForm.cou_type == 6">
                  <q-input
                    outlined
                    bottom-slots
                    :label="'会员获得' + `${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}`"
                    ...
                    :rules="[
                      ...
                      (val) =>
                        ...
                        '请输入正确的' + `${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}`,
                    ]"
                  >
                  </q-input>
                </div>
                ...
                <div class="col" v-if="dataForm.cou_type == 7">
                  <q-input
                    outlined
                    bottom-slots
                    :label="`会员获得${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit}`"
                    ...
                    :rules="[
                      ...
                      (val) =>
                        /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/.test(val) ||
                        '请输入正确的' + $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit,
                    ]"
                  >
                  </q-input>
                </div>
              ...
              <div class="row q-mt-sm q-col-gutter-xs">
                <div v-if="dataForm.cou_couponProperty == 2 || dataForm.cou_couponProperty == 5" class="col-12 col-md-4">
                  <q-input
                    outlined
                    bottom-slots
                    :label="`${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit}支付`"
                    v-model="dataForm.cou_use_balance"
                    dense
                    :rules="[
                      (val) => !!val || $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit + '支付是必填项',
                ...
                <div v-if="dataForm.cou_couponProperty == 3 || dataForm.cou_couponProperty == 5" class="col-12 col-md-4">
                  <q-input
                    outlined
                    bottom-slots
                    :label="`${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}支付`"
                    v-model="dataForm.cou_use_integral"
                    dense
                    :rules="[
                      (val) =>
                        !!val || `${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}` + '支付是必填项',
                      (val) => /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/.test(val) || '请输入正确的数字',
                    ]"
                  >
                  </q-input>
                  ...
                  <q-card flat bordered>
                    <q-card-section class="q-pa-sm">
                      <q-item>
                        <q-item-section>
                          <q-item-label>{{
                            limitCouponTypeArr.indexOf(dataForm.cou_type) > -1 ? '请选择关联商品' : '可选择指定商品可用'
                          }}</q-item-label>
                        </q-item-section>
                        <q-item-section side>
                    ...
                    <q-card-section class="q-pa-none" v-if="bindProperty && bindProperty.goodsList && bindProperty.goodsList.length > 0">
                      <q-list separator>
                        <q-item class="q-pa-sm" :key="key" v-for="(item, key) in bindProperty.goodsList">
                          <q-item-section>
                            <q-item-label
                              >名称：{{ item.goodsName }} <span v-if="item.specId > 0">-{{ item.specificationName }}</span>
                            </q-item-label>
                            <q-item-label>
                              价格：
          ...
          <!-- <q-card flat bordered square class="rounded-borders q-mt-xs q-pa-sm" v-if="parseInt(dataForm.cou_couponProperty) == 8">
            ...
            <div class="row q-pt-md q-col-gutter-sm">
              <div class="col-4">
                <q-select
                  outlined
                  v-model="merchantForm.pc_market_status"
                  map-options
                  emit-value
                  dense
                  option-label="label"
                  option-value="value"
                  :options="marketStatusOption"
                  label="是否推广"
                  bottom-slots
                      ...
                      <template v-slot:prepend>
                        <q-icon name="event" class="cursor-pointer">
                          <q-menu square :offset="[12, 10]" transition-show="jump-down" transition-hide="jump-up">
                            <q-date v-model="dataForm.cou_startTime" mask="YYYY-MM-DD HH:mm" minimal today-btn>
                  ...
                  <div class="col" v-if="dataForm.cou_dateType == 1">
                    <q-input outlined v-model="dataForm.cou_endTime" label="结束时间" :rules="[(val) => !!val || '请选择时间']" dense readonly>
                      ...
                      <template v-slot:append>
                        <q-icon name="access_time" class="cursor-pointer">
                          <q-menu square :offset="[12, 10]" transition-show="jump-down" transition-hide="jump-up">
                            <q-time v-model="dataForm.cou_endTime" mask="YYYY-MM-DD HH:mm" minimal format24h>
                              <div class="row items-center justify-end">
                                <q-btn v-close-popup label="确定" color="primary" flat />
                              </div>
                            </q-time>
...
export default {
  ...
  methods: {
    setPageCache () {
      ...
      if (applyLocal) {
        this.tradeListData = applyLocal && applyLocal.tradeListData ? applyLocal.tradeListData : []
      }
    },
    getMerchantData () {
      this.$store
        .dispatch('merchantManage/getListData', {})
        .then((res) => {
          if (res.code == 200) {
            this.merchantListData = res.data && res.data ? res.data : []
          }
        })
        .catch((error) => {
          this.$q.notify({
        ...
        .then((res) => {
          if (res.code == 200) {
            ...
            if (res.data.pmerchantMarket) {
              ...
              this.merchantForm.pc_limit_json = res.data.pmerchantMarket && res.data.pmerchantMarket.pc_limit_json ? JSON.parse(res.data.pmerchantMarket.pc_limit_json) : []
            }
          }
        })
        .catch((error) => {
