  ...
  methods: {
    ...
    delBut () {
      this.$q
        ...
        .onOk(() => {
          if (parseInt(this.params.data.m_pmerchant_id) <= 0) {
            this.$q.notify({
              message: '失败',
              caption: '删除会员失败！',
          ...
          this.$store
            .dispatch('member/deleteMemberData', {
              m_pmerchant_id: this.params.data.m_pmerchant_id
            })
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
    ...
    batchRecovery () {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            id: this.params.data.m_pmerchant_id,
            m_pmerchant_status: 1,
