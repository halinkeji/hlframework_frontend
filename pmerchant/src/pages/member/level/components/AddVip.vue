    ...
    <q-form ref="vipDataRef" class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <div class="row">
          <q-tabs
            ...
            dense
          >
            <q-tab name="basicSettings" label="基本信息"></q-tab>
            <q-tab name="upgrade" v-if="isDefaultStatus" label="权益设置"></q-tab>
      ...
      <div class="col-shrink q-px-xs full-height bg-white q-mt-xs" v-show="activeName == 'basicSettings'">
        <q-scroll-area class="fit full-height">
          <q-card flat class="row justify-start">
            <div class="q-pa-sm col-md-5 col-12">
              <div class="text-subtitle2 q-pa-xs"><q-icon name="las la-square" color="primary"></q-icon> 基本信息</div>
              ...
              <div class="row">
                ...
                <div class="col-12 q-pr-sm q-pb-md">
                  <q-input label="权益说明" outlined v-model="memberCardLevelData.mcl_explanation" dense type="textarea" :rows="3" />
                </div>
              </div>

              <div class="text-subtitle2 q-pa-xs q-mt-sm" v-if="isDefaultStatus"><q-icon name="las la-square" color="primary"></q-icon> 类型设置</div>
              <q-separator class="q-my-xs" />
              ...
              <div class="row q-mt-sm q-col-gutter-xs" v-if="memberCardLevelData.mcl_type == 'pay'">
                ...
                <div class="row full-width q-col-gutter-xs" v-if="memberCardLevelData.mcl_time_type == '2'">
                  ...
                  <div class="text-caption text-red"><q-icon name="las la-exclamation-circle" color="red" />请点击日历与时钟图标选择日期与时间</div>
                </div>
                ...
                <div class="row full-width q-col-gutter-xs" v-if="memberCardLevelData.mcl_time_type == '3'">
                  ...
                  <div class="text-caption text-red">
                    <q-icon name="las la-exclamation-circle" color="red" />获得级别权限起计算。例如购买后 '1' '天' ,结束时间为当天的第二天的当前时间。
                  </div>
                </div>
              </div>
            </div>

            <div class="col-md-2 col-12 text-center q-pt-xl">
              <hl-upload
                :multiple="false"
                ...
                "
                currentDescription="封面:建议尺寸比例4:3"
              />
            </div>
          </q-card>
        </q-scroll-area>
          ...
          <div class="col-6">
            <q-input
              ...
              :rules="[
                (val) =>
                  /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                  '赠送' + `${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}` + '为非负两位以内小数',
              ]"
            />
          </div>
          <div class="col-6">
            <q-input
              ...
              :rules="[
                (val) =>
                  /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                  '赠送' + $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit + '为非负两位以内小数',
              ]"
            />
          </div>
        </div>
        ...
        <q-form ref="managementDataRef" class="q-gutter-md">
          <div class="row q-pl-xs">
                ...
                <q-icon name="event" class="cursor-pointer">
                  <q-menu square :offset="[12, 10]" transition-show="jump-down" transition-hide="jump-up">
                    <q-date
                      ...
                      minimal
                      :mask="
                        parseInt(management.lsr_cycle) == 1 ? 'YYYY-MM-DD HH:mm:ss' : parseInt(management.lsr_cycle) == 4 ? 'DD HH:mm' : 'MM-DD HH:mm'
                      "
                    >
                      <div class="row items-center justify-end">
                        <q-btn v-close-popup label="确定" color="primary" flat />
            ...
            <q-input
              ...
              class="col-3 col-md-2 q-mr-lg"
              :label="'扣除' + `${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}`"
              outlined
              ...
              :rules="[
                (val) =>
                  /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                  '扣除' + `${$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit}` + '为非负两位以内小数',
              ]"
            />
            <q-input
      ...
      <div class="col-shrink q-px-xs full-height bg-white q-mt-xs q-pt-sm" v-show="activeName == 'deduct'">
        <div class="row q-gutter-sm items-start">
          ...
          <q-banner class="bg-grey-5 text-white" dense rounded>
            <q-icon name="las la-exclamation-circle" color="white" />成长值的扣除是会员自动降低级别的必要条件
          </q-banner>
        </div>
        <q-table
          separator="cell"
...
export default {
  ...
  data () {
    return {
      ...
      deductionmethod: [
        { label: '扣除' + this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit, value: '1' },
        { label: '扣除' + this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit, value: '2' },
        { label: this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit + '清零', value: '3' },
        { label: this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit + '清零', value: '4' }
      ],
  ...
  methods: {
    ...
    serviceRuleText (item) {
      ...
      if (item.lsr_deduct_type == 1) {
        data += item.lsr_deduct_amount + this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneyUnit
      }
      if (item.lsr_deduct_type == 2) {
        data += item.lsr_deduct_amount + this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit
      }
    ...
    deductRuleText (row) {
      return (
        row.ldr_cycle_number +
        this.deductCycleTypeObject[parseInt(row.ldr_cycle_type)] +
        '内' +
        this.deductTypeObject[parseInt(row.ldr_type)] +
        '扣除' +
        row.ldr_deduct_grow_rate +
        '成长值'
      )
    },
