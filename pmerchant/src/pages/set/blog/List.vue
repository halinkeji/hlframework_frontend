<template>
  <q-page class="fit">
        ...
        <q-card flat class="q-pa-sm">
          <div class="row q-gutter-x-sm">
            <div class="col-12 col-md-2">
              <q-input clearable  label="请输入信息标题" outlined v-model="searchQuery" dense />
            </div>
    ...
    <add-blog v-if="isPoppupStatus" @getMethods="getList" ref="addRef"></add-blog>
  </q-page>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      attributeObject: this.$store.state.blog.attributeObject,
      columnDefs: [
        {
          headerName: '',
          field: '',
  ...
  methods: {
    getList () {
      ...
      this.$store
        .dispatch('blog/getData', obj)
        .then((res) => {
          if (res.code == 200) {
            if (res.data) {
              this.page.totalCount = res.data.page.totalCount
              this.blogsListData = res.data.list
            } else {
    ...
    addNewData (id) {
      ...
      this.$nextTick(() => {
        this.$refs.addRef.init(id, this.attribute)
      })
    },
    ...
    deleteRecord () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('blog/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
