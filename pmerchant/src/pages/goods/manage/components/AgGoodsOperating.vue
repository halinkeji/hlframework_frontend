<template>
  <div>
    ...
    <q-btn
      color="red"
      ref="buttonRefss"
      @click="statusBut(params.data.id)"
      v-if="params.data.goo_status == 1 && authorityMeta('del')"
      >删除</q-btn
    >
    <q-btn
      color="red"
      ref="buttonRefss"
      v-if="params.data.goo_status == 2 && authorityMeta('deleRecy')"
      @click="deleteBut(params.data.id)"
      >删除</q-btn
    >
  </div>
</template>
