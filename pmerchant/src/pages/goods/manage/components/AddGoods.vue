          ...
          <div v-show="tab == '1'">
            <q-card flat>
              <q-card-section class="row  q-gutter-sm">
                <div class="col-md-9 col-12">
                  ...
                  >
                    <template v-slot:after class="q-pa-none">
                      ...
                      <q-btn v-if="brcodeStatus && !goodsId" unelevated color="green" label="条码查询" :disable="dataForm.goo_code ? false : true" @click="getCodeGoodsData">
                      </q-btn>
                    </template>
                  </q-input>
                </div>
                ...
                <div class="col-md-3 col-12 ">
                  <q-input v-model="dataForm.goo_pinYin" outlined bottom-slots dense label="助记码 *" :rules="[(val) => !!val || '助记码是必填项']"> </q-input>
                </div>

                <div class="col-md-3 col-12">
                  <q-input v-model="dataForm.goo_sort" outlined bottom-slots dense label="排序" :rules="[(val) => /^\d+$/.test(val) || '请输入正确的排序']"> </q-input>
                </div>
                ...
                <div class="col-md-4 col-12">
                  <q-select outlined v-model="dataForm.goo_status" class="q-pb-md" map-options emit-value dense option-label="label" option-value="value" :options="statusOptions" label="选择状态" />
                </div>
            ...
            <q-card flat v-if="dataForm.goo_size == 1">
              ...
              <q-card-section class="q-ma-md">
                <div class="q-ml-sm text-grey-8">
                  启用商品规格后，商品的价格及库存以商品规格为准
                </div>
                <div class="q-mt-sm">
                  1.系统自动根据规格项排序。 更改规格及规格项后请点击下方的【刷新规格项目表】来更新数据。
                </div>
                <div>
                  2.每一种规格代表不同型号，例如颜色为一种规格，尺寸为一种规格。目前支持三种规格；
                </div>
                <div>
                  3.特别注意，添加规格项时一定要区分开，每个规格下的规格项不能有重复名字的，否则信息会显示错误
                </div>
                <div class="row q-mt-md q-ml-xs" v-if="goodsSizeDisable">
                  <q-btn-group unelevated>
                    <q-btn color="primary" label="新增规格" @click="addSpecification" icon="las la-plus-square" :disable="parseInt(goodsUnitData.gu_is_many) == 1 ? true : false" />
                ...
                <div v-for="(normItem, normIndex) in specificationLayoutData" :key="normIndex" v-show="goodsSizeDisable">
                    ...
                    <div class="col-md-7 col-12">
                      ...
                      >
                        ...
                        <template v-slot:append>
                          <div class="q-pl-none">
                            <div @click="deleteSpecification(normIndex)" class="text-h6" v-if="parseInt(goodsUnitData.gu_is_many) == 1 ? false : true">
                              ×
                            </div>
                          </div>
                        </template>
                  ...
                  <div class="row">
                    <div class="col-md-4 col-12 q-pr-sm" v-for="(item, itemIndex) in normItem.layoutItem" :key="itemIndex">
                      <q-input v-model="item.itemValue" outlined bottom-slots dense @change="refreshSpecification" :disable="parseInt(goodsUnitData.gu_is_many) == 1 ? true : false">
                        <template v-slot:append>
                          <div v-if="parseInt(goodsUnitData.gu_is_many) == 1 ? false : true" class="q-pl-none " @click="deleteSpecificationItem(normIndex, itemIndex)">
                ...
                <div class="row q-pb-md" v-if="goodsSizeDisable">
                  <div class="col-md-3 col-12 q-px-sm">
                    <q-input readonly v-model="changeSpe.standardBarCode" standout dense outlined class="q-mt-md" label="请输入条码" @change="refreshSpecification">
                      <template v-slot:append>
              ...
              <div class="col-12 row q-mb-sm">
                <div class="col-md-2 col-12 q-px-lg" v-if="!product_cover_image">
                   <hl-upload
                    ...
                    @input="(val)=>{ $nextTick(()=>{ dataForm.goo_cover_image = val })}"
                ...
                <div class="col-md-6 col-12 q-px-lg" v-if="product_loop_image.length < 1">
                   <hl-upload
                    ...
                    @input="(val)=>{ $nextTick(()=>{ dataForm.goo_loop_image = val })}"
                    currentDescription="建议尺寸比例1:1"
                    currentTitle="详情图"
                  />
                </div>
...
export default {
  ...
  created () {
    this.context = this
    const systemLocal = LocalStorage.getItem('pmerchant_system_local')
