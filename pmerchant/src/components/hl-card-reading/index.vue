        ...
        <div class="col-12 col-md-7" v-if="userData.id">
          <div class="row q-pa-xs q-mt-sm">
            <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">卡号：{{ userData.card_number }}</q-chip>
            </div>
            <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">昵称：{{ userData.nick_name }}</q-chip>
            </div>
            <div class="col-6 col-md-3">
            ...
            <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">
                有效期：
                <span v-if="parseInt(userData.level_end_time) < 2147457847">
                  {{ this.$q_date.formatDate(userData.level_end_time * 1000, 'YYYY-MM-DD') }}
                </span>
                <span v-else>
                  永久有效
                </span>
            ...
            <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">
                总{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }}：{{ userData.points_count > 0 ? userData.points_count : '0' }}
              </q-chip>
            </div>
            <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">
                总{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit }}：{{ userData.money_count > 0 ? userData.money_count : '0' }}
              </q-chip>
            </div>

             <div class="col-6 col-md-3">
              <q-chip square color="blue-1" text-color="blue-8">
               成长值：{{ userData.grow_rate > 0 ? userData.grow_rate : '0' }}
              </q-chip>
            </div>
        ...
        <q-card flat bordered class="q-mt-xs" v-if="userData.id">
          <q-item>
            <q-item-section avatar>
              <q-avatar v-if="userData.coverImage && userData.coverImage.id" text-color="white">
                <img :src="$q.localStorage.getItem('pmerchant_system_local').attach_url + userData.coverImage.path" />
              </q-avatar>
            ...
            <q-item-section side>
              ...
              <q-item-label caption>卡号：{{ userData.card_number }}</q-item-label>
            </q-item-section>
            <q-item-section>
          ...
          <div class="row q-col-gutter-sm items-center text-caption text-blue-9">
            <div class="col text-center">
              <div class="q-py-sm">
                <div class="text-h6 text-bold">
                  {{ formatAmount(userData.money_count) }}
                </div>
            ...
            <div class="col text-center">
              <div class="q-py-sm">
                <div class="text-h6 text-bold">
                  {{ formatAmount(userData.points_count) }}
                </div>
...
export default {
  ...
  mounted () {
    this.focusInput()
    this.openPlugins = this.$q.localStorage.getItem('pmerchant_plugins_local').openPlugins
