                ...
                <q-card flat square class="full-width q-mb-xs" v-if="memberInfo.id">
                  <q-item class="text-center">
                    ...
                    <q-item-section class="text-left">
                      ...
                      <q-item-label> <span class="text-caption">卡号：</span> {{ memberInfo.card_number }}</q-item-label>
                    </q-item-section>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label class="text-h5">{{ formatAmount(memberInfo.money_count) }} </q-item-label>
                    </q-item-section>
                    <q-item-section>
                      <q-item-label caption lines="1">
                        {{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }}
                      </q-item-label>
                      <q-item-label class="text-h5">{{ formatAmount(memberInfo.points_count) }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </q-card>
                  ...
                  <q-card-section class="row q-pa-xs">
                    <div class="col-3 text-center" v-for="(v, k) in paymentMethodData">
                      <div @click="selectPaymentMethod(v, k)">
                        <q-img
                          :src="v.iconImage && v.iconImage.id ? $q.localStorage.getItem('pmerchant_system_local').attach_url + v.iconImage.path : ''"
                          ratio="1"
                          width="60px"
                          :class="{
                ...
                <q-card flat square class="full-width text-h6">
                  <div class="row justify-between q-py-xs">
                    ...
                    <div class="col-4 text-right q-pr-md">
                      {{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_handleMoneyCount }}
                    </div>
                  </div>
                  <div class="row justify-between q-py-xs" v-if="sc_couponPaymentCount">
                    ...
                    <div class="col-4 text-red text-right q-pr-md">
                      -{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_couponPaymentCount }}
                    </div>
                  </div>
                  <div class="row justify-between q-py-xs" v-if="sc_rounding_amount">
                    <div class="col-4 q-pl-md">抹零</div>
                    <div class="col-4 text-red text-right q-pr-md">
                      -{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_rounding_amount }}
                    </div>
                  </div>
                  <div class="row justify-between q-py-xs" v-if="sc_change_amount">
                    <div class="col-4 q-pl-md">现金找零</div>
                    <div class="col-4 text-red text-right q-pr-md">
                      {{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_change_amount }}
                    </div>
                  </div>
                  ...
                  <div class="row justify-between q-py-xs">
                    ...
                    <div class="col-4 text-right q-pr-md">
                      {{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_realMoneyCount }}
                    </div>
                  </div>
                </q-card>
                          ...
                          <q-item-section>
                            ...
                            <q-item-label caption>
                              折扣: <span class="text-subtitle1 text-red">{{ formatAmount(scope.opt.ic_discount_ratio) }}</span> 折 ，{{
                                $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit
                              }}比例: 1{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneyUnit }} =
                  ...
                  <div class="row justify-between">
                    ...
                    >
                      ...
                      <div class="text-red text-body2" v-if="v.key == 'rechargePay'">
                        <!-- 总{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_balanceUnit }} :{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}
                        {{ memberInfo.mem_memberMoneyCount }}  -->
                        获赠:{{ useMoneyGivePoint }}{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }}
                      </div>
                      <div class="text-red text-body2" v-if="v.key == 'pointPay'">
                        <!-- 已用{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }}：{{ useDiscountPoint }}， -->
                        ...
                        {{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }})
                        <!-- (总{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_integralUnit }} ： {{ memberInfo.mem_memberPointsCount }}) -->
                      </div>
                      <div class="q-mt-sm text-body2" v-if="v.key == 'cashPay'">
                        <div class="text-red text-body2">
                          实收:{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}
                          {{ formatAmount(sc_real_payment_cash) }}，
                          <span v-if="formatAmount(formatAmount(sc_real_payment_cash) - formatAmount(v.value)) > 0">
                            找零:{{ $q.localStorage.getItem('pmerchant_store_local').storeBaseConfig.consume_base_moneySymbol }}
...
export default {
  ...
  computed: {},
  methods: {
    checkoutPageReceive (consumeType, money, m_pmerchant_id, return_param = {}) {
      ...
      this.storeBaseConfig = this.$q.localStorage.getItem('pmerchant_store_local').storeBaseConfig

      this.$nextTick(() => {
        this.getChectoutSettle(consumeType, m_pmerchant_id, return_param)
      })
    },

    getChectoutSettle (consumeType = null, m_pmerchant_id = null, return_param = null) {
      const obj = {
        m_pmerchant_id: m_pmerchant_id,
    ...
    paymentMethodModel (payment = null, pay_config = null) {
      ...
      _(payment).forEach(function (v) {
        switch (v.key) {
          case 'rechargePay':
            if (parseInt(pay_config.moneyCashSwitch) == 1 && that.formatAmount(pay_config.moneyCashRatio) > 0 && parseInt(that.m_pmerchant_id) > 0) {
              v.show_status = true
            } else {
              v.show_status = false
            }
          ...
          case 'pointPay':
            if (
              ...
              parseInt(that.m_pmerchant_id) > 0
            ) {
              v.show_status = true
            } else {
              v.show_status = false
