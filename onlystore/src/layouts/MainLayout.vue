    ...
    <q-header elevated class="bg-primary text-white" height-hint="98">
      <q-toolbar>
        ...
        <q-avatar class="q-mb-sm" size="md" v-if="$q.screen.gt.sm">
          <q-item-section avatar v-if="!store_info.logoData">
          ...
          <q-img
            ...
            v-else
            :src="
              `${$q.localStorage.getItem('onlystore_system_local').attach_url}` +
              (store_info.logoData && store_info.logoData.path ? store_info.logoData.path : '')
            "
          >
          ...
          <q-menu transition-show="scale" transition-hide="scale">
            <q-list bordered style="width: 200px">
              ...
              <q-item clickable v-ripple @click="openShiftDialog">
                <q-item-section avatar>
                  <q-avatar color="primary" icon="las la-calendar-check" text-color="white"></q-avatar>
                </q-item-section>
...
export default {
  ...
  computed: {
    ...
    storeEndTime () {
      const storeLocal = this.$q.localStorage.getItem('onlystore_store_local')
    ...
    mobileRegUrl () {
      const account_info = this.$q.localStorage.getItem('onlystore_account_info')
      const system_local = this.$q.localStorage.getItem('onlystore_system_local')
      if (account_info && system_local) {
        return system_local.systemInfo.site_url + '/m/onlystore/?id=' + account_info.lib_id
      }
      return ''
    },
    current_title () {
      if (LocalStorage.getItem('onlystore_apply_local')) {
        const applyLocal = LocalStorage.getItem('onlystore_apply_local')
  ...
  created () {
    if (LocalStorage.getItem('onlystore_apply_local')) {
      const copyright = LocalStorage.getItem('onlystore_apply_local').copyright
      if (copyright) {
    ...
    this.getWxappQrcode()
    this.getMenuData()
    const user_info = LocalStorage.getItem('onlystore_user_info')
  ...
  methods: {
    ...
    getMenuData () {
      const onlystore_left_menu_navigate_local = LocalStorage.getItem('onlystore_left_menu_navigate_local')

      this.linksData = onlystore_left_menu_navigate_local
    },
    logoutAccount () {
    ...
    updateMenuData () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            LocalStorage.set('onlystore_system_local', res.data.system_local)
            LocalStorage.set('onlystore_apply_local', res.data.apply_local)
            LocalStorage.set('onlystore_store_local', res.data.store_local)
            LocalStorage.set('onlystore_plugins_local', res.data.plugins_local)

            const allRouter = this.getFormatRoutes(res.data.router)
            LocalStorage.set('onlystore_router_local', res.data.router)
            routes[1].children = allRouter

            this.$router.addRoutes(routes)
            LocalStorage.set('onlystore_left_menu_navigate_local', res.data.menu)
