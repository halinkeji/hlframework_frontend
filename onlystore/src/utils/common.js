...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('onlystore_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('onlystore_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('onlystore_access_token')
  )
}
// 判断是否当前应用
export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('onlystore_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'onlystore') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('onlystore_access_token', Cookies.get('access_token'))
  LocalStorage.set('onlystore_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('onlystore_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('onlystore_token_type', Cookies.get('token_type'))
  LocalStorage.set('onlystore_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'onlystore_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('onlystore_user_info', Cookies.get('user_info'))
  LocalStorage.set('onlystore_account_info', Cookies.get('account_info'))
