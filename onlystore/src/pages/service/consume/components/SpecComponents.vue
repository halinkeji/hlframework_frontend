<template>
  <div class="flex flex-wrap">
    ...
    <q-dialog v-model="specificationPopup">
      ...
      >
        <q-toolbar>
          <q-toolbar-title> 商品规格 </q-toolbar-title>
            ...
            <template v-slot:item="props">
              <div class="q-pa-xs col-xs-12 col-sm-6 col-md-2">
                <q-card @click="confirmWeighResult(props.row)" flat bordered>
                  <q-img
                    :src="
                      currentGoodsInfo.coverImage && currentGoodsInfo.coverImage.id
                        ? $q.localStorage.getItem('onlystore_system_local').attach_url + currentGoodsInfo.coverImage.path
                        : ''
                    "
                    :ratio="1"
                    class="full-width"
                  ...
                  <q-card-section class="q-pa-xs" style="font-size: 10px">
                    <div class="text-dark">条码: {{ props.row.sl_barCode }}</div>
                    ...
                    <q-item-label class="text-primary row"> 库存:{{ props.row.sl_inventory }} </q-item-label>
                  </q-card-section>
                </q-card>
              </div>
            </template>
