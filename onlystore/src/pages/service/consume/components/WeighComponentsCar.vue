      ...
      >
        <q-form ref="consumeFormRef">
          ...
          <q-card-section>
            ...
            <div class="text-caption text-grey">单价（{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_moneyUnit }}/千克）</div>
          </q-card-section>
...
export default {
  ...
  methods: {
    ...
    init (val, type) {
      ...
      setTimeout(() => {
        if (that.$refs.consumeNumberRef) {
          ...
          const pluginLocal = LocalStorage.getItem('onlystore_plugins_local')
          if (pluginLocal && pluginLocal.eWeighing && parseInt(pluginLocal.eWeighing.psc_status) == 1) {
            const SerialPort = window.serialport
            const weighingConfigInfo = pluginLocal.eWeighing
            try {
    ...
    confirmWeighResult () {
      this.$refs.consumeFormRef.validate().then((success) => {
        if (success) {
          ...
          this.$emit('confirmWeighResult', 3, this.chud_consumeNumber)
        }
      })
    },
    ...
    computeAmountSubtotal () {
      if (this.otherValue) {
        this.amountSubtotal = this.formatAmount(this.formatAmount(this.chud_consumeNumber) * this.formatAmount(this.otherValue.chud_goo_price))
      }
    },
    // 格式化保留小数点后两位数据
    formatAmount (value) {
