  ...
  <div>
    <template v-if="parseInt(params.data.rtb_duration_type) == 2">
      <div class="row" text-align="center">
        <div class="col-4">
          <q-input dense class=" q-px-sm full-height" v-model="params.data.rtb_time_compute_number" @change="computedDuration"> </q-input>
        </div>
        <div class="col-4">
          <q-select
        ...
        <div class="col-4">
          <q-chip outline color="primary" text-color="white"> {{ params.data.rtb_duration_total }}分 </q-chip>
        </div>
      </div>
    </template>
    <template v-if="parseInt(params.data.rtb_duration_type) == 1">
      <q-input dense class="col-12 col-xs-6 col-sm-2" v-model="params.data.rtb_duration_total" @change="durationError" >
