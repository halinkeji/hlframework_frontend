  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2">
        ...
        {{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit }}扣费
      </div>
              ...
              <q-card-section class="q-py-sm" v-if="fixedMoneyList.length > 0">
                <div class="row">
                  <div class="col-6 col-md-3 q-mt-md q-px-xs" v-for="item in fixedMoneyList">
                    <q-btn unelevated rounded color="green" class="q-px-md full-width" @click="deductAmount = item.fm_parameter">
                      {{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ item.fm_parameter }}
                    </q-btn>
                  </div>
                </div>
              </q-card-section>
      ...
      <div class="col-shrink bg-white q-pb-md q-gutter-md text-center q-mt-xs">
        ...
        <q-btn
          ...
          size="lg"
          label="扣除"
          :disable="deductAmount ? false : true"
          @click="minusBut()"
    ...
    getMemberInfo(val) {
      if (val) {
        if (val.id) {
          ...
          this.$nextTick(() => {
            if (this.fixedMoneyList && this.fixedMoneyList.length > 0) {
              // this.deductAmount = this.fixedMoneyList[0].fm_parameter
              this.$nextTick(() => {
                this.minusBut()
              })
            }
          ...
          this.$store
            ...
            .then((res) => {
              if (res.code == 200) {
                ...
                this.$q.notify({
                  message: '失败',
                  caption: '扣除失败,' + this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit + '不足',
