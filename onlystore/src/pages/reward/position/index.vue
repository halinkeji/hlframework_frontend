<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-py-md row q-gutter-x-sm">
      <div class="col-2">
      ...
      <q-btn :disable="dataListSelections.length > 0 ? false : true" @click="delAll" color="red"  unelevated v-if="authorityMeta('del')" label="批量删除" />
    </div>
...
export default {
  ...
  methods: {
    getList () {
      ...
      this.$store
        .dispatch('commonReward/getData', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    deleteRecord () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('commonReward/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
