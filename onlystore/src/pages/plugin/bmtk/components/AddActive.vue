            ...
            <div v-show="activeName == '1'">
              ...
              <div class="row">
                <div class="col-12 col-md-6 q-mt-sm q-pr-md">
                  <q-input outlined v-model="startTime" label="活动开始时间" dense readonly :rules="[(val) => !!val || '请输入活动开始时间']">
                ...
                <div class="col-12 col-md-6 q-mt-sm q-pr-md">
                  <q-input outlined v-model="endTime" label="活动结束时间" dense readonly :rules="[(val) => !!val || '请输入活动开始时间']">
                    <template v-slot:prepend>
                      <q-icon name="event" class="cursor-pointer">
                        <q-menu :offset="[12, 10]" transition-show="jump-down" transition-hide="jump-up">
              ...
              <div class="row">
                <div class="col-12 col-md-6 q-mt-sm q-pr-md">
                  <q-input outlined v-model="subData.bmt_background" label="背景颜色" :error="false" dense>
                    <template v-slot:prepend>
            ...
            <div v-show="activeName == '2'">
              <q-card flat class="row">
                <div class="q-pr-md col-6">
                      ...
                      <q-item-section>
                        ...
                        <q-item-label caption lines="2">
                          <q-badge outline color="red" style="font-size: 10px">
                            <span v-if="parseInt(item.cou_type) == 1"> 折 扣 券 </span>
                            <span v-if="parseInt(item.cou_type) == 2"> 代 金 券 </span>
                            <span v-if="parseInt(item.cou_type) == 3"> 礼 品 券 </span>
                            <span v-if="parseInt(item.cou_type) == 4"> 团 购 券 </span>
                            <span v-if="parseInt(item.cou_type) == 5"> 优 惠 券 </span>
                          </q-badge>
                        </q-item-label>
                  ...
                  <div class="q-ml-md" v-else>暂无赠送</div>
                </div>
              </q-card>
    ...
    saveData () {
      this.$refs.bmtkDataRef.validate().then((success) => {
        if (success) {
          ...
          subData.bmt_start_time = subData.bmt_start_time
            ? this.$q_date.formatDate(subData.bmt_start_time, 'X')
