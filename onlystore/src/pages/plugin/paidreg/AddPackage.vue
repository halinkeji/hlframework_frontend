            ...
            <q-form ref="packageDataRef">
              <div class="q-pr-sm">
                <q-banner rounded class="text-white bg-red" dense>
                  注意：赠送卡券、次卡、{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit }}、{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit }}时，不填不选则不赠送! 开启会员权益会直接赠送会员级别相关权益！
                </q-banner>
              </div>
              ...
              <div class="row q-mt-lg">
                <div class="col-6 q-pr-md">
                  <q-input outlined dense v-model="packageData.pp_give_balance" :label="`赠送${$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit}`" :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的赠送' + $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit]" />
                </div>
                <div class="col-6 q-pr-sm">
                  <q-input outlined dense v-model="packageData.pp_give_integral" :label="`赠送${$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit}`" :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的赠送'+`${$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit}`]" />
                </div>
              </div>
              <div class="q-mt-xs">
                <q-chip outline square color="red" text-color="white"> 赠送{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit }} {{ packageData.pp_give_balance }} {{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_moneyUnit }}，赠送 {{ packageData.pp_give_integral }} {{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit }} </q-chip>
              </div>
