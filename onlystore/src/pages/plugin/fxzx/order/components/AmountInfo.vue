<template>
  <div :style="{ width: params.column.actualWidth - 30 + 'px' }" >
   <div class="row justify-between" v-if="parseFloat(fxzxData.oneFxzxMemberAmount)>0">
     ...
     <div>
       佣金：{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{fxzxData.oneFxzxMemberAmount}}
     </div>
   </div>
   <div class="row justify-between" v-if="parseFloat(fxzxData.twoFxzxMemberAmount)>0">
     ...
     <div >
       佣金：{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{fxzxData.twoFxzxMemberAmount}}
     </div>
   </div>
   <div class="row justify-between" v-if="parseFloat(fxzxData.threeFxzxMemberAmount)>0">
     ...
     <div>
       佣金：{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{fxzxData.threeFxzxMemberAmount}}
     </div>
   </div>
   <div class="row justify-end" v-if="parseFloat(fxzxData.totalFxzxAmount)>0">
       合计佣金：{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{fxzxData.totalFxzxAmount}}
   </div>
  </div>
</template>
<script>
