    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <div class="row  q-gutter-x-sm">
          ...
          <q-btn :to="{ name: 'csjzAddActive' }" color="primary" class="col-12 col-md-1 q-mt-sm" unelevated label="添加活动" v-if="authorityMeta('add')" />
        </div>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '总限捐人数',
          field: 'pca_limit_buy_count',
          width: this.nowpx(0.08) + 'px'
        },
        {
          headerName: '虚拟捐赠人数',
          field: 'pca_fake_buy_count',
          width: this.nowpx(0.08) + 'px'
        },
        {
          headerName: '虚拟浏览量',
  ...
  methods: {
    ...
    expireDate (p) {
      return this.$q_date.formatDate(p.data.pca_start_time * 1000, 'YYYY-MM-DD HH:mm:ss') + ' 至 ' + this.$q_date.formatDate(p.data.pca_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') + ' 有效'
    },
    pinnedComputed (val) {
      const nowWidth = this.$q.screen.width
      let pinned = val
