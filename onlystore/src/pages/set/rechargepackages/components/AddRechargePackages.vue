            ...
            <div class="col-8 col-md-3 q-px-xs">
              ...
              >
                <template v-slot:append>
                  ...
                  <span style="font-size: 14px">{{ returnRecharge }}{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit }}/期</span>
                </template>
              </q-input>
            </div>
           <div class="col-8 col-md-3 q-px-xs">
            ...
            <div class="col-8 col-md-3 q-px-xs">
              ...
              >
                <template v-slot:append>
                  <q-separator dark color="grey" class="q-mx-sm" vertical />
                  <span style="font-size: 14px">{{ returnPoint }}{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit }}/期</span>
                </template>
              </q-input>
            </div>
            ...
            <div class="q-mt-lg">
              合 计 : <span class="text-h6 text-negative"> {{ rechargeSum }} </span> {{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit }}，赠 送
              <span class="text-h6 text-negative"> {{ rechargeFrom.rp_give_points }} </span>{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit }}
            </div>
          </q-form>
        </div>
  ...
  methods: {
    ...
    getItem () {
      this.$store.dispatch('rechargepackages/getItem', this.packageId).then((res) => {
        if (res.code == 200) {
          const data = res.data
          this.startTime = this.$q_date.formatDate(data.rp_start_time * 1000, 'YYYY-MM-DD HH:mm:ss')
          this.endTime = this.$q_date.formatDate(data.rp_end_time * 1000, 'YYYY-MM-DD HH:mm:ss')
