  ...
  <q-dialog @hide="hideNowPage" v-model="isSidebarActiveLocal" persistent>
    <q-card>
      <q-card-section class="row q-pa-sm items-center">
        <div class="text-subtitle2">{{ dataId ? '修改' : '新增' }}{{ typeDataItem[fixedAmountFrom.fm_type] }}</div>
        <q-space />
...
export default {
  ...
  data () {
    return {
      ...
      typeDataItem: this.$store.state.fixed.typeDataItem,
  ...
  methods: {
    init (id, type) {
      ...
      this.fixedAmountFrom.fm_type = type
      if (id > 0) {
        const obj = {
          id: this.dataId
        }
