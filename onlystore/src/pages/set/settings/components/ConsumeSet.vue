  ...
  <div class="centerx labelx">
    <q-form ref="consumeSetFormRef" class="row">
      ...
      <div class="col-md-4 col-12 q-pr-md q-mt-md">
        <q-card bordered flat>
          <q-card-section>
            <span class="text-subtitle1">自定义商品单价折扣</span>
            <q-toggle
              v-model="consumeDataForm.consume_consume_priceDiscountSwitch"
              color="green"
              :true-value="'1'"
              :false-value="'0'"
              class="float-right"
              @input="handleSwitchChange(consumeDataForm.consume_consume_priceDiscountSwitch, 'consume_consume_priceDiscountSwitch')"
            />
          </q-card-section>
          <q-separator />
          <div class="q-ma-md">
            启用后，消费时会员可根据设置的折扣比例进行折扣。示例设置80,90。在消费收银改价时 可选择80%、90%进行单价折扣，例如本次购买 2 件商品，商品单价：15元，选择80%进行折扣，(2 * 15) * 80% = 24元。
          </div>
          <q-input
            outlined
            bottom-slots
            v-model="consumeDataForm.consume_consume_priceDiscountValue"
            dense
            class="q-px-md"
            @change="inputChange(consumeDataForm.consume_consume_priceDiscountValue, 'consume_consume_priceDiscountValue')"
          >
          </q-input>
        </q-card>
      </div>
  ...
  data () {
    return {
      consumeDataForm: {
        ...
        consume_consume_priceDiscountSwitch: '0',
        consume_consume_priceDiscountValue: ''
      },
      consumeData: []
    }
  },
