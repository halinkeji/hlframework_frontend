<template>
  <div>
    <q-dialog @hide="hideNowPage" v-model="viewDetails" persistent>
        ...
        <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
          ...
          <div class="col-shrink q-pa-sm full-height bg-white">
            <q-scroll-area class="fit full-height">
                ...
                <div class="col-6">
                  ...
                  <q-item v-if="invoiceData.sc_handleMoneyCount">
                    ...
                    <q-item-section side>
                      ...
                      }}<q-btn round color="secondary" @click="openChangeTimeDialog" flat dense size="sm" icon="las la-edit"
                      /></q-item-label>
                    </q-item-section>
                  </q-item>
                  <q-item v-if="invoiceData.sc_handleMoneyCount">
                ...
                >
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      ...
                      <q-td
                        :key="rowSpan(props.row, props.rowIndex, 'cid_packageName') > 0 ? 'cid_packageName' : ''"
              ...
              <div class="q-mt-sm" v-if="returnGoodsDetail  && returnGoodsDetail.length > 0">
                <q-table
                  title="退货详情"
              ...
              <div class="q-mt-sm" v-if="invoiceData.pointsCountRecord && invoiceData.pointsCountRecord.length > 0">
                <q-table
                  :title="`${$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit}详情`"
              ...
              <div class="q-mt-sm" v-if="invoiceData.moneyCountRecord && invoiceData.moneyCountRecord.length > 0">
                <q-table
                  :title="`${$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit}详情`"
      ...
      <q-dialog v-model="changeTimeDialogStatus" persistent>
        <q-card
          :style="{
        ...
        >
          <q-card-section class="row">
            <div class="text-subtitle2">修改单据时间</div>
            <q-space />
            <q-btn icon="close" flat round dense v-close-popup @click="hideChangeTimeDialog" />
          </q-card-section>
          <q-separator />
          <q-card-section class="row">
            <q-form ref="fixedDataRef" class="row full-width">
             <div class="col-12">
               <q-input outlined dense label="开始时间" clearable v-model="newTime" readonly>
                <template v-slot:prepend>
                  <q-icon name="event" class="cursor-pointer">
                    <q-popup-proxy transition-show="scale" transition-hide="scale">
                      <q-date minimal v-model="newTime" mask="YYYY-MM-DD HH:mm:ss">
                ...
                <template v-slot:append>
                  <q-icon color="grey" v-if="newTime" @click="newTime = ''" name="cancel"></q-icon>
                  <q-icon name="access_time" class="cursor-pointer">
                    <q-popup-proxy transition-show="scale" transition-hide="scale">
                      <q-time v-model="newTime" mask="YYYY-MM-DD HH:mm:ss" format24h>
                        <div class="row items-center justify-end">
                          <q-btn v-close-popup label="确定" color="primary" flat />
                        </div>
                      </q-time>
          ...
          <q-card-section class="row justify-center q-gutter-sm">
            <q-btn class="q-px-xl" label="提交" unelevated color="primary" @click="changeInvoiceCreateTimeData()" />
            <q-btn class="q-px-xl" label="关闭" unelevated @click="hideChangeTimeDialog()" color="grey" />
          </q-card-section>
        </q-card>
      </q-dialog>
    </q-dialog>
...
export default {
  ...
  data () {
    return {
      ...
      integralData: [
        {
          label: '变更' + this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit,
          align: 'center',
          field: 'pcr_pointsCount'
        },
        {
          align: 'center',
          label: this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit + '来源',
          field: 'pcr_pointsCountSource'
        },
        {
          label: this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit + '状态',
          align: 'center',
      ...
      storedData: [
        {
          label: '变更' + this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit,
          align: 'center',
          field: 'mcr_moneyCount'
        },
        {
          align: 'center',
          label: this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit + '来源',
          field: 'mcr_moneyCountSource'
        },
        {
          label: this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit + '状态',
          align: 'center',
      ...
      consumeBasisColumn: [
        ...
        {
          label: '获得' + this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit,
          align: 'left',
          field: 'sc_getPoint'
        }
      ],
      consumPayColumn: [
        {
          label: this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_balanceUnit + '支付',
          align: 'left',
        ...
        {
          align: 'left',
          label: this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit + '支付',
  ...
  methods: {
    ...
    getViewData (id) {
      ...
      this.invoiceId = id
      if (this.$q.localStorage.getItem('onlystore_store_local') && this.$q.localStorage.getItem('onlystore_store_local').openPayment) {
        this.paymentChannel = this.$q.localStorage.getItem('onlystore_store_local').openPayment
      }
      if (id > 0) {
        const obj = {
          id: id
    ...
    changeInvoiceCreateTimeData () {
      const obj = {
        invoice_id: this.invoiceId,
        newTime: this.newTime
      }
      this.$store
        .dispatch('invoice/changeInvoiceCreateTimeData', obj)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              message: '成功',
            ...
            this.hideChangeTimeDialog()
            this.invoiceData.created_at = res.data
          } else {
            this.$q.notify({
              message: '失败',
              caption: res.message,
    ...
    hideChangeTimeDialog () {
      this.newTime = ''
      this.changeTimeDialogStatus = false
    },
    openChangeTimeDialog () {
      this.newTime = ''
      this.changeTimeDialogStatus = true
    }
  }
}
</script>
