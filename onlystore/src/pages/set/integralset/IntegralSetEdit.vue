  ...
  <q-dialog v-model="dialogShow" persistent>
    ...
    >
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">修改{{ $q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit }}折扣设置-{{ integralData.ic_name }}</div>
        <q-space />
      ...
      <q-card-section>
        <q-form ref="integralDataRef" class="q-gutter-md">
          <q-card flat square class="q-pa-xs">
            <div class="row">
              <div class="col-12">
                <q-input outlined dense class="q-px-sm" v-model="integralData.ic_name" label="规则名称" lazy-rules :rules="[(val) => (val && val.length > 0) || '规则名称不能为空']" />
              </div>
              <div class="col-12 q-mt-sm">
               ...
               <div class="col-12 q-mt-sm">
                <q-select  class="q-px-sm" outlined label="是否会员可用" bottom-slots v-model="integralData.ic_is_member_use" :options="statusOptions" dense emit-value map-options> </q-select>
              </div>
