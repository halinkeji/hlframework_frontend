...
import MemberUseMode from './components/MemberUseMode'
export default {
  name: 'index',
  components: {
    Operating,
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit + '换算比例',
        ...
        {
          headerName: '是否会员可用',
          field: 'ic_is_member_use',
          cellRendererFramework: 'MemberUseMode',
          width: this.nowpx(0.1) + 'px'
        },
  ...
  methods: {
    ...
    delAll () {
      this.$q
        .dialog({
          title: '是否确认',
          message: `确定删除 ${this.gridOptions.api.getSelectedRows().length} 条${this.$q.localStorage.getItem('onlystore_store_local').storeBaseConfig.consume_base_integralUnit}设置吗`,
          cancel: true
        })
        .onOk(() => {
          this.deleteRecord()
