<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md q-gutter-xs row q-pt-sm">
      ...
      <div class="col-12 col-md-2 q-mr-xs">
        <q-input outlined v-model="endTime" label="结束时间" dense square readonly>
          <template v-slot:prepend>
            <q-icon name="event" class="cursor-pointer">
              <q-popup-proxy transition-show="scale" transition-hide="scale">
                <q-date v-model="endTime" mask="YYYY-MM" today-btn minimal default-month default-view="Months" years-in-month-view>
                  <!-- <q-date v-model="endTime" mask="YYYY-MM-DD" today-btn minimal> -->
                  <div class="row items-center justify-end">
                    <q-btn v-close-popup label="确定" color="primary" flat />
                  </div>
                </q-date>
      ...
      <q-btn
        color="primary"
        class="col-6 col-xs-3 col-sm-1"
        unelevated
        @click="countExport"
        label="导出Xls文件"
        :disable="openCardRecord.length == 0"
      />
    </div>
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          valueGetter: (p) => {
            return this.$q_date.formatDate(p.data.end_time * 1000, 'YYYY-MM ');
          },
        },
        {
          headerName: '门店名称',
  ...
  methods: {
    ...
    getList() {
      ...
      this.$store
        .dispatch('rewardPosition/getStoreRecordList', obj)
        .then((res) => {
          if (res.code == 200) {
            this.openCardRecord = res.data ? res.data : [];
    ...
    countExport() {
      ...
      this.$store
        .dispatch('rewardPosition/getExport', obj)
        .then((res) => {
          if (res.code == 200) {
