      ...
      <div class="col-shrink bg-white q-px-xs q-pb-md q-pt-sm row q-gutter-xs items-center">
        <div class="col-3">
          <q-input label="可搜索:收件人/手机号/地址/详细地址" outlined v-model="searchText" dense />
           ...
           <q-btn color="primary" class="q-px-xl" unelevated label="搜索" @click="getList" />
        <q-btn color="warning" class="q-px-xl" unelevated label="重置" @click="clearBut" />
          <q-btn color="primary" class="q-px-xl" unelevated label="新增" @click="editMemberAddress(memberId,0)" />
      </div>
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      this.$store
        .dispatch('address/getAddressList', obj)
        .then((res) => {
          if (res.code == 200) {
