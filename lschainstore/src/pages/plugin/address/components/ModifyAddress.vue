    ...
    >
     <div class="col-shrink bg-white row q-px-xs q-pa-sm">
      ...
      <q-form ref="addressDataRef" class="q-gutter-xs">
        <div class="row">
          <q-input outlined dense class="col-12 col-md-6 q-mt-sm q-px-sm" v-model="addressData.ma_name" label="收件人" lazy-rules :rules="[(val) => (val && !!val) || '收件人不能为空']" />
          <q-input
            outlined
            dense
        ...
        <div class="row">
          <div class="col-6 q-px-sm q-pt-xs">
            <hl-address :province="addressData.province" :city="addressData.city" :area="addressData.district" @province="provinceupdate" @city="cityupdate" @area="areaupdate"></hl-address>
          </div>
          <q-input outlined dense class="col-12 col-md-6 q-mt-sm q-px-sm" v-model="addressData.ma_address" label="详细地址" lazy-rules :rules="[(val) => (val && !!val) || '详细地址不能为空']" />
        </div>
      </q-form>
...
export default {
  ...
  methods: {
    ...
    // 获取单条
    getItem () {
      this.$store.dispatch('address/getItem', this.currentAddressId).then((res) => {
        if (res.code == 200) {
