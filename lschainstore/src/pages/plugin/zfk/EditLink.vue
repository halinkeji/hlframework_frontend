      ...
      <q-form ref="linkDataRef">
            ...
            <q-card-section class="col-6 row">
              <q-item-section avatar>
                <q-avatar v-if="memberLinkData.member && memberLinkData.member.coverImage && memberLinkData.member.coverImage.id" text-color="white">
                  <img :src="$q.localStorage.getItem('lschainstore_system_local').attach_url + memberLinkData.member.coverImage.path" />
                </q-avatar>
            ...
            <q-card-section class="col-6 row">
              <q-item-section avatar>
                <q-avatar v-if="memberLinkData.sonMember && memberLinkData.sonMember.coverImage && memberLinkData.sonMember.coverImage.id" text-color="white">
                  <img :src="$q.localStorage.getItem('lschainstore_system_local').attach_url + memberLinkData.sonMember.coverImage.path" />
                </q-avatar>
                <q-avatar v-else><q-icon color="grey" name="las la-user-circle" size="xl"></q-icon></q-avatar>
              </q-item-section>
              <q-item-section>
        ...
        <div class="q-mt-sm">
          <q-select
            ...
            :label="`是否共享${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit}`"
            emit-value
            map-options
          />
        </div>
        <div class="q-mt-lg" v-if="parseInt(memberLinkData.zml_is_share_balance) == 2">
          <q-input
            ...
            :label="`共享${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit}`"
            :rules="[
              (val) => !!val || '必填项',
              (val) => /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/.test(val) || '请输入正确的' + $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit,
            ]"
          />
        </div>
        <div class="q-mt-xs" v-if="parseInt(memberLinkData.zml_is_share_balance) == 2">
          <q-select
            ...
            option-label="label"
            :options="timeOptions"
            :label="`${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit}日期限制`"
            emit-value
            map-options
          />
        </div>
        <div class="q-mt-lg">
          <q-select
            ...
            option-label="label"
            :options="shareOptions"
            :label="`是否共享${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit}`"
            emit-value
            map-options
          />
        </div>
        <div class="q-mt-lg" v-if="parseInt(memberLinkData.zml_is_share_integral) == 2">
          <q-input
            ...
            :label="`共享${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit}`"
            :rules="[
              (val) => !!val || '必填项',
              (val) => /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/.test(val) || `请输入正确的${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit}`,
            ]"
          />
        </div>
        <div class="q-mt-xs" v-if="parseInt(memberLinkData.zml_is_share_integral) == 2">
          <q-select
            ...
            option-label="label"
            :options="timeOptions"
            :label="`${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit}日期限制`"
            emit-value
            map-options
          />
        </div>
