        ...
        <div class="col-12 " v-for="(item, key) in packageList" :key="key">
          <q-card  flat bordered @click="checkPackage(item)" class="cursor-pointer ">
            <q-item >
              ...
              <q-item-section side>
            <span class="text-h6 text-deep-orange" v-if="parseInt(item.pp_type) == 2">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ item.pp_selling_price }}</span>
                <span v-else>
                  免费
                </span>
