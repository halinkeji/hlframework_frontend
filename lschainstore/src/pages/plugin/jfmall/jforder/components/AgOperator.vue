<template>
  <div>
    ...
    <q-btn v-if="params.data.jo_state == 2" @click="params.context.updateState(params.data.id, 3)" unelevated round icon="las la-check-circle" color="primary" class="q-mr-md">
      <q-tooltip>
        确认订单
      </q-tooltip>
    </q-btn>
    <q-btn v-if="params.data.jo_state == 3 && params.data.jo_type == 1" round icon="las la-campground" @click="addCourier" unelevated  color="primary" class="q-mr-md">
      <q-tooltip>
        商品出库
      </q-tooltip>
       </q-btn>
    <q-btn v-if="params.data.jo_state == 3 && params.data.jo_type == 2" round icon="las la-boxes" @click="params.context.updateState(params.data.id, 4)" unelevated  color="primary" class="q-mr-md">
      <q-tooltip>
        商品打包
      </q-tooltip>
       </q-btn>
    <!-- <q-btn v-if="params.data.jo_state == 6" unelevated color="info"  class="q-mr-md" @click="seeEvaluation()">查看评价 </q-btn> -->
    <q-btn v-if="params.data.jo_state == 4 && params.data.jo_type == 2" round icon="las la-list-alt" @click="code = true" unelevated  color="negative" class="q-mr-md">
      <q-tooltip>
        自取核销
      </q-tooltip>
       </q-btn>
    <q-btn v-if="params.data.jo_state < 3 && params.data.jo_state != -1" round icon="las la-times-circle" unelevated color="red"   @click="params.context.updateState(params.data.id, 7)" class="q-mr-md" >
    <q-tooltip>
        取消订单
      </q-tooltip>
    </q-btn>
    <q-btn unelevated color="amber-6" round icon="las la-file-alt" @click="openDetailDialog" >
...
export default {
  ...
  methods: {
    ...
    openDetailDialog () {
      this.params.context.openDetailDialog(this.params.data.id)
    }
  }
}
</script>
