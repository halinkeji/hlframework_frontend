<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll bg-white  q-pa-sm">
    <q-item class="q-mb-sm"  :v-ripple="false">
      <q-item-section>
        <q-item-label>启用会员价</q-item-label>
      </q-item-section>
      <q-item-section side >
        <q-toggle
          v-model="jfmallGoodsData.jg_is_member_price"
          :disable="parseInt(goodsId) > 0 ? false : true"
          class="q-ma-none"
          color="green"
          true-value="1"
          false-value="2"
    ...
    <div class="fit" v-if="jfmallGoodsData.jg_is_member_price == 1">
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll" v-if="!formatStatus">
        <div class="row q-gutter-x-sm col-shrink bg-white  q-pb-md">
          <q-select
            ...
            label="会员级别"
          />
          <q-input class="col-12 col-md-2 q-mt-sm" label="请输入会员价格" outlined v-model="memberLevelPrice" dense />
      ...
      <div class="fit" v-else>
        <specification-member-price :goods="goodsId" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  props: ['goodsId', 'formatStatus', 'jfmallGoodsData'],
  components: { SpecificationMemberPrice },
  data () {
    return {
      levelData: '',
      context: null,
  ...
  methods: {
    ...
    setGoodsData () {
      const obj = {
        jg_is_member_price: this.jfmallGoodsData.jg_is_member_price,
    ...
    getMemberLevelList () {
      ...
      this.$store
        .dispatch('memberPrice/getLevelList', obj)
        .then((res) => {
    ...
    changePrice (p) {
      ...
      if (!regu.test(data.price.jmp_price)) {
        this.$q.notify({
          message: '错误',
          icon: 'ion-close-circle-outline',
          timeout: 500,
        ...
        const obj = {
          ...
          level_id: data.id,
          spe_id: 0,
          jmp_price: data.price.jmp_price,
          id: data.price.id
        }
        this.saveData(obj)
      }
    ...
    changeStatus () {
      ...
      const obj = {
        goods_id: this.goodsId,
        level_id: data.id,
        jmp_price: this.memberLevelPrice,
        id: data.price.id
      }
      this.saveData(obj)
    },
    saveData (val) {
      this.$store
        .dispatch('memberPrice/setData', val)
        .then((res) => {
          if (parseInt(res.code) == 200) {
            this.$q.notify({
              message: '成功',
