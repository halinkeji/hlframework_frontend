    ...
    <q-dialog @hide="hideNowPage" v-model="popupActive" persistent>
      ...
      >
        <q-card-section class="row items-center q-pb-none">
          <div class="text-subtitle2">{{ dataId > 0 ? '修改' : '新增' }}标签</div>
        ...
        <q-card-section>
          <q-form ref="dataForm">
            <q-input outlined v-model="dataForm.gt_sort" dense label="排序" :rules="[(val) => /^\d+$/.test(val) || '请输入正确的排序']" />
            <q-input outlined v-model="dataForm.gt_name" dense label="名称" :rules="[(val) => !!val || '请输入名称']" />
            <q-input outlined v-model="dataForm.gt_hot_key" dense label="快捷键" :rules="[(val) => !!val || '请输入快捷键']" />
            <q-select
              outlined
              option-value="value"
              option-label="label"
