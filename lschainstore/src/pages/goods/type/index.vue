      ...
      <div class="col-shrink bg-white q-px-xs q-pb-lg">
        ...
        <div class="q-gutter-x-xs row">
          <q-select
            ...
            clearable
          />
          <q-select class="col-12 col-md-2 q-mt-sm" clearable outlined v-model="modeId" :options="mode" dense label="请选择分类状态" emit-value map-options />
...
export default {
  ...
  methods: {
    ...
    selectedTypeData (row, status) {
      if (status) {
        const dataIndex = _.findIndex(this.currentSelectType, function (o) { return parseInt(o.id) == parseInt(row.id) })
        if (dataIndex > -1) {
