  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md">
      <div class="q-gutter-xs row">
        ...
        <q-btn color="primary" unelevated label="新增" v-if="authorityMeta('directAdd')" @click="chooseInvoice(15)" />
    ...
    <show-wuliu-data ref="showWuliuDataRef" v-if="showWuliuStatus" @dataList="showWuliuStatus = false"></show-wuliu-data>
...
import ShowWuliuData from './components/ShowWuliuData'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      this.$store
        .dispatch('erpDirectOut/getDataList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        const obj = {
          direct_allocation_order_id: val[0].id
        }
        this.$store
          .dispatch('erpDirectOut/createOutDepotOrder', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              this.$q.notify({
                color: 'green',
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addErpDirectOut', query: { id: res.data.id } })
              })
            } else {
              this.$q.notify({
                message: '生成失败',
    ...
    countExport () {
      ...
      this.$store
        .dispatch('exports/getOutExport', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    showLogData (val) {
      ...
      this.$nextTick(() => {
        this.$refs.showDataRef.init(val, 16)
      })
    },
    ...
    showWuliuData (val) {
      ...
      this.$nextTick(() => {
        this.$refs.showWuliuDataRef.init(val)
      })
    },
    authorityMeta (key) {
      if (this.$route.meta) {
