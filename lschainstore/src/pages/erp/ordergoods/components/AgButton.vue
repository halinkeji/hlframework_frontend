<template>
  <div v-if="!params.data.show_status">
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 1 && authorityMeta('confirm')"
      :to="{ name: 'addErpOrderGoods', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 2 && authorityMeta('cancel')"
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 2 && authorityMeta('audit')"
      :to="{ name: 'addErpOrderGoods', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 5 && authorityMeta('hangup')"
      :to="{ name: 'addErpOrderGoods', query: { id: params.data.id } }"
    >
  ...
  methods: {
    ...
    saveDataOperate (text, cepStatus) {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            epd_store_order_goods_id: this.params.data.id,
            cepStatus
          }
          this.$store
