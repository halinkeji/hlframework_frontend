        ...
        <q-card-section class="q-pt-lg">
          ...
          <div class=" col-12  q-mt-sm q-mr-xs ">
            <q-input
              ...
              lazy-rules
              :rules="[
                val => !!val || /^\d+$/.test(val) || '库存预警数量为非负整数'
              ]"
            />
          </div>
          <div class="col-12  q-mt-sm q-mr-xs">
            <q-input
              outlined
              v-model="numberForm.gn_maturity_time"
              label="开始时间"
              dense
              square
              readonly
            >
              <template v-slot:prepend>
                <q-icon name="event" class="cursor-pointer">
                  <q-popup-proxy
                    @before-show="numberForm.gn_maturity_time"
  ...
  methods: {
    ...
    submitData () {
      this.$refs.numberDataRef.validate().then(success => {
        if (success) {
          const obj = {
            gn_maturity_time: this.$q_date.formatDate(
              this.numberForm.gn_maturity_time,
              'X'
            ),
    ...
    selectWarning (val) {
      ...
      this.numberForm.gn_maturity_time = this.$q_date.formatDate(
        val.wr_time * 1000,
        'YYYY-MM-DD  HH:mm:ss'
      )
    },
