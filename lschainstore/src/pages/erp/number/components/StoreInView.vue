<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <!-- <div class="col-shrink bg-white q-px-xs q-pb-md">
      <q-btn color="primary" class=" q-mt-sm q-mb-md " unelevated @click="exportCsv" label="导出Csv文件" :disable="storeReturnData.length == 0" />
    </div> -->
    ...
    <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-sm">
      ...
      <q-btn color="primary" unelevated @click="exportCsv" label="导出Csv文件" :disable="storeReturnData.length == 0" />
    </div>
...
<script>
import AgInput from 'pages/erp/return/store/components/AgInput'
export default {
  name: 'StoreReturnView',
  ...
  methods: {
    ...
    getSaleData () {
      ...
      this.$store.dispatch('erpNumber/getStoreReturn', obj).then((res) => {
        if (res.code == 200) {
          this.page.totalCount = res.data ? res.data.page.totalCount : 0
          this.storeReturnData = res.data ? res.data.list : []

          if (this.storeReturnData && this.storeReturnData.length > 0) {
            this.setTopTabel(this.storeReturnData)
          }
        }
      })
    },
