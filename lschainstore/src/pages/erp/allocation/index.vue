    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        <div class="q-gutter-sm row">
          ...
          <q-btn color="primary" unelevated label="新增" v-if="authorityMeta('add')" @click="chooseInvoice" />
...
export default {
  ...
  methods: {
    ...
    chooseInvoice () {
      ...
      this.$nextTick(() => {
        this.$refs.chooseInvoiceList.getInvoice()
      })
    },
    // 选择返回的数据
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        ...
        const obj = {
          cep_goods_order_id: patchOrderData.id
        }
        this.$store
          .dispatch('erpAllocation/createAllocationOrderData', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addErpAllocation', query: { id: res.data.id } })
              })
            }
          })
          .catch((error) => {
    ...
    getList () {
      ...
      this.$store
        .dispatch('erpAllocation/getData', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    showLogData (val) {
      ...
      this.$nextTick(() => {
        this.$refs.showDataRef.init(val, 5)
      })
    },
    authorityMeta (key) {
      if (this.$route.meta) {
