      ...
      <div class="col-shrink bg-white q-px-xs q-py-sm ">
        <q-form ref="addDataRef" class=" row q-gutter-md">
          <q-select
            ...
            v-model="turnData.cet_in_depot_id"
            :options="depotData"
            label="请选择调入仓库"
            option-value="id"
            ...
            emit-value
            map-options
            :disable="turnData.cet_in_depot_id ? true : false"
          />
          <q-select
            outlined
            dense
            class="col-12 col-xs-6 col-sm-2 q-mr-md"
            v-model="turnData.cet_out_depot_id"
            :options="depotData"
            label="请选择调出仓库"
            option-value="id"
            option-label="cd_name"
            emit-value
            map-options
            :disable="turnData.cet_out_depot_id ? true : false"
          />
        </q-form>
        <div class="q-gutter-x-xs row q-pb-lg">
          <div class="col-12 col-md-2 q-pa-xs">
            <q-input dense disable v-model="turnData.cet_order_number" label="调拨单据号" />
          </div>
          <div class="col-12 col-md-2 q-pa-xs">
            <q-input disable dense v-model="turnData.cet_total_amount" label="调拨金额" />
          </div>
          <div class="col-12 col-md-2 q-pa-xs">
            <q-input disable dense v-model="turnData.cet_total_quantity" label="调拨数量" />
          </div>
          <div class="col-12 col-md-2 q-pa-xs">
            <q-input disable dense v-model="turnData.number" label="调拨数量" />
          </div>
          <div class="col-12 col-md-2 q-pa-xs">
            <q-input dense v-model="turnData.cet_note" label="调拨备注" />
          </div>
        </div>
      </div>
