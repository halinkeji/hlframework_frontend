    ...
    <div class="col-shrink bg-white q-px-xs q-pb-md">
      <div class="row q-gutter-xs ">
        ...
        <div class="col">
          ...
          <q-btn color="primary" unelevated label="新增" v-if="authorityMeta('outAdd')" @click="chooseInvoice(5)" />
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      this.$store
        .dispatch('erpOut/getDataList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        const obj = {
          allocation_order_id: val[0].id
        }
        this.$store
          .dispatch('erpOut/createOutDepotOrder', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addErpOutGoods', query: { id: res.data.id } })
              })
            } else {
              this.$q.notify({
                message: '生成失败',
    ...
    showLogData (val) {
      ...
      this.$nextTick(() => {
        this.$refs.showDataRef.init(val, 6)
      })
    },
