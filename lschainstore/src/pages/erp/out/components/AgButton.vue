<template>
  <div v-if="!params.data.show_status">
    <q-btn
      ...
      :to="{ name: 'addErpOutGoods', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      class="q-ma-xs"
      color="positive"
      v-if="params.data.cep_status >= 3 && params.data.allocationOrderData && params.data.allocationOrderData.cep_goods_order_id && authorityMeta('wuliu')"
      unelevated
      icon=" local_shipping"
      round
      size="sm"
  ...
  methods: {
    ...
    saveDataOperate (text, cepStatus) {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            out_depot_order_id: this.params.data.id,
            cepStatus
          }
          this.$store
    ...
    showWuliu () {
      this.params.context.showWuliuData(this.params.data.allocationOrderData.cep_goods_order_id, this.params.data.id)
    },
    authorityMeta (key) {
      if (this.$route.meta) {
