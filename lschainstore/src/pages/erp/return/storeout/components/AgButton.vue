<template>
  <div v-if="!params.data.show_status">
    <q-btn
      ...
      :to="{ name: 'addStoreReturnout', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      class="q-ma-xs"
      color="positive"
      v-if="params.data.cep_status >= 3 && params.data.storeReturnOrderData && params.data.storeReturnOrderData.id && authorityMeta('wuliu')"
      unelevated
      icon=" local_shipping"
      round
      size="sm"
...
export default {
  ...
  methods: {
    ...
    showWuliu () {
      this.params.context.showWuliuData(this.params.data.storeReturnOrderData.id, this.params.data.id)
    },
    authorityMeta (key) {
      if (this.$route.meta) {
