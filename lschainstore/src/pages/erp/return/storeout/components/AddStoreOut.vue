    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md ">
        <div class="q-gutter-xs row ">
          <q-btn color="grey" unelevated :to="{ name: 'erpOut', params: { tab: '4' } }" label="返回" />

          <q-btn
            v-if="storeOutData.cep_if_pack == '1' && storeReturnOutBuffer.length > 1"
            color="secondary"
            unelevated
            label="装箱信息"
            :disable="storeReturnOutBuffer.length > 1 ? false : true"
          ...
          <q-btn
            ...
            @click="clickOutDepot"
            unelevated
            v-if="storeOutData.cep_status == 2"
            label="直接出库"
          />
          <q-btn color="primary" unelevated @click="exportCsv" label="导出Csv文件" :disable="storeReturnOutBuffer.length == 0 && storeInId < 1" />

          <q-btn color="info" unelevated @click="erpPrint" label="打印" v-if="storeReturnOutBuffer.length > 0 && storeInId > 0" />
          <q-space />
      ...
      <div class="col-shrink bg-white q-pa-md q-pb-md row">
        ...
        <div class="col-2" v-if="storeReturnOrderData && storeReturnOrderData.id">
          <q-item>
            <q-item-section>
              <q-item-label class="text-subtitle2 text-weight-bold">{{ storeReturnOrderData.cep_contract_number }}</q-item-label>
              <q-item-label caption lines="2">退货单据号</q-item-label>
            </q-item-section>
          </q-item>
        </div>

        <div class="col" v-if="storeReturnOrderData && storeReturnOrderData.id">
          <q-item>
            <q-item-section>
              <q-item-label class="text-subtitle2 text-weight-bold">{{ storeReturnOrderData.cep_depot_name }}</q-item-label>
        ...
        <div class="col" v-if="storeReturnOrderData && storeReturnOrderData.id">
          <q-item>
            <q-item-section>
              <q-item-label class="text-subtitle2 text-weight-bold">{{ storeReturnOrderData.cep_total_purchase_amount }}</q-item-label>
        ...
        <div class="col" v-if="storeReturnOrderData && storeReturnOrderData.id">
          <q-item>
            <q-item-section>
              <q-item-label class="text-subtitle2 text-weight-bold">{{ storeReturnOrderData.cep_total_purchase_num }}</q-item-label>
...
export default {
  ...
  methods: {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            ...
            this.storeReturnOutBuffer = res.data.storeReturnOutBuffer.length > 0 ? res.data.storeReturnOutBuffer : []
            this.storeReturnOrderData = res.data.storeReturnOrderData ? res.data.storeReturnOrderData : {}
    ...
    saveDataOperate (text, cepStatus) {
      if (this.storeReturnOutBuffer.length <= 0) {
        this.$q.notify({
          message: '注意',
          caption: '请选择出库商品!',
          color: 'red',
      ...
      this.$q
        ...
        .onOk(() => {
          const obj = {
            ...
            cepStatus,
            storeReturnOutBuffer: this.storeReturnOutBuffer
          }
          this.$store
            .dispatch('erpStoreout/changeStoreOutDepotOrder', obj)
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
    ...
    changeNumber (p) {
      ...
      this.$store
        .dispatch('erpStoreout/editBuffer', row)
        .then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
    ...
    getDataColumnSum () {
      ...
      const tableData = this.storeReturnOutBuffer
      let inTotalNum = 0
      let number = 0
      let inTotalAmount = 0
