...
export default {
  ...
  methods: {
    ...
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        const obj = {
          store_return_out_id: val[0].id
        }
        this.$store
          .dispatch('erpStorein/createStoreReturnInDepotOrder', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addStoreReturnin', query: { id: res.data.id } })
              })
            } else {
              this.$q.notify({
                message: '生成失败',
    ...
    getList () {
      ...
      this.$store
        .dispatch('erpStorein/getDataList', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    countExport () {
      ...
      this.$store
        .dispatch('exports/getStoreReturnInExport', obj)
        .then((res) => {
          if (res.code == 200) {
