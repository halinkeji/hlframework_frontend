  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-py-sm">
      <div class="q-gutter-xs row ">
        ...
        <q-btn color="primary" @click="chooseInvoice(1)" unelevated v-if="authorityMeta('transferAdd')" label="新增" />
...
export default {
  ...
  methods: {
    ...
    receivingAdministrator (val) {
      if (val && val.length > 0) {
        const obj = {
          transfer_order_id: val[0].id
        }
        this.$store
          .dispatch('erpTransferOut/createTransferOutDepotOrder', obj)
          .then((res) => {
            if (res.code == 200 && res.data) {
              ...
              this.$nextTick(() => {
                this.$router.push({ name: 'addErpTransferOut', query: { id: res.data.id } })
              })
            } else {
              this.$q.notify({
                message: '生成失败',
    ...
    getStockList () {
      ...
      this.$store
        .dispatch('erpTransferOut/getDataList', obj)
        .then((res) => {
          if (res.code == 200) {
