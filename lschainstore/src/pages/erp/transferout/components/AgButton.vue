<template>
  ...
  <div>
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 1 && authorityMeta('transferConfirm')"
      :to="{ name: 'addErpTransferOut', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 2 && authorityMeta('transferCancel')"
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 2 && authorityMeta('transferAudit')"
      :to="{ name: 'addErpTransferOut', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.id > 0 && params.data.cep_status == 5 && authorityMeta('transferHangup')"
      :to="{ name: 'addErpTransferOut', query: { id: params.data.id } }"
    >
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.cep_status == 1 && authorityMeta('transferClose')"
    ...
    <q-btn
      ...
      round
      size="sm"
      v-if="params.data.cep_status == 3 && authorityMeta('transferRevoke')"
      :to="{ name: 'addErpTransferOut', query: { id: params.data.id } } "
    >
      <q-tooltip anchor="top middle" self="bottom middle" :offset="[10, 10]">取消审核</q-tooltip>
    </q-btn>

    <q-btn class="q-ma-xs" color="info" v-if="authorityMeta('transferLog')" unelevated icon="assignment" round size="sm" @click="showLog">
  ...
  methods: {
    ...
    saveDataOperate (text, cepStatus) {
      this.$q
        ...
        .onOk(() => {
          const obj = {
            transfer_out_orde_id: this.params.data.id,
            cepStatus
          }
          this.$store
