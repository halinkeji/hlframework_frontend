...
export default {
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '结算对账单开始时间',
          field: 'pdw_withdraw_start_time',
          valueGetter: (p) => {
            return p.data.pdw_withdraw_start_time > 0 ? this.$q_date.formatDate(p.data.pdw_withdraw_start_time * 1000, 'YYYY-MM-DD HH:mm:ss') : ''
          }
        },

        {
          headerName: '结算对账单结束时间',
          field: 'pdw_withdraw_end_time',
          valueGetter: (p) => {
            return p.data.pdw_withdraw_end_time > 0 ? this.$q_date.formatDate(p.data.pdw_withdraw_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') : ''
          }
        },
  ...
  computed: {},
  created () {
    if (this.$q.localStorage.getItem('lschainstore_store_local')) {
      this.storeInfo = this.$q.localStorage.getItem('lschainstore_store_local').storeInfo
    }
