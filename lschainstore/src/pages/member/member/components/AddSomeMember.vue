      ...
      <q-form ref="memberDataRef" class="q-gutter-md q-mt-md">
        <div class="row q-pl-xs">
          <div class="col-12 col-md-6  q-pr-lg ">
            ...
            <q-input
              :label="`会员初始${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit}`"
              outlined
              v-model="mem_memberMoneyCount"
              class="q-mt-xs"
              dense
              lazy-rules
              :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || `会员初始${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit}为非负两位以内小数`]"
            />
          </div>
          <div class="col-12 col-md-6  q-pr-lg ">
            <q-select
            ...
            <q-input
              :label="`会员初始${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit}`"
              outlined
              v-model="mem_memberPointsCount"
              class="q-mt-xs"
              dense
              lazy-rules
              :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || `会员初始${$q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit}为非负两位以内小数`]"
            />
          </div>
        </div>
      </q-form>
