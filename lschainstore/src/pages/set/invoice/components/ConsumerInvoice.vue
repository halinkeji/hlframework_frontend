<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll" >
        ...
        <div class="row q-col-gutter-xs items-center">
          ...
          <div class="col-12 col-md-2">
            ...
            <q-select
              ...
              v-model="searchFrom.reward_user_id"
              :options="userInfo"
              dense
              emit-value
              map-options
         ...
         <div class="col-12 col-md-2">
          ...
          <q-btn @click="consumeExport()" v-if="searchFrom.member_id == 0" color="red" class="q-px-md q-mx-xs" unelevated label="导出" />
          <q-btn
            @click="consumeDetailExport()"
            v-if="searchFrom.member_id == 0"
            color="positive"
            class="q-px-md q-mx-xs"
            unelevated
            label="导出详情"
          />
        </div>
    ...
    <return-all-goods ref="returnAllGoods" v-if="returnAllGoodsStatus" @getMethods="getDatalist"></return-all-goods>
    <revoke-order ref="revokeOrderRef" v-if="revokeOrderStatus" @getMethods="getDatalist"></revoke-order>
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '导购',
          field: 'reward.pru_name',
          filter: false,
          valueGetter: (p) => {
            return p.data.reward
              ? p.data.reward.pru_name
              : ''
          },
          width: this.nowpx(0.1) + 'px'
        },
...

        {
          ...
          comparator: (a, b) => a - b,
          valueGetter: (p) => {
            return this.formatAmount(p.data.sc_handleMoneyCount)
          }
        },
        {
          headerName: '实收金额',
          ...
          comparator: (a, b) => a - b,
          valueGetter: (p) => {
            return this.formatAmount(p.data.sc_realMoneyCount)
          }
        },
...

        {
          ...
          comparator: (a, b) => a - b,
          valueGetter: (p) => {
            return this.formatAmount(p.data.sc_rulePaymentCount)
          }
        },
        {
          headerName: '抹零金额',
          ...
          comparator: (a, b) => a - b,
          valueGetter: (p) => {
            return this.formatAmount(p.data.sc_rounding_amount)
          }
        },
        {
          headerName: '卡券优惠',
          ...
          comparator: (a, b) => a - b,
          valueGetter: (p) => {
            return this.formatAmount(p.data.sc_couponPaymentCount)
          }
        },
  ...
  methods: {
            ...
            valueGetter: (p) => {
              if (!p.data.show_status) {
                if (item.channel == 'local') {
                  ...
                  const currentIndex = _.findIndex(extend_pay, function (o) {
                  ...
                  const currentIndex = _.findIndex(hl_pay, function (o) {
                    return o.key == item.key
                  })
    ...
    getUser () {
      this.$store.dispatch('rewardPosition/getUserLists').then((res) => {
          ...
          for (var j in i) {
            ...
            if (!i[j]) {
              data.forEach((value, key) => {
                if (_.findIndex(that.paymentChannel, function (o) { return j == o.key }) > -1) {
    ...
    consumeExport () {
      ...
      this.$store
        .dispatch('exports/getConsumeExport', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    consumeDetailExport () {
      ...
      this.$store
        .dispatch('exports/getConsumeDetailExport', obj)
        .then((res) => {
          if (res.code == 200) {
            window.location.href = res.data
          } else {
