...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('lschainstore_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('lschainstore_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('lschainstore_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('lschainstore_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'lschainstore') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('lschainstore_access_token', Cookies.get('access_token'))
  LocalStorage.set('lschainstore_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('lschainstore_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('lschainstore_token_type', Cookies.get('token_type'))
  LocalStorage.set('lschainstore_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'lschainstore_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('lschainstore_user_info', Cookies.get('user_info'))
  LocalStorage.set('lschainstore_account_info', Cookies.get('account_info'))
