    ...
    <q-card flat bordered class="bg-white" style="width: 900px; max-width: 80vw">
      ...
      <q-banner inline-actions rounded class="bg-grey-1 text-black">
        <template v-slot:action>
          <q-btn unelevated outline v-if="type == 'purchase'|| type == 'orderGoods'||type == 'order'" color="primary" label="下载导入模板" icon="get_app" href="https://cdn.halin.net/download/xlsx/零售版-采购订货补货导入模板 .xlsx" target="_blank" />
          <q-btn unelevated outline v-if="type == 'check'|| type == 'storeReturn'||type == 'transfer'|| type == 'purchaseReturn'" color="primary" label="下载导入模板" icon="get_app" href="https://cdn.halin.net/download/xlsx/零售版-盘点模板导入模板.xlsx" target="_blank" />
        </template>
      </q-banner>
    </q-card>
  </q-dialog>
