    ...
    >
      ...
      <div class="row col-9 fit">
        ...
        <div class="col-12 col-md-9" v-else>
          ...
          >
            <q-page-container>
              <q-header reveal class="bg-white row">
                <div class="col-4 q-px-xs">
                  <q-input outlined v-model="search_data.name" label="文件名称" dense>
                ...
                <div class="col-5 q-px-xs">
                  <q-input label="选择日期" v-model="search_time" dense outlined>
                    <template v-slot:append>
                      <q-icon name="event" class="cursor-pointer">
                        <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
                          <q-date v-model="search_time" @input="changeDate">
                            <div class="row items-center justify-end">
                              <q-btn v-close-popup label="确定" color="primary" flat @click="getImageList" />
                            </div>
                          </q-date>
                        </q-popup-proxy>
                      </q-icon>
              ...
              <q-page>
                <div class="row" v-if="list.length > 0">
                  <div class="col-3 col-md-2 q-ma-sm" v-for="(img, imgKey) in list">
                    <q-img
                      :src="$q.localStorage.getItem('lschainstore_system_local').attach_url + img.path"
                      style="width: 100%"
                      :ratio="1"
                      native-context-menu
      ...
      <div class="justify-end q-mr-md row q-gutter-md q-py-sm">
        <q-btn @click="handleCancelAll" unelevated color="grey" class="text-dark q-px-lg" label="取消已选">
          <q-badge v-if="parseInt(selectedImgNum) > 0" color="red" floating>{{ selectedImgNum }}</q-badge>
        </q-btn>
        <q-btn unelevated color="red" @click="updataImageData('del')" class="q-px-lg" label="确定删除">
          <q-badge v-if="parseInt(selectedImgNum) > 0" color="red" floating>{{ selectedImgNum }}</q-badge>
        </q-btn>
        <q-btn unelevated color="amber-7" @click="openChangeSearchData" class="q-px-lg" label="更换分组">
          <q-badge v-if="parseInt(selectedImgNum) > 0" color="red" floating>{{ selectedImgNum }}</q-badge>
        </q-btn>
