          ...
          <div class="col-12 col-md-7 q-pr-xs">
            <q-card flat square class="full-width q-mb-xs" v-if="memberInfo.id">
              <q-item class="text-center">
                ...
                <q-item-section>
                  <q-item-label caption lines="1">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}</q-item-label>
                  <q-item-label class="text-h5">{{ formatAmount(memberInfo.mem_memberMoneyCount) }}  </q-item-label>
                </q-item-section>
                <q-item-section>
                  <q-item-label caption lines="1">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit }}</q-item-label>
          ...
          <div class="col-12 col-md-5">
            <q-card flat square class="full-width text-h6">
              <div class="row justify-between q-py-xs">
                ...
                <div class="col-4 text-right q-pr-md">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_handleMoneyCount }}</div>
              </div>

              <div class="row justify-between q-py-xs" v-if="sc_rounding_amount">
                <div class="col-4 q-pl-md">抹零</div>
                <div class="col-4 text-red text-right q-pr-md">-{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_rounding_amount }}</div>
              </div>
              <div class="row justify-between q-py-xs" v-if="sc_change_amount">
                <div class="col-4 q-pl-md">现金找零</div>
                <div class="col-4 text-red text-right q-pr-md">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_change_amount }}</div>
              </div>
              <div class="row justify-between q-py-xs" v-if="ruleGiveIntegral">
                ...
                <div class="col-4 text-primary text-right q-pr-md">
                  {{ ruleGiveIntegral }} <span class="text-body1">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit }}</span>
                </div>
              </div>

              <div class="row justify-between q-py-xs">
                <div class="col-4 q-pl-md">实付金额</div>
                <div class="col-4 text-right q-pr-md">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ sc_realMoneyCount }}</div>
              </div>
            </q-card>
              ...
              <div class="row justify-between">
                ...
                >
                  <q-input dense ref="payMoneyRef" outlined v-model="v.value" @change="confirmSelectPay(nowPaymentMethod, v)" :label="v.name" :readonly="v.key == 'cashPay'">
                    <template v-slot:append>
                      <span class="text-body1 text-dark">{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneyUnit }}</span>
                    </template>
                  </q-input>

                  <div class="text-red text-body2" v-if="v.key == 'rechargePay'">
                    当前{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_balanceUnit }} : {{ memberInfo.mem_memberMoneyCount }} 赠送{{
                  ...
                  <div class="text-red text-body2" v-if="v.key == 'pointPay'">
                    总积分{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit }} ： {{ memberInfo.mem_memberPointsCount }} <br/>
                    本单可用{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_integralUnit }} ：{{ useDiscountPoint }}
                  </div>
                  <div class="q-mt-sm text-body2" v-if="v.key == 'cashPay'">
                    <div class="text-red text-body2">
                      实收:{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ formatAmount(sc_real_payment_cash) }}，
                      <span v-if="formatAmount(formatAmount(sc_real_payment_cash) - formatAmount(v.value)) > 0">
                        找零:{{ $q.localStorage.getItem('lschainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ formatAmount(formatAmount(sc_real_payment_cash) - formatAmount(v.value)) }}
                      </span>
                    </div>
                  </div>
                </div>
