...
export function ONOPEN (state, payload) {
  ...
  // const accessToken = LocalStorage.getItem('sspacechainstore_access_token')
  if (systemLocal && systemLocal.websocketClient) {
    state.ws = new WebSocket(systemLocal.websocketClient)
    //  state.ws = new WebSocket("ws://192.168.3.232:10000")
    state.ws.onopen = function (res) {
      console.log('连接成功')
      // const message = {
      //   type: 'login',
      //   data: {
      //     'Electron-Token' : "ssssssss"
      //   }
      // }
      // const message = "01 05 00 03 FF 00 7C 3A"
