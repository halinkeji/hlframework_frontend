      ...
      <div class="col-12 col-md-4">
        <q-input outlined bottom-slots v-model="addressText" label="搜索地址" clearable type="text" name="place"  ref="placeAddressText" dense>
          ...
          <template v-slot:hint>
            {{ addressInfo.address }}-经度:{{ addressInfo.ad_info.location.lng }},纬度:{{ addressInfo.ad_info.location.lat }}
          </template>
        </q-input>
      </div>
...
<script>
import { TMap, getMapAddress, getSuggestion } from './qqmap'
...
export default {
  ...
  created: function () {},
  mounted () {
    if (LocalStorage.getItem('sspacechainstore_system_local')) {
      const mapInfo = LocalStorage.getItem('sspacechainstore_system_local').map
    ...
    getMapAddressPage (key, lat, lng) {
      getMapAddress(key, lat, lng)
        .then((res) => {
          ...
          if (resultData.formatted_addresses && resultData.formatted_addresses.recommend) {
            obj.address = resultData.formatted_addresses.recommend
          }

          if (resultData && resultData.ad_info) {
            obj.province = resultData.ad_info.province
            obj.city = resultData.ad_info.city
            obj.district = resultData.ad_info.district
            const adcode = resultData.ad_info.adcode
            if (adcode) {
              obj.province_code = adcode.substr(0, 2) + '0000'
              obj.city_code = adcode.substr(0, 4) + '00'
              obj.district_code = resultData.ad_info.adcode
            }
          }

          if (resultData && resultData.location) {
            obj.lng = resultData.location.lng
            obj.lat = resultData.location.lat
          }

          this.addressInfo = resultData

          this.$emit('getAddressInfo', obj)
        })
        .catch((err) => {
