<template>
  <q-card flat>
    <q-input outlined  dense readonly v-model="inputData" :placeholder="textName">
      <q-menu>
        <q-list :style="{ width: list_width + 'px' }">
          <div v-for="item in dataList">
            <item-menu @getItem="getNowItem" v-if="item.child.length > 0" :menu="item"></item-menu>

            <q-item clickable v-else @click="getNowItem(item)" v-close-popup="item.child.length > 0 ? false : true">
              <q-item-section side>
                <q-badge color="primary" v-if="parseInt(item.gt_goods_type) == 1">
                  实
                </q-badge>
                <q-badge color="positive" v-if="parseInt(item.gt_goods_type) == 2">
                  服
                </q-badge>
                <q-badge color="negative" v-if="parseInt(item.gt_goods_type) == 3">
                  套
                </q-badge>
              </q-item-section>
              <q-item-section>
