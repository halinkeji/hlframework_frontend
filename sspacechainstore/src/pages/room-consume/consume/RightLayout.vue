          ...
          <template v-slot:item="props">
            <div class="q-pa-xs col-xs-12 col-sm-6 col-md-2">
              <q-card flat bordered @click="triggerOperate(props.row)">
                ...
                <q-img
                  :src="props.row.coverImage && props.row.coverImage.id ? $q.localStorage.getItem('sspacechainstore_system_local').attach_url + props.row.coverImage.path : ''"
                  :ratio="1"
                  class="full-width"
                >
                ...
                <q-card-section class="q-pa-xs" style="font-size: 10px">
                  ...
                  <div class="text-deep-orange" v-else>
                    {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneySymbol }} {{ props.row.goo_price }}
                  </div>
...
export default {
  name: 'RightLayout',
  props: ['member_id', 'consumehangup_id', 'currentRoomId'],
  components: {
    ...
    addingGoodsBut (val, type, data) {
      if (parseInt(type) < 3 && val) {
        var goodsData = {
          ...
          cgb_consumeNumber: val.cgb_consumeNumber,
