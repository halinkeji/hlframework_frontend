...
export default {
  ...
  methods: {
    setData (status) {
      const messageOne = status == 2 ? '进行接单？' : '已完成预订?'
      const message = status == 4 ? '撤回预订？' : messageOne
      this.$q
        .dialog({
          title: '提示',
          message: `确定${message}`,
        ...
        .onOk(() => {
          ...
          this.$store
            ...
            .then((res) => {
              if (res.code == 200) {
                const captionOne = this.detailData.prd_status == 3 ? '已完成预订' : '接单成功'
                const caption = this.detailData.prd_status == 4 ? '已撤销预订' : captionOne
                this.$q.notify({
                  message: '提示',
                  caption: `${caption}`,
