        ...
        <q-btn-dropdown color="red" label="批量操作" v-if="authorityMeta('batch')" :menu-offset="[10, 6]" unelevated>
          <q-list class="text-center">
            ...
            <q-item clickable @click="showPopup" v-if="authorityMeta('editAll')" :disable="dataListSelections.length > 0 ? false : true">
              <q-item-section>
                <q-item-label
                  >批量操作会员{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit
      ...
      >
        <q-form ref="memberDataRef" class="q-gutter-md q-mt-md">
          <q-card-section class="row items-center q-pb-none">
            <div class="text-h6">
              批量操作会员{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}与{{
          ...
          <q-card-section class="q-pt-sm">
            <div class="col-12">
              <q-input
                ...
                :label="`${$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit}`"
                outlined
                ...
                :rules="[
                  (val) =>
                    /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                    `${$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit}为非负两位以内小数`,
                ]"
              />
            </div>
            <div class="col-12">
              <q-input
                :disable="parseInt(batchOperate) == 3"
                :label="`${$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit}`"
                outlined
                ...
                :rules="[
                  (val) =>
                    /^(\d+|\d+\.\d{1,2})$/.test(val) ||
                    `${$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit}为非负两位以内小数`,
                ]"
              />
            </div>
...
export default {
  ...
  mounted () {
    if (
      this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig &&
      this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_regBindFormField &&
      this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_regBindFormField > 0
    ) {
      this.getFieldData(this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_regBindFormField)
    } else {
      this.restColumnData()
    }
  },
  methods: {
    ...
    restColumnData (column) {
      const columnData = [
        ...
        {
          headerName: '总' + this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit,
        ...
        {
          headerName: '总' + this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit,
