      ...
      <q-card-section class="col-shrink full-height">
        <q-scroll-area class="fit">
          <q-form ref="roomDataRef" class="row q-col-gutter-sm">
            <div class="col-6">
              <q-input outlined dense v-model="roomData.qrt_name" label="名称" lazy-rules :rules="[(val) => !!val || '请输入名称']" />
            </div>
            ...
            <div class="col-12">
              <q-item class="bg-grey-2">
                <q-item-section>
                  ...
                  <q-item-label caption>当状态为【使用中】、【清洁中】、【暂停中】时不可设置修改</q-item-label>
                </q-item-section>
                <q-item-section side top v-if="!(roomData.qrt_status == 2 || roomData.qrt_status == 3)">
                  <q-option-group
            ...
            <div class="col-6" v-if="parseInt(roomData.qrt_is_min_consume) > 0">
              <q-input
            ...
            <div class="col-12 row">
              <div class="col-6">
                <hl-upload
                  :multiple="false"
                  :currentAttach="roomData.coverImage"
                  @input="
                    (val) => {
                      $nextTick(() => {
                        roomData.qrt_cover_image = val;
                      });
                    }
                  "
              ...
              <div class="col-6">
                <hl-upload
                  :multiple="true"
                  :maxTotal="5"
                  :currentAttach="roomData.loopImage"
                  @input="
                    (val) => {
                      $nextTick(() => {
                        roomData.qrt_loop_image = val;
                      });
                    }
                  "
                  currentDescription="建议尺寸比例1:1"
            ...
            <div class="col-12">
              <hl-editor
                @change="
                  (v) => {
                    roomData.qrt_description = v;
                  }
                "
                :content="roomData.qrt_description"
                titleText="描述"
...
export default {
  ...
  methods: {
    ...
    getItem () {
      this.$store
        .dispatch('roomManage/getRoomItem', this.roomId)
        .then((res) => {
          if (res.code == 200 && res.data && res.data.id) {
            this.roomData = res.data

            if (res.data && res.data.bindTagData) {
              this.bindTagData = res.data.bindTagData.map((item) => {
                return item.rtb_tag_id
              })
            }
          } else {
            this.$q.notify({
    ...
    setData () {
      this.$refs.roomDataRef.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch('roomManage/saveRoomData', params)
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
                  caption: `${this.roomData.id > 0 ? '修改' : '新增'}` + '桌号成功',
                  icon: 'ion-checkmark-circle-outline',
                  timeout: 500,
                  position: 'top-right',
                  color: 'green'
