<template>
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      ...
      <div class="col-shrink bg-white full-height q-mt-xs" v-if="activeTab == 'count'">
        <deduct-count ref="deductCountRef" />
      </div>
      <div class="col-shrink bg-white full-height q-mt-xs" v-if="activeTab == 'timing'">
        <time-card ref="timeCardRef" />
        <!-- <deduct-duration ref="deductDurationRef" /> -->
      </div>
      <div class="col-shrink bg-white full-height q-mt-xs" v-if="activeTab == 'growRecord'">
        <grow-record ref="memberGrowRecord" />
      </div>
  ...
  methods: {
    changeTab () {
      ...
      if (this.activeTab == 'timing') {
        this.$nextTick(() => {
          this.$refs.timeCardRef.init(this.memberId)
        })
      }
      if (this.activeTab == 'growRecord') {
        this.$nextTick(() => {
          this.$refs.memberGrowRecord.init(this.memberId)
        })
      }
    },
    // 读卡传值
        ...
        if (val.id) {
          ...
          if (this.userData.mem_memberBirthdayYear) {
            this.userData.birthday =
              this.userData.mem_memberBirthdayYear + '-' + this.userData.mem_memberBirthdayMonth + '-' + this.userData.mem_memberBirthdayDay
          }
