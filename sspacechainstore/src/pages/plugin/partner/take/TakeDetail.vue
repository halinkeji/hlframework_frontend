<template>
  <q-dialog @hide="hideNowPage" v-model="showDialog" persistent>
    ...
    >
      ...
      <div class="row  justify-center" v-if="parseInt(memberData.ptr_status) == 4">
        ...
        <div class="col-4">
          <div class="row">
            <q-avatar color="primary" size="sm" text-color="white" v-if="parseInt(memberData.ptr_status) == 1">2</q-avatar>
          ...
          <div class="q-pl-lg" v-if="parseInt(memberData.ptr_status) >= 2">
            <span class="q-pl-sm">{{ $q_date.formatDate(memberData.ptr_check_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}</span>
          </div>
        </div>
        <div class="q-pr-lg">
          <div class="row">
          ...
          <div class="q-pl-lg">
            <span class="q-pl-sm">{{ $q_date.formatDate(memberData.ptr_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}</span>
          </div>
        </div>
      </div>
      <div class="row  justify-center" v-else>
        ...
        <div class="col-3">
          <div class="row">
            ...
            <div class="q-px-sm self-center">商家审核</div>
            <div class="col-8 self-center">
              <q-linear-progress color="primary" :value="progress" size="xs" v-if="parseInt(memberData.ptr_status) >= 2" />
        ...
        <div class="col-3">
          <div class="row">
            <q-icon name="check_circle_outline" color="primary" size="24px" v-if="parseInt(memberData.ptr_status) == 3"></q-icon>
            <q-avatar color="grey" size="sm" text-color="white" v-else>3</q-avatar>
            <div class="q-px-sm self-center">商家打款</div>
            <div class="col-8 self-center">
              <q-linear-progress color="primary" size="xs" :value="progress" v-if="parseInt(memberData.ptr_status) == 3" />
              <q-linear-progress color="primary" size="xs" v-else />
            </div>
          </div>
          <div class="q-pl-lg" v-if="parseInt(memberData.ptr_status) >= 3">
            <span class="q-pl-sm">{{ $q_date.formatDate(memberData.ptr_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}</span>
          </div>
        </div>
        <div class="q-pr-lg">
        ...
        <div class="row justify-around">
          <div class="q-gutter-y-sm col-3" v-if="memberData.member">
            <div class="text-weight-bold row">
              <div class="col-6 text-right">合伙人信息</div>
            </div>
            <div class="row">
              <div class="col-6 text-right">昵称</div>
              <div class="col-6">：{{ memberData.member.mem_memberName }}</div>
            </div>
            <div class="row" v-if="memberData.partner.level">
              <div class="col-6 text-right">合伙人等级</div>
              <div class="col-6">：{{ memberData.partner.level.ppl_name }}</div>
            </div>
            <div class="row">
            ...
            <div class="row">
              <div class="col-6 text-right">累计分红</div>
              <div class="col-6">： {{ memberData.partner.ppi_total_partner_amount }}</div>
            </div>
          </div>
          <div class="q-gutter-y-sm col-3">
            <div class="text-weight-bold row">
              <div class="col-7 text-right q-pr-lg">分红信息</div>
            </div>
            <div class="row">
              <div class="col-7 text-right q-pr-sm">申请分红：</div>
              <div class="col-5">{{ memberData.ptr_amount }}</div>
            </div>
            <div class="row">
            ...
            <div class="row" v-if="parseInt(memberData.ptr_status) >= 2">
              <div class="col-7 text-right q-pr-sm">审核时间：</div>
              <div class="col-5">{{ $q_date.formatDate(memberData.ptr_check_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}</div>
            </div>
            <div class="row" v-if="parseInt(memberData.ptr_status) == 3">
              <div class="col-7 text-right q-pr-sm">打款时间：</div>
              <div class="col-5">{{ $q_date.formatDate(memberData.ptr_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}</div>
            </div>
            <div class="row" v-if="parseInt(memberData.ptr_status) == 4">
              <div class="col-7 text-right q-pr-sm">无效时间：</div>
              <div class="col-5">{{ $q_date.formatDate(memberData.ptr_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}</div>
            </div>
            <div class="row" v-if="parseInt(memberData.ptr_status) == 1">
          ...
          <div class="q-gutter-y-sm col-3">
            ...
            <div class="row">
              ...
              <div class="col-6" v-if="memberData.ptr_take_mode == 'balance'">：{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}</div>
              <div class="col-6" v-else-if="memberData.ptr_take_mode == 'wxpay'">：微信</div>
              <div class="col-6" v-else-if="memberData.ptr_take_mode == 'alipay'">：支付宝</div>
            </div>
            <div class="row"  v-if="memberData.ptr_take_mode == 'alipay'">
              <div class="col-6 text-right">收款人</div>
              <div class="col-6">：{{ memberData.ptr_alipay_name }}</div>
            </div>
            <div class="row" v-if="memberData.ptr_take_mode == 'alipay'">
              <div class="col-6 text-right">收款账号</div>
              <div class="col-6">：{{ memberData.ptr_alipay_account }}</div>
            </div>
          </div>
        </div>
      </q-card-section>
  ...
  methods: {
    init (id) {
      ...
      if (id > 0) {
        this.takeId = id
        this.$store
          .dispatch('partner/getTakeItemData', { id })
          .then((res) => {
            if (res.code == 200) {
    ...
    setCheckAmountData (takeStatus) {
      this.$q
        ...
        .onOk(() => {
          ...
          const params = {
            id: this.takeId,
            ptr_amount: this.memberData.ptr_amount,
    ...
    setTakeAmountData (takeStatus) {
      this.$q
        ...
        .onOk(() => {
          let checkTime = this.memberData.ptr_check_time
          let endTime = ''
          ...
          const params = {
            id: this.takeId,
            ptr_amount: this.memberData.ptr_amount,
            ptr_member_id: this.memberData.ptr_member_id,
            ptr_status: takeStatus,
            ptr_check_time: checkTime,
            ptr_end_time: endTime
          }
