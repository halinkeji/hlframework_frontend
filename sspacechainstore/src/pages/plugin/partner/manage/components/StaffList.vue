<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md q-gutter-x-xs row">
      ...
      <q-input class="col-12 col-md-2" label="可搜索:员工姓名/员工手机号/会员卡号/会员姓名/会员姓名" outlined v-model="searchQuery" dense />
      <q-select
        outlined
        dense
        label="选择合伙人级别"
    ...
    <add-staff ref="addRef" v-if="isDialogStatus" @getMethods="getList"></add-staff>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getList () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            this.staffData = res.data ? res.data.list : []
