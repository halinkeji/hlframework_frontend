<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    ...
    <add-freight ref="wxmallFreightRef" @closeDialog="getList" v-if="addDialogStatus" />
  </div>
</template>
<script>
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          field: 'psc_app_key',
          valueGetter: (p) => {
            return p.data.wft_delivery_province + p.data.wft_delivery_city + p.data.wft_delivery_district + p.data.wft_delivery_address
          },
          width: this.nowpx(0.2) + 'px'
        },
        {
          headerName: '是否包邮',
          field: 'wft_is_pay_postage',
          valueGetter: (p) => {
            if (p.data.wft_is_pay_postage == '1') {
              return '包邮'
            } else {
              return '自定义运费'
            }
        ...
        },
        {
          ...
          valueGetter: (p) => {
            if (p.data.wft_is_pay_postage == '1') {
              ...
              return (
                '默认运费' +
                p.data.wft_default_quantity +
                '件内' +
                p.data.wft_default_freight +
                this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit +
                '，每增加' +
                p.data.wft_continue_quantity +
                '件，增加运费' +
                p.data.wft_continue_freight +
                this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit
              )
            }
          },
  ...
  methods: {
    getList () {
      ...
      this.$store
        .dispatch('wxMallFreight/getData', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    openAddDialog (id) {
      ...
      this.$nextTick(() => {
        this.$refs.wxmallFreightRef.init(id)
      })
    },
    // 每页数
    ...
    deleteRecord () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('wxMallFreight/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
