...
const nochainConfig = LocalStorage.getItem('sspacechainstore_plugins_local').nochainConfig
if (nochainConfig) {
  if (nochainConfig.nc_google_map_key) {
    Vue.use(GmapVue, {
      load: {
...
export default {
  ...
  mounted () {
    this.getItemData()
    const nochainConfig = LocalStorage.getItem('sspacechainstore_plugins_local').nochainConfig
    this.currentConfig = nochainConfig || {}
  },
  ...
  methods: {
    getItemData () {
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            this.dataFrom = res.data && res.data.config ? res.data.config : {}

            this.addressData = res.data && res.data.address ? res.data.address : {}
          }
        })
        .catch((error) => {})
    },
