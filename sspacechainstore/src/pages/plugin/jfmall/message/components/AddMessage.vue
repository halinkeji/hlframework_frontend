            ...
            <q-form ref="newsDataRef" class="q-gutter-md">
              ...
              <div class="q-gutter-sm">
                <hl-editor
                  @change="
                    (v) => {
                      messageData.jm_content = v;
                    }
                  "
                  :content="messageData.jm_content"
                  class="fit"
                  titleText="详情"
...
export default {
  ...
  methods: {
    getTypeList () {
      this.$store.dispatch('jfMessage/getType').then((res) => {
        if (res.code == 200) {
    ...
    selectBuyer () {
      const that = this
      if (that.messageData.jm_type) {
        const data = that.typeData.find(function (x) {
          return x.type == that.messageData.jm_type
        })
        that.messageData.jm_name = data.name
      }
    },
    init (id) {
      this.dataId = id
      ...
      if (id > 0) {
        this.$store.dispatch('jfMessage/getItem', id).then((res) => {
          if (res.code == 200) {
    ...
    saveData () {
      this.$refs.newsDataRef.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch('jfMessage/setData', obj)
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
