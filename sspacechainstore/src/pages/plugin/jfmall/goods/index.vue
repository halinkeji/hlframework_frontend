...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          valueGetter: (p) => {
            return this.attributeObject[p.data.jg_attribute]
          }
        },
        {
          headerName: '商品名称',
        ...
        {
          headerName: '是否开启会员价',
          field: 'jg_is_member_price',
          width: this.nowpx(0.1) + 'px',
          valueGetter: (p) => {
            if (parseInt(p.data.jg_is_member_price) == 1) {
              return '开启'
            } else {
              return '关闭'
            }
        ...
        },
        {
          ...
          width: this.nowpx(0.1) + 'px',
          valueGetter: (p) => {
            return this.topObject[p.data.jg_tag]
          }
        },
        {
          headerName: '总限购',
      ...
      dataListSelections: [],
      attributeObject: this.$store.state.jfGoods.attributeObject,
      topObject: this.$store.state.jfGoods.topObject
    }
  },
  mounted () {},
  computed: {},
  ...
  methods: {
    ...
    changeSort (p) {
      ...
      const data = p.data
      const regu = /^\+?[0-9][0-9]*$/
      data.jg_sort = data.jg_sort.replace(/\s+/g, '')
      if (!data.jg_sort) {
        this.getList()
        return ''
      }
      if (!regu.test(data.jg_sort)) {
        this.$q.notify({
          message: '错误',
          caption: '请输入正整数',
          color: 'red',
