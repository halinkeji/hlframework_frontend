            ...
            <q-form ref="configDataRef">
              <div class="row">
                ...
                <div class="col-12 col-md-7">
                  ...
                  <hl-address
                    :province="dataFrom.jft_delivery_province"
                    :city="dataFrom.jft_delivery_city"
                    :area="dataFrom.jft_delivery_district"
                ...
                <div class="col-12 col-md-7">
                  <span>详细地址</span>
                  <q-input class="q-pa-xs" outlined maxlength="255" bottom-slots v-model="dataFrom.jft_delivery_address" dense> </q-input>
                </div>
              </div>
              ...
              <div class="col-auto q-gutter-sm">
                <q-radio dense v-model="dataFrom.jft_is_pay_postage" val="1" label="包邮" />
                <q-radio dense v-model="dataFrom.jft_is_pay_postage" val="2" label="自定义运费" />
              </div>
              <div v-if="dataFrom.jft_is_pay_postage == '2'">
                <div class="col-12 q-px-sm row q-mt-xs">
                  <q-input
                    outlined
                    dense
                    v-model="dataFrom.jft_default_quantity"
                    lazy-rules
                  ...
                  <q-input
                    outlined
                    dense
                    v-model="dataFrom.jft_default_freight"
                    lazy-rules
                  ...
                  <q-input
                    outlined
                    dense
                    v-model="dataFrom.jft_continue_quantity"
                    lazy-rules
                    :rules="[(val) => /^[+]{0,1}(\d+)$/.test(val) || '请输入正确件数']"
                  >
                    <template v-slot:before>
                  ...
                  <q-input
                    outlined
                    dense
                    v-model="dataFrom.jft_continue_freight"
                    lazy-rules
                    :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确运费']"
                  >
                    <template v-slot:after>
                ...
                <div class="row q-mt-md full-width" v-if="detailFrom.length > 0">
                  <div class="row text-center full-width">
                    ...
                    <div class="col-2 q-pa-xs">
                      首费（{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit }}）
                    </div>
                    <div class="col-2 q-pa-xs">续件数（件）</div>
                    <div class="col-2 q-pa-xs">
                      续费（{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit }}）
                    </div>
                  </div>
                  <q-separator />
...
export default {
  ...
  data () {
    return {
      ...
      dataFrom: {
        jft_name: '',
        jft_default_quantity: '',
        jft_default_freight: '',
        jft_continue_quantity: '',
        jft_continue_freight: '',
        jft_delivery_province: '',
        jft_delivery_province_code: '',
        jft_delivery_city: '',
        jft_delivery_city_code: '',
        jft_delivery_district: '',
        jft_delivery_district_code: '',
        jft_delivery_address: '',
        jft_is_pay_postage: '1'
      },
      detailFrom: [],
      mainkey: '',
      btnDisable: false,
  ...
  methods: {
    ...
    getItem (id) {
      if (id > 0) {
        this.$store.dispatch('jfFreight/getItem', id).then((res) => {
          if (res.code == 200) {
    ...
    saveData () {
      this.$refs.configDataRef.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch('jfFreight/setData', obj)
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  message: '成功',
    ...
    provinceupdate (a) {
      this.dataFrom.jft_delivery_province = a.value
      this.dataFrom.jft_delivery_province_code = a.code
    },

    cityupdate (a) {
      this.dataFrom.jft_delivery_city = a.value
      this.dataFrom.jft_delivery_city_code = a.code
    },

    areaupdate (a) {
      this.dataFrom.jft_delivery_district = a.value
      this.dataFrom.jft_delivery_district_code = a.code
    },
    addDetail () {
