<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-py-sm">
      <q-btn color="primary" unelevated @click="exportCsv" label="导出Csv文件" :disable="returnGoods.length == 0" />
    </div>
...
export default {
  ...
  methods: {
    ...
    getReturnGoods() {
      ...
      this.$store.dispatch('erpnumber/getConsumeReturn', obj).then((res) => {
        if (res.code == 200) {
    ...
    rowSpan(params) {
      const fields = ['sc_invoiceCode', 'rgd_goods_name', 'rgd_spe_name', 'rgd_goods_code']
      const data = this.returnGoods
