        ...
        <q-card flat class="q-pa-sm">
          <div class="row q-gutter-x-sm">
            <div class="col-12 col-md-2 ">
              <q-input clearable  label="规则名称" outlined v-model="searchQuery" dense />
            </div>
            ...
            <q-btn color="primary" @click="addIntegralRule" v-if="authorityMeta('add')"  unelevated label="新增" />
            <q-btn
...
export default {
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit + '系数',
  ...
  created () {
    ...
    if (this.is_main_store || (storeLocal && storeLocal.supermodel && parseInt(storeLocal.supermodel.psc_private_integral_config) == 1)) {
  ...
  methods: {
    ...
    delAll () {
      this.$q
        .dialog({
          title: '是否确认',
          message: `确定删除 ${this.gridOptions.api.getSelectedRows().length} 条${
            this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit
          }设置吗`,
          cancel: true
        })
        .onOk(() => {
