    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      ...
      <div class="col-shrink q-pa-sm full-height bg-white q-mt-xs">
        <q-scroll-area class="fit full-height">
          ...
          <div class="bg-white q-py-md q-px-sm" v-if="chartsData.payData && chartsData.payData.length > 0">
            <div class="row">
              <div class="q-px-sm col-2 q-pt-sm" v-for="(item, key) in chartsData.payData" :key="key">
                <q-card flat bordered>
                  <q-card-section align="center">
                    <div class="text-h5">
                      {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}{{ item.amount }}
                    </div>
                  </q-card-section>
                  <q-separator />
          ...
          <div class="q-pa-md bg-white">
            <q-btn-group spread unelevated class="my-custom-toggle">
              ...
              />
              <q-btn
                :color="payType == item.field ? 'primary' : ''"
                :flat="payType == item.field ? false : true"
                :key="key + '-btn'"
                @click="getPaytype(item.field)"
                class="col"
                :label="item.name"
                v-for="(item, key) in chartsData.paymentChannel"
              />
            </q-btn-group>
          </div>
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          headerName: '金额',
          valueGetter: (p) => {
            return this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneySymbol + p.data.sc_realMoneyCount
          },
  ...
  methods: {
    ...
    getchartsData () {
      ...
      this.$store
        .dispatch('charts/getChartsRechargeMoneyData', params)
        .then((res) => {
          if (res.code == 200) {
