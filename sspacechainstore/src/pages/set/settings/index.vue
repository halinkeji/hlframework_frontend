    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-md">
        ...
        <q-tab name="other" icon="las la-icons" v-if="authorityMeta('other')" label="其他设置"> </q-tab>
         <q-tab name="erp" icon="las la-campground" v-if="authorityMeta('erp')" label="erp设置"> </q-tab>
        ...
        <q-tab-panels v-model="tab" animated>
          <q-tab-panel name="2" class="q-pa-none q-px-sm">
            <q-tabs v-model="tabOne" dense indicator-color="primary" active-color="primary" class="text-grey-8" align="left">
              ...
              <q-tab name="roomConsume" label="营业消费" />
              <q-tab name="addSpaceTimeCard" label="时卡购买" />
              <q-tab name="spaceBook" label="桌台预订" />
    <!-- <q-tab name="1" label="消费收银" /> -->
              <q-tab name="2" label="快速消费" />
              <q-tab name="3" label="会员充值" />
              <!-- <q-tab name="4" label="增加计次" />
              <q-tab name="5" label="套餐充次" />
              <q-tab name="6" label="增加计时" />
              <q-tab name="7" label="套餐充时" /> -->
            </q-tabs>
            <q-tab-panels v-model="tabOne" animated>
              ...
              <q-tab-panel name="4" class="q-pa-none">
                <add-count-set></add-count-set>
              </q-tab-panel>
              <q-tab-panel name="5" class="q-pa-none">
                <add-group-count-set></add-group-count-set>
              </q-tab-panel>
              <q-tab-panel name="6" class="q-pa-none">
                <add-duration-set></add-duration-set>
              </q-tab-panel>
              <q-tab-panel name="7" class="q-pa-none">
                <add-group-duration-set></add-group-duration-set>
              </q-tab-panel>

              <q-tab-panel name="roomConsume" class="q-pa-none">
                <room-consume></room-consume>
              </q-tab-panel>

              <q-tab-panel name="addSpaceTimeCard" class="q-pa-none">
                <add-space-time-card></add-space-time-card>
              </q-tab-panel>

               <q-tab-panel name="spaceBook" class="q-pa-none">
                <space-book></space-book>
              </q-tab-panel>
      ...
      <div class="col-shrink full-height q-mt-xs fit" v-if="tab == 'prefix'">
        <invoice-prefix ref="invoicePrefix"></invoice-prefix>
      </div>
...
<script>
import AddCountSet from './components/AddCountSet.vue'
import AddGroupCountSet from './components/AddGroupCountSet.vue'
import AddDurationSet from './components/AddDurationSet.vue'
import AddGroupDurationSet from './components/AddGroupDurationSet.vue'
...
import RoomConsume from './components/RoomConsume.vue'
import AddSpaceTimeCard from './components/AddSpaceTimeCard.vue'

import SpaceBook from './components/SpaceBook.vue'
