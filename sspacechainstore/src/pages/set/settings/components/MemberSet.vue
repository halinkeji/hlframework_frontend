        ...
        <q-form ref="paramsForm" class="q-gutter-md row">
          <q-card flat class="bg-white q-pa-md col-6">
            <q-list separator bordered padding>
              ...
              <q-item>
                <q-item-section>
                  <q-item-label>是否开启自动升降级</q-item-label>
                  <q-item-label caption>默认关闭，开启会员根据成长规则和降级条件自动升降至对应等级</q-item-label>
                </q-item-section>
              ...
              <q-item>
                <q-item-section>
                  ...
                  <q-item-label caption>默认消费：1 {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit }} = 1 成长值。（示例：输入 3 ，则 1 {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit }} = 3 成长值） </q-item-label>
                </q-item-section>
                <q-item-section side>
                  <q-input
                    outlined
