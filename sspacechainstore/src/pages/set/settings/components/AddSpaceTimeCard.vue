      ...
      <div class="col-md-4 col-12 q-pr-md q-mt-md">
        <q-card bordered flat>
          <q-card-section>
            ...
            <q-toggle
              v-model="roomConsumeDataForm.consume_addSpaceTimeCard_moneyCashSwitch"
              color="green"
              :true-value="'1'"
              :false-value="'0'"
              class="float-right"
              @input="handleSwitchChange(roomConsumeDataForm.consume_addSpaceTimeCard_moneyCashSwitch, 'consume_addSpaceTimeCard_moneyCashSwitch')"
            />
          </q-card-section>
          <q-separator />
          <div class="q-ma-md">
           启用后，会员使用{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}结账时，单笔可使用{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}总额的百分比。如：营业消费结账，选择{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}，会根据如下设置的百分比自动计算出本次交易可使用的{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}金额数。<br />
          ...
          <q-input
            outlined
            bottom-slots
            v-model="roomConsumeDataForm.consume_addSpaceTimeCard_moneyCashRatio"
            dense
            :rules="[(val) =>  (val ?  /^(([1-9]{1}\d*)|(0{1}))(\.\d{1,2})?$/.test(val) && val >= 1 && val <= 100 : true) || '请输入正确的数值']"
            class="q-px-md"
            @change="inputChange(roomConsumeDataForm.consume_addSpaceTimeCard_moneyCashRatio, 'consume_addSpaceTimeCard_moneyCashRatio')"
          >
          </q-input>
        </q-card>
      </div>
      <div class="col-md-4 col-12 q-pr-md q-mt-md">
        <q-card bordered flat>
          ...
          <q-input
            outlined
            bottom-slots
            v-model="roomConsumeDataForm.consume_addSpaceTimeCard_moneyGivePointRatio"
            @change="inputChange(roomConsumeDataForm.consume_addSpaceTimeCard_moneyGivePointRatio, 'consume_addSpaceTimeCard_moneyGivePointRatio')"
            dense
      ...
      <div class="col-md-4 col-12 q-pr-md q-mt-md">
        <q-card bordered flat>
          <q-card-section>
            ...
            <q-toggle
              v-model="roomConsumeDataForm.consume_addSpaceTimeCard_pointCashSwitch"
              color="green"
              :true-value="'1'"
              :false-value="'0'"
              class="float-right"
              @input="handleSwitchChange(roomConsumeDataForm.consume_addSpaceTimeCard_pointCashSwitch, 'consume_addSpaceTimeCard_pointCashSwitch')"
            />
          </q-card-section>
          <q-separator />
          <div class="q-ma-md">
            启用后，会员使用{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}结账时，单笔可使用{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}总额的百分比。如：营业消费结账，选择{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}，会根据如下设置的百分比自动计算出本次交易可使用的{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}总数。<br />
            示例：某会员张三一共有1000{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}，商家设置的可用百分比为80，则张三本单交易只能使用总{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}的（1000*80%=800{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}）。设置范围为1~100。
          </div>
          <q-input
            outlined
            bottom-slots
            v-model="roomConsumeDataForm.consume_addSpaceTimeCard_pointCashRatio"
            dense
            :rules="[(val) =>  (val ?  /^(([1-9]{1}\d*)|(0{1}))(\.\d{1,2})?$/.test(val) && val >= 1 && val <= 100 : true) || '请输入正确的数值']"
            class="q-px-md"
            @change="inputChange(roomConsumeDataForm.consume_addSpaceTimeCard_pointCashRatio, 'consume_addSpaceTimeCard_pointCashRatio')"
          >
          </q-input>
        </q-card>
      </div>
      <div class="col-md-4 col-12 q-pr-md q-mt-md">
        <q-card bordered flat>
          <q-card-section>
            ...
            <q-toggle
              v-model="roomConsumeDataForm.consume_addSpaceTimeCard_pointUseCashSwitch"
              color="green"
              :true-value="'1'"
              :false-value="'0'"
              class="float-right"
              @input="handleSwitchChange(roomConsumeDataForm.consume_addSpaceTimeCard_pointUseCashSwitch, 'consume_addSpaceTimeCard_pointUseCashSwitch')"
            />
          </q-card-section>
          <q-separator />
          <div class="q-ma-md">
            启用后，会员使用{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}结账时，{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}换算货币比例。如：营业消费结账，选择{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}，会根据如下设置的比率自动计算出本次交易{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}可抵现总金额。<br />
          ...
          <q-input
            outlined
            bottom-slots
            v-model="roomConsumeDataForm.consume_addSpaceTimeCard_pointUseCashSRatio"
            dense
            :rules="[(val) => (val ?  /^(0|[1-9]\d*)(.\d{1,2})?$/.test(val) && val >= 1 : true) || '请输入正确的数值' ]"
            class="q-px-md"
            @change="inputChange(roomConsumeDataForm.consume_addSpaceTimeCard_pointUseCashSRatio, 'consume_addSpaceTimeCard_pointUseCashSRatio')"
          >
          </q-input>
        </q-card>
      </div>
...
<script>
export default {
  name: 'AddSpaceTimeCard',
  data () {
    return {
      roomConsumeDataForm: {
        consume_addSpaceTimeCard_pointSwitch: 0,
        consume_addSpaceTimeCard_pointRatio: '',
        consume_addSpaceTimeCard_levelSwitch: 0,
        consume_addSpaceTimeCard_moneyCashSwitch: 0,
        consume_addSpaceTimeCard_moneyCashRatio: '',
        consume_addSpaceTimeCard_moneyGivePointSwitch: 0,
        consume_addSpaceTimeCard_moneyGivePointRatio: '',
        consume_addSpaceTimeCard_pointCashSwitch: 0,
        consume_addSpaceTimeCard_pointCashRatio: '',
        consume_addSpaceTimeCard_pointUseCashSwitch: 0,
        consume_addSpaceTimeCard_pointUseCashSRatio: ''
      }
    }
  },
  mounted () {
  ...
  methods: {
    getConsumeSet () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            this.roomConsumeDataForm = res.data
          }
        })
        .catch((error) => {})
    },
