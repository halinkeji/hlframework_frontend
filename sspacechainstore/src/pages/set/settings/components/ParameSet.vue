          ...
          <q-card flat class="bg-white q-pa-md col-6">
            <q-list separator bordered padding>
              <q-item>
                <q-item-section>
                  <q-item-label
                    >{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}消费是否需要密码</q-item-label
                  >
              ...
              <q-item>
                <q-item-section>
                  <q-item-label
                    >{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}消费是否需要密码</q-item-label
                  >
                  <q-item-label caption
                    >默认关闭，开启后收银台{{
              ...
              <q-item>
                <q-item-section>
                  <q-item-label
                    >{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}换算比例</q-item-label
                  >
                  <q-item-label caption
                    >默认：1 {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit }} = 1
                    {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}。（示例：输入 3 ，则 1
                    {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneyUnit }} = 3
                    {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}）
                  </q-item-label>
                </q-item-section>
                <q-item-section side>
              ...
              <q-item>
                <q-item-section>
                  <q-item-label
                    >{{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}消费免密支付额度</q-item-label
                  >
                  <q-item-label caption
                    >默认‘0’，当时开启{{
                      $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit
  ...
  data () {
    return {
      baseConfigData: {
        ...
        consume_base_wifiPassword: '',
        consume_base_wifiSsid: '',
        consume_base_wifiUser: ''
      },
      formFieldOptions: []
    }
  },
