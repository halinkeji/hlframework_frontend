    ...
    <div class="col-shrink bg-white q-px-xs q-pb-md">
      <q-tabs v-model="activeName" class="bg-white q-mt-sm" active-color="primary" indicator-color="primary" align="left" content-class="text-grey-9" narrow-indicator dense>
        ...
        <q-tab name="pointRecord" v-if="authorityMeta('integral')">
          <span>
            {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit }}记录
          </span>
        </q-tab>
        <q-tab name="moneyRecord" v-if="authorityMeta('balance')">
          <span>
            {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit }}记录
          </span>
        </q-tab>
     ...
     <div class="col-shrink bg-white full-height bg-grey-2 fit" v-if="activeName != 'prefix'">
      <q-tab-panels v-model="activeName" animated transition-prev="fade" transition-next="fade" class="fit">
        <q-tab-panel class="q-ma-none q-pa-none fit" name="consume">
         ...
         <q-tab-panel class="q-ma-none q-pa-none fit" name="timeCard">
          <time-card-record :memberId="memberId" ref="timeCardRecord"></time-card-record>
        </q-tab-panel>
...
import TimeCardRecord from './components/TimeCardRecord.vue' // 计时消费
    ...
    searchBut () {
      switch (this.activeName) {
        ...
        case 'timeCard':
          ...
          setTimeout(() => {
            this.$nextTick(() => {
              this.$refs.timeCardRecord.getDatalist()
            })
          }, 100)
          break
      }
