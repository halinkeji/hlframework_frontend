          ...
          <div class="col-shrink q-pa-sm full-height bg-white">
            <q-scroll-area class="fit full-height">
                ...
                >
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      ...
                      <q-td key="cid_selling_price" :props="props">
                        <span v-if="parseInt(props.row.cid_packageId) > 0" class="text-red">[套]</span>
                        {{ $q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_moneySymbol }}
                        {{ props.row.cid_selling_price }}
                      </q-td>
                        ...
                        {{
                          formatAmount(
                            formatAmount(props.row.cid_consumeNumber * props.row.cid_goo_price) -
                              formatAmount(props.row.cid_consumeNumber * props.row.cid_selling_price)
                          )
                        }}
                      </q-td>
                    </q-tr>
              ...
              <div class="q-mt-sm" v-if="invoiceData.pointsCountRecord && invoiceData.pointsCountRecord.length > 0">
                <q-table
                  :title="`${$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit}详情`"
              ...
              <div class="q-mt-sm" v-if="invoiceData.moneyCountRecord && invoiceData.moneyCountRecord.length > 0">
                <q-table
                  :title="`${$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit}详情`"
              ...
              <div class="q-mt-sm" v-if="invoiceData.sspaceChainInvoice && invoiceData.sspaceChainInvoice.length > 0">
                <q-table
                  title="营业收银-桌台信息"
                  :data="invoiceData.sspaceChainInvoice"
                  :columns="roomConsumeData"
                  row-key="name"
                  hide-bottom
                  flat
                  bordered
                ...
                >
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      <q-td key="sci_room_table_id" :props="props">
                        <div>{{ props.row.roomTable && props.row.roomTable.qrt_name ? props.row.roomTable.qrt_name : '' }}</div>
                      </q-td>
                      <q-td key="sci_charge_rule_id" :props="props">
                        <div>
                         {{ props.row.chargeRule && props.row.chargeRule.qcr_name ? props.row.chargeRule.qcr_name : '' }}
                        </div>
                      </q-td>
                      <q-td key="sci_create_time" :props="props">
                        {{ $q_date.formatDate(props.row.sci_create_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}
                      </q-td>

                      <q-td key="sci_end_time" :props="props">
                        {{ $q_date.formatDate(props.row.sci_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}
                      </q-td>

                      <q-td key="sci_total_time" :props="props">
                        {{ maintainTimeText(props.row.sci_total_time) }}
                      </q-td>

                      <q-td key="sci_create_reward_user_id" :props="props">
                        {{ props.row.rewardUser && props.row.rewardUser.pru_name ? props.row.rewardUser.pru_name : '' }}
                      </q-td>

                        <q-td key="sci_remark" :props="props">
                        {{ props.row.sci_remark }}
                      </q-td>

                        <q-td key="sci_goods_total_amount" :props="props">
                        {{ props.row.sci_goods_total_amount }}
                      </q-td>

                        <q-td key="sci_room_total_amount" :props="props">
                        {{ props.row.sci_room_total_amount }}
                      </q-td>
                    </q-tr>
                  </template>
                </q-table>
              ...
              <div class="q-mt-sm" v-if="invoiceData.sspaceChainDetail && invoiceData.sspaceChainDetail.length > 0">
                <q-table
                  title="消费详情"
                  :data="invoiceData.sspaceChainDetail"
                  :columns="roomColumnsConsumeData"
                  row-key="id"
                  hide-bottom
                  flat
                  bordered
              ...
              <div class="q-mt-sm" v-if="invoiceData.sspaceTimeCardRecord && invoiceData.sspaceTimeCardRecord.length > 0">
                <q-table
                  title="时卡详情"
                  :data="invoiceData.sspaceTimeCardRecord"
                  :columns="sspaceTimeCardData"
                  row-key="id"
                  hide-bottom
                  flat
                  bordered
                ...
                >
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      <q-td key="tcr_card_name" :props="props">
                        <div v-if="props.row.tcr_card_name">
                          {{ props.row.tcr_card_name }}
                        </div>
                      </q-td>
                      <q-td key="tcr_status" :props="props">
                        <div>
                          {{ formatStatus(props.row.tcr_status) }}
                        </div>
                      </q-td>
                      <q-td key="tcr_minutes" :props="props">
                        {{ props.row.tcr_minutes }}
                      </q-td>
                    <q-td key="tcr_real_end_time" :props="props">
                        {{ parseInt(props.row.tcr_real_end_time) >= 2147457847 ? '永久有效' : $q_date.formatDate(props.row.tcr_real_end_time * 1000, 'YYYY-MM-DD HH:mm:ss') }}
                      </q-td>
...
export default {
  ...
  data () {
    return {
      ...
      integralData: [
        {
          label: '变更' + this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit,
          align: 'center',
          field: 'pcr_pointsCount'
        },
        {
          align: 'center',
          label: this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit + '来源',
          field: 'pcr_pointsCountSource'
        },
        {
          label: this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit + '状态',
          align: 'center',
      ...
      storedData: [
        {
          label: '变更' + this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit,
          align: 'center',
          field: 'mcr_moneyCount'
        },
        {
          align: 'center',
          label: this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit + '来源',
          field: 'mcr_moneyCountSource'
        },
        {
          label: this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit + '状态',
          align: 'center',
      ...
      consumeBasisColumn: [
        ...
        {
          label: '获得' + this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit,
          align: 'left',
          field: 'sc_getPoint'
        }
      ],
      consumPayColumn: [
        {
          label: this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_balanceUnit + '支付',
          align: 'left',
        ...
        {
          align: 'left',
          label: this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit + '支付',
      ...
      roomConsumeData: [
        {
          label: '房间',
          align: 'center',
          field: 'sci_room_table_id',
          classes: 'ellipsis',
          name: 'sci_room_table_id'
        },
        {
          label: '计费模式',
          align: 'center',
          field: 'sci_charge_rule_id',
          name: 'sci_charge_rule_id'
        },
        {
          label: '开单时间',
          align: 'center',
          field: 'sci_create_time',
          name: 'sci_create_time'
        },
        {
          label: '结单时间',
          align: 'center',
        ...
        },
        {
          label: '消费时间',
          align: 'center',
          field: 'sci_total_time',
          name: 'sci_total_time'
        },
        {
          label: '开单员工',
          align: 'center',
          field: 'sci_create_reward_user_id',
          name: 'sci_create_reward_user_id'
        },
        {
          label: '备注',
          align: 'center',
        ...
        },
        {
          label: '商品消费金额',
          align: 'center',
          field: 'sci_goods_total_amount',
          name: 'sci_goods_total_amount'
        },
        {
          label: '房费',
          align: 'center',
          field: 'sci_room_total_amount',
          name: 'sci_room_total_amount'
        }
      ],
      roomColumnsConsumeData: [
        {
          label: '套餐名称',
          align: 'left',
          name: 'cid_packageName',
  ...
  methods: {
    ...
    getViewData (id) {
      ...
      this.invoiceId = id
      if (this.$q.localStorage.getItem('sspacechainstore_store_local') && this.$q.localStorage.getItem('sspacechainstore_store_local').openPayment) {
        this.paymentChannel = this.$q.localStorage.getItem('sspacechainstore_store_local').openPayment
      }
      if (id > 0) {
        const obj = {
          id: id
