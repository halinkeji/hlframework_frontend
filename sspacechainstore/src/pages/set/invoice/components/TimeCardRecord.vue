  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md">
      <div class="row q-gutter-sm">
        <div class="col-12 col-md-5" v-if="is_main_store">
          <q-select
            label="查询门店"
            use-chips
...
export default {
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '时卡名称', // 列标题
          field: 'tcr_card_name', // 列与数据源对应的字段
          filter: false,
          width: this.nowpx(0.1) + 'px'
        },
        ...
        {
          headerName: '会员姓名', // 列标题
          field: 'tcr_member_nickname', // 列与数据源对应的字段
          filter: false,
          width: this.nowpx(0.1) + 'px'
        },
...

        {
          ...
          valueGetter: (p) => {
            if (!p.data.show_status) {
              return p.data.tcr_status == 1 ? '+' + p.data.tcr_minutes : '-' + p.data.tcr_minutes
            } else {
              return p.data.tcr_minutes
            }
          },
          width: this.nowpx(0.1) + 'px'
        },

        {
          ...
          valueGetter: (p) => {
            if (!p.data.show_status) {
              return p.data.tcr_status == 1 ? '增加' : '减少'
            }
          },
          width: this.nowpx(0.1) + 'px'
        },
  ...
  methods: {
    ...
    getDatalist () {
      ...
      this.$store
        .dispatch('timeCardPackage/getSpaceTimeCardRecord', obj)
        .then((res) => {
          if (res.code == 200) {
            if (res.data.list) {
    ...
    exportBut () {
      ...
      this.gridOptions.api.exportDataAsCsv({
        fileName: date + '-' + this.$q.localStorage.getItem('sspacechainstore_store_local').storeBaseConfig.consume_base_integralUnit + '记录'
      })
    },
    pinnedComputed (val) {
      const nowWidth = this.$q.screen.width
