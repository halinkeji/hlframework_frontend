    ...
    <q-header elevated class="bg-primary text-white" height-hint="98">
      <q-toolbar>
        ...
        <q-avatar class="q-mb-sm" size="md" v-if="$q.screen.gt.sm">
          <q-img ratio="1" :src="store_info.logoData ? `${$q.localStorage.getItem('hlpay_system_local').attach_url}` + store_info.logoData.path : ''">
            <template v-if="!store_info.logoData">
            ...
            <q-card class="full-width" flat bordered>
              ...
              <q-card-section class="flex flex-center">
                <q-avatar square size="100px" v-if="sy_serve_qrcode">
                  <img :src="sy_serve_qrcode" /> </q-avatar>
...
export default {
  ...
  computed: {
    ...
    storeEndTime () {
      const storeInfo = this.$q.localStorage.getItem('hlpay_store_local')
      if (storeInfo && storeInfo.storeInfo) {
        const store_info = storeInfo.storeInfo
        return this.$q_date.formatDate(store_info.end_time * 1000, 'YYYY年MM月DD日')
      }
      return '错误'
    },
    current_title () {
      if (LocalStorage.getItem('hlpay_apply_local')) {
        const systemLocal = LocalStorage.getItem('hlpay_apply_local')
  ...
  created () {
    if (LocalStorage.getItem('hlpay_apply_local')) {
      const copyright = LocalStorage.getItem('hlpay_apply_local').copyright
      if (copyright) {
