            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="is_mch_mode" :props="props">
                  <q-toggle
                    @input="updatePayConfig(props.row.id, 'is_mch_mode', props.row.is_mch_mode)"
                    v-model="props.row.is_mch_mode"
                    false-value="0"
                    true-value="1"
                    v-if="authorityMeta('channel-edit')"
                  />
                </q-td>
                <q-td key="is_isv_mode" :props="props">
                  <q-toggle
                    v-model="props.row.is_isv_mode"
                    @input="updatePayConfig(props.row.id, 'is_isv_mode', props.row.is_isv_mode)"
                    false-value="0"
                    true-value="1"
                    v-if="authorityMeta('channel-edit')"
                  />
                ...
                <q-td key="icon" :props="props">
                  <q-img
                    :src="props.row.iconData && props.row.iconData.id ? $q.localStorage.getItem('hlpay_system_local').attach_url + props.row.iconData.path : ''"
                    :ratio="1"
                    style="height: 50px ;width: 50px"
                    v-if="props.row.iconData && props.row.iconData.id"
                  />

                  <q-img v-else width="50px" height="50px">
                    <template >
                ...
                <q-td key="id" :props="props">
                  ...
                  <q-btn round v-if="authorityMeta('channel-edit')" @click="setPayConfig(props.row.id, props.row.type, props.row.related_id)" flat color="primary" icon="las la-edit">
...
export default {
  ...
  methods: {
    ...
    setPayConfig (configId, type, relatedId) {
      ...
      this.$nextTick(() => {
        this.$refs.payConfig.init(configId, type, relatedId)
      })
    },
    closeDialog () {
