              ...
              <div class="row q-col-gutter-sm">
                ...
                <div class="col">
                  <q-card flat bordered>
                    <q-item>
                      <q-item-section>
                        <q-item-label>支付渠道响应支付请求数据</q-item-label>
                ...
                <div class="col">
                  <q-card flat bordered>
                    <q-item>
                      <q-item-section>
                        <q-item-label>支付渠道响应支付结果数据</q-item-label>
                        <q-item-label caption>{{ valueTimeFormat(invoiceData.channel_result_time) }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    <q-separator />
                    <q-card-section>
                      <vue-json-viewer :value="invoiceData.channel_result_data"></vue-json-viewer>
                    </q-card-section>
                  </q-card>
                </div>
                <div class="col">
...
export default {
  ...
  methods: {
        ...
        this.$store
          ...
          .then((res) => {
            if (res.code == 200) {
              ...
              this.invoiceData.channel_result_data = res.data && res.data.channel_result_data ? JSON.parse(res.data.channel_result_data) : {}
            }
          })
          .catch((error) => {})
      }
    ...
    valueTimeFormat (val) {
      if (parseInt(val)) {
        return this.$q_date.formatDate(parseInt(val), 'YYYY-MM-DD HH:mm:ss')
      }
      return ''
    }
