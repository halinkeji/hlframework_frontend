      ...
      <q-toolbar>
        ...
        <q-avatar class="q-mb-sm" size="md" v-if="$q.screen.gt.sm">
          ...
          <q-img
            ...
            v-else
            :src="
              `${$q.localStorage.getItem('oneyouka_system_local').attach_url}` +
...
export default {
  ...
  computed: {
    ...
    mobileRegUrl () {
      ...
      if (account_info && system_local) {
        return system_local.systemInfo.site_url + '/m/oneyouka/?id=' + account_info.lib_id
      }
      return ''
    },
    current_title () {
      if (LocalStorage.getItem('oneyouka_apply_local')) {
        const applyLocal = LocalStorage.getItem('oneyouka_apply_local')
  ...
  created () {
    if (LocalStorage.getItem('oneyouka_apply_local')) {
      const copyright = LocalStorage.getItem('oneyouka_apply_local').copyright
      if (copyright) {
    ...
    this.getWxappQrcode()
    this.getMenuData()
    const user_info = LocalStorage.getItem('oneyouka_user_info')
  ...
  methods: {
    ...
    getMenuData () {
      const oneyouka_left_menu_navigate_local = LocalStorage.getItem('oneyouka_left_menu_navigate_local')

      this.linksData = oneyouka_left_menu_navigate_local
    },
    ...
    updateMenuData () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            LocalStorage.set('oneyouka_system_local', res.data.system_local)
            LocalStorage.set('oneyouka_apply_local', res.data.apply_local)
            LocalStorage.set('oneyouka_store_local', res.data.store_local)
            LocalStorage.set('oneyouka_plugins_local', res.data.plugins_local)

            const allRouter = this.getFormatRoutes(res.data.router)
            LocalStorage.set('oneyouka_router_local', res.data.router)
            routes[1].children = allRouter

            this.$router.addRoutes(routes)
            LocalStorage.set('oneyouka_left_menu_navigate_local', res.data.menu)
