...
export const isLocalStorageTokenExpir = () => {
  const expiredTime = parseInt(
    LocalStorage.getItem('oneyouka_accessToken_info').expires
  )
  /* 获取本地时间 */
...
export const isAuthenticated = () => {
  return (
    LocalStorage.getItem('oneyouka_loggedIn') &&
    Cookies.get('access_token') ===
      LocalStorage.getItem('oneyouka_access_token')
  )
}

export const isCurrentApply = () => {
  const storageAccountInfo = LocalStorage.getItem('oneyouka_account_info')
  if (storageAccountInfo && storageAccountInfo.apply_id) {
    if (storageAccountInfo.apply_id != 'oneyouka') {
      return false
    }
  }
...
export function setAllLocalStorage () {
  LocalStorage.set('oneyouka_access_token', Cookies.get('access_token'))
  LocalStorage.set('oneyouka_expires_in', Cookies.get('expires_in'))
  LocalStorage.set('oneyouka_refresh_token', Cookies.get('refresh_token'))
  LocalStorage.set('oneyouka_token_type', Cookies.get('token_type'))
  LocalStorage.set('oneyouka_loggedIn', Cookies.get('loggedIn'))
  LocalStorage.set(
    'oneyouka_accessToken_info',
    Cookies.get('accessToken_info')
  )
  LocalStorage.set('oneyouka_user_info', Cookies.get('user_info'))
  LocalStorage.set('oneyouka_account_info', Cookies.get('account_info'))
  if (Cookies.get('store_info')) {
    LocalStorage.set('oneyouka_lib_local', {
