            ...
            <q-form ref="oilProductRef" class="row q-col-gutter-sm">
              ...
              <div class="col-md-3 col-12">
                <q-input
                  outlined
                  bottom-slots
                  v-model="oilProductFrom.op_stock"
                  dense
                  lazy-rules
                  label="剩余库存"
              ...
              <div class="col-md-3 col-12">
                <q-select
                  ...
                  emit-value
                  map-options
                  v-model="oilProductFrom.op_status"
                  :options="[
                    { label: '开启', value: '1' },
                    { label: '关闭', value: '0' },
                  ]"
              ...
              <div class="col-md-12">
                <q-item>
                  ...
                  <q-item-section side>
                    <q-toggle v-model="oilProductFrom.op_is_member_price" @input="setPriceModel" color="green" true-value="1" false-value="2" />
                  </q-item-section>
                </q-item>
              </div>

              <q-card flat class="q-mt-sm q-pb-md" v-if="parseInt(oilProductFrom.op_is_member_price) == 1">
                <div class="q-px-md q-py-sm row">
                  ...
                  <div class="col text-right">
                    <q-radio @input="setPriceModel" v-model="oilProductFrom.op_member_price_type" val="1" label="等级折扣/积分比例" />
                    <q-radio @input="setPriceModel" v-model="oilProductFrom.op_member_price_type" val="2" label="会员价/积分换算" />
                  </div>
                </div>
...
export default {
  ...
  data () {
    return {
      oilProductFrom: {
        ...
        op_price: '',
        op_unit: 1,
        op_is_member_price: '2',
        op_member_price_type: '2',
        op_stock: '',
        op_status: '1',
        op_source_group: ''
      },
  ...
  methods: {
    init (oilId) {
      ...
      this.$store.dispatch('product/getOilItem', { oilId }).then((res) => {
        if (res.code == 200) {
          ...
          if (this.oilProductFrom && parseInt(this.oilProductFrom.op_is_member_price) == 1) {
            this.setPriceModel()
          }
        }
      })
    ...
    setPriceModel () {
      if (this.oilProductFrom.op_is_member_price == 1) {
        this.$nextTick(() => {
          const columnData = [
          // 表格列数据
            {
          ...
          if (this.oilProductFrom && parseInt(this.oilProductFrom.op_member_price_type) == 1) {
            columnData.push({
              headerName: '等级折扣',
              field: 'omp_rebate',
              filter: false,
          ...
          if (this.oilProductFrom && parseInt(this.oilProductFrom.op_member_price_type) == 2) {
            columnData.push({
              headerName: '价格',
              field: 'omp_price',
              filter: false,
