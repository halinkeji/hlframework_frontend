  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    <div class="col-shrink bg-white q-px-xs q-pb-md row q-gutter-sm q-pt-sm">
      <div class="col-4 " v-if="is_main_store">
        <q-select label="查询油站" use-chips clearable v-model="selectStoreData" option-value="id" option-label="name" multiple emit-value map-options dense outlined :options="libInStoreList" />
      </div>
      <div class="col-12 col-md-2 " >
        <q-input label="请输入职位名称" outlined v-model="searchQuery" dense />
      </div>
      ...
      <q-btn :disable="dataListSelections.length > 0 ? false : true" @click="delAll" color="red"  unelevated v-if="authorityMeta('delAll')" label="批量删除" />
    </div>
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      this.$store
        .dispatch('rewardPosition/getData', obj)
        .then((res) => {
          if (res.code == 200) {
    ...
    deleteRecord () {
      ...
      if (dataListSelections.length > 0) {
        ...
        this.$store.dispatch('rewardPosition/delData', obj).then((res) => {
          if (res.code == 200) {
            this.$q.notify({
              color: 'green',
