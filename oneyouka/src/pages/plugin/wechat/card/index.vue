                  ...
                  :style="{
                    backgroundImage:
                      parseInt(cardFormInfo.mc_bg_type) == 1
                        ? `url('${$q.localStorage.getItem('oneyouka_system_local').attach_url}${cardFormInfo.mc_bg_img_local}')`
                        : '',
                  ...
                  <q-item>
                    <q-item-section avatar>
                      <q-avatar class="bg-grey">
                        <q-img :src="`${$q.localStorage.getItem('oneyouka_system_local').attach_url}` + cardFormInfo.mc_logo_url">
                ...
                <q-card flat bordered class="bg-white q-px-md q-pb-md">
                  ...
                  <div class="row q-mt-lg text-subtitle1 q-pl-md items-center">
                    ...
                    <div class="col-12 col-md-6">
                      <div>
                        <q-img
                          :src="`${$q.localStorage.getItem('oneyouka_system_local').attach_url}` + cardFormInfo.mc_logo_url"
                  ...
                  <div class="row q-mt-lg text-subtitle1 q-pl-md items-center">
                    ...
                    <div class="col-12 col-md-6">
                      ...
                      {{ cardFormInfo.mc_bg_color }}
                      {{ bg_color }}
                      <div v-if="parseInt(cardFormInfo.mc_bg_type) == 1">
                        <div>
                          <q-img
                            :src="`${$q.localStorage.getItem('oneyouka_system_local').attach_url}` + cardFormInfo.mc_bg_img_local"
                        ...
                        <div>
                          <q-btn unelevated @click="clickImage('mc_bg_img_local')" color="primary" label="选择图片" />
                        </div>
                      </div>
                      ...
                      <div class="text-grey">
                        ...
                        >
                          <template v-slot:hint>
                            建议会员卡标题包含商户名或服务内容，如腾讯会员黄卡尊享卡。字数上限为9个汉字
                          </template>
                        </q-input>
                      </div>
                    </div>
                  ...
                  <div class="row q-mt-lg text-subtitle1 q-pl-md items-center">
                    ...
                    <div class="col-12 col-md-6">
                      <q-input
                        ...
                        dense
                        :placeholder="`注：1、上文设置中，如已经填写“${$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit}优惠”“折扣优惠”的内容，将会自动在用户会员卡详情展示，无需重复填写。2、建议填写其他特权，举例：100${$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit}可兑换精美礼品；会员日可享受折上折优惠等`"
                        type="textarea"
                  ...
                  <div class="row q-mt-sm text-subtitle1 q-pl-md items-center">
                    ...
                    <div class="col-12 col-md-6">
                      <q-input
                        ...
                        dense
                        :placeholder="`注：1、上文设置中，如已经填写“${$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit}优惠”“折扣优惠”的内容，将会自动在用户会员卡详情展示，无需重复填写。2、建议填写其他注意事项，举例：${$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit}不支持兑换现金`"
                        type="textarea"
                  ...
                  <div class="q-pl-md text-grey text-subtitle1">
                    可配置图文消息、货架、网页链接，用户可查看详情或领取更多卡券。
                  </div>
                  ...
                  <div class="text-red q-px-md">
                    (注意:因微信卡包设置问题，以下配置只能同时展示三个，如全部开启则随机选取三个配置进行展示)
                  </div>
                  <q-item>
                    <q-item-section>
                      <q-item-label>是否开启{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}<span class="text-red">(不可修改)</span></q-item-label>
                      <q-item-label caption>
                        微信卡包内微信会员卡{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}与系统{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}同步，开启跳转链接
                      </q-item-label>
                    </q-item-section>
                    <q-item-section side top>
                      <q-toggle
                  ...
                  <q-item>
                    <q-item-section>
                      <q-item-label
                        >是否开启{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}
                        <span class="text-red">(不可修改)</span>
                      </q-item-label>
                      <q-item-label caption>
                        微信卡包内微信会员卡{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}与系统{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}同步，开启跳转链接
                        会员卡商户无需申请，可直接通过API接口，使用“{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}展示”功能，将会员{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}显示在微信会员卡首页。具有预付卡资质的商家可申请“{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}”功能，申请成功后，可通过API接口设置此入口，帮助会员通过微信支付为会员卡充值。
                      </q-item-label>
                    </q-item-section>
                    <q-item-section side top>
                      <q-toggle
...
export default {
  ...
  created () {
    ...
    const lib_id = LocalStorage.getItem('oneyouka_account_info').lib_id

    const account_info = this.$q.localStorage.getItem('oneyouka_account_info')
    const system_local = this.$q.localStorage.getItem('oneyouka_system_local')

    if (account_info && system_local) {
      this.mc_member_link = system_local.systemInfo.site_url + '/m/oneyouka?id=' + lib_id
      this.cardFormInfo.mc_center_data.url = this.mc_member_link
      this.cardFormInfo.mc_level_link = system_local.systemInfo.site_url + '/m/oneyouka/level?id=' + lib_id
      this.cardFormInfo.mc_coupon_link = system_local.systemInfo.site_url + '/m/oneyouka/hlcoupon?id=' + lib_id
    }
  },
  computed: {},
  methods: {
