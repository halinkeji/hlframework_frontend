<template>
  <div>
    <q-btn color="warning" unelevated label="查看" v-if="authorityMeta('view')" :to="{ name: 'partnerInfo', query: { id: dataId } }" />
    ...
    confirmDeleteRecord () {
      this.$q
        .dialog({
          ...
          color: 'primary',
          title: '是否确认',
          message: `确定删除姓名为 "${this.params.data.ppi_partner_name}" 的合伙人吗`,
          cancel: true
        })
        .onOk(() => {
          this.$store
