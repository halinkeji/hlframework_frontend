<template>
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    ...
    <pay-market-order v-if="payMarketOrderStatus" ref="payMarketOrderRef" @closePayDialog="closePayDialog" />
  </div>
</template>
<script>
import AgOperating from './AgOperating.vue'
import AgProperty from './AgProperty.vue'
import AgStatusSwitch from './AgStatusSwitch.vue'
import PayMarketOrder from './PayMarketOrder.vue'
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          filter: false,
          width: this.nowpx(0.1) + 'px',
          cellRendererFramework: 'AgProperty'
        },
  ...
  methods: {
    ...
    openPmerchantCardInfo (id) {
      ...
      this.$nextTick(() => {
        this.$refs.pmerchantCardInfoRef.init(id)
      })
    },
    closeCardInfoDialog () {
    ...
    openPayMarketDialog (couponItem) {
      this.payMarketOrderStatus = true
      this.$nextTick(() => {
        if (couponItem && couponItem.id && couponItem.pmerchantMarket && couponItem.pmerchantMarket.id) {
          this.$refs.payMarketOrderRef.init(couponItem)
        }
      })
    },
    closePayDialog () {
      this.payMarketOrderStatus = false
