            ...
            <q-form ref="packageDataRef">
              <div class="q-pr-sm">
                <q-banner rounded class="text-white bg-red" dense>
                  注意：规则不可修改！注册规则必须选择会员等级！开启会员权益会直接赠送会员级别相关权益！赠送卡券、次卡、计时卡、{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}、{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}时，不填不选则不赠送!
                </q-banner>
              </div>
              ...
              <div class="row q-mt-xs">
                <div class="col-6 q-pr-md">
                  <q-input outlined dense v-model="packageData.pkp_give_balance" :label="`赠送${$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit}`" :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的赠送'+$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit]" />
                </div>
                <div class="col-6 q-pr-sm">
                  <q-input outlined dense v-model="packageData.pkp_give_integral" :label="`赠送${$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit}`" :rules="[(val) => /^(\d+|\d+\.\d{1,2})$/.test(val) || '请输入正确的赠' + $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit]" />
                </div>
              </div>
              <div class="q-mt-xs">
                <q-chip outline square color="red" text-color="white"> 赠送{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }} {{ packageData.pkp_give_balance }}{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_moneyUnit }}，赠送 {{ packageData.pkp_give_integral }} {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }} </q-chip>
              </div>
                    ...
                    <q-list bordered separator>
                      <q-item>
                        <q-item-section>
                          ...
                          <q-item-label>
                            价格：
                            <span class="text-orange"> {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_moneySymbol }} </span>
                      ...
                      <q-item>
                        <q-item-section>
                          ...
                          <q-item-label caption lines="3" v-if="parseInt(item.cou_type) == 6">
                            兑换 <span class="text-h6 text-deep-orange text-text-weight-bolder"> {{ item.cou_give_integral }} </span> {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}
                          </q-item-label>
                          <q-item-label caption lines="3" v-if="parseInt(item.cou_type) == 7">
                            兑换 <span class="text-h6 text-deep-orange text-text-weight-bolder"> {{ item.cou_give_balance }} </span> {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}
                          </q-item-label>
                        </q-item-section>
