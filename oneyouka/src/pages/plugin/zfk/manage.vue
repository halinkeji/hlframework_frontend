  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '共享' + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit,
          valueGetter: this.ruleBalanceValue,
          width: this.nowpx(0.12) + 'px'
        },
        {
          headerName: '共享' + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit,
  ...
  methods: {
    ...
    ruleIntegralValue (p) {
      if (parseInt(p.data.zml_is_share_integral) == -1) {
        return '不共享' + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit
      } else if (parseInt(p.data.zml_is_share_integral) == 1) {
        return '共享' + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit + '（不限制）'
      } else if (parseInt(p.data.zml_is_share_integral) == 2) {
        return '每' + this.dateText(p.data.zml_limit_integral_time) + '共享' + p.data.zml_integral_amount + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit
      } else {
        return ''
      }
    },
    ruleBalanceValue (p) {
      if (parseInt(p.data.zml_is_share_balance) == -1) {
        return '不共享' + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit
      } else if (parseInt(p.data.zml_is_share_balance) == 1) {
        return '共享' + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit + '（不限制）'
      } else if (parseInt(p.data.zml_is_share_balance) == 2) {
        return '每' + this.dateText(p.data.zml_limit_balance_time) + '共享' + p.data.zml_balance_amount + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit
      } else {
        return ''
      }
    },
