<template>
  <div class="fit bg-grey-3">
    <div class="fit column  no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="bg-white">
        <q-card flat class="q-py-sm q-px-xs row q-gutter-xs">
          <div class="col-12 col-md-2">
              <q-input clearable  label="请输入标题" outlined v-model="searchQuery" dense />
            </div>
          <div class="col-12 col-md-1">
            <q-select outlined dense v-model="ruleStatus" :options="statusOptions" label="活动状态" option-label="label" emit-value map-options />
          </div>
          ...
          <q-btn unelevated color="primary"  label="新增" @click="openRuleDialog(0)"  v-if="authorityMeta('add')" />
          <q-btn
            unelevated
            color="red"
      ...
      <add-activity v-if="addRuleDialogStatus" ref="addRuleDialogRef" @closeDialog="closeDialog" />
    </div>
  </div>
</template>
<script>
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          headerName: '随机金额区间',
          valueGetter: (p) => {
            return p.data.pha_random_amount_min + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_moneyUnit + '-' + p.data.pha_random_amount_max + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_moneyUnit
          },
          width: this.nowpx(0.1) + 'px'
        },
  ...
  methods: {
    ...
    getList () {
      ...
      this.$store
        .dispatch('hbyx/getActivityData', obj)
        .then((res) => {
    ...
    delAllRule () {
      this.$q
        .dialog({
          title: '是否确认',
          message: `确定删除 ${this.gridOptions.api.getSelectedRows().length} 条活动吗`,
          cancel: true
        })
        .onOk(() => {
