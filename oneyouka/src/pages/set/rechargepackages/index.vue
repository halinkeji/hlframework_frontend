    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white" :class="{ 'q-pb-lg': parseInt(activeName) == 0 ? true : false }">
        ...
        <div class="row q-px-sm q-gutter-x-sm" v-if="parseInt(activeName) == 0">
          <div class="col-3">
            <q-input clearable label="请输入套餐名称" outlined v-model="packageName" dense />
          </div>
          <div class="col-1">
            <q-select
              outlined
          ...
          <q-btn
            ...
            label="批量删除"
          />
          <q-btn @click="testRecharge" color="primary" v-if="false" unelevated label="测试" />
        </div>
      </div>
  ...
  data () {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: '套餐类别',
          field: 'rp_source_group',
          valueGetter: (p) => {
            return this.$store.state.components.balanceGroupObject[p.data.rp_source_group]
          },
          width: this.nowpx(0.1) + 'px'
        },
        {
