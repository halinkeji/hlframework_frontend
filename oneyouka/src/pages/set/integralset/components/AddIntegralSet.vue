    ...
    >
      <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
        <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2 q-pa-sm row">
          新增{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}折扣设置
          <q-space />
            ...
            <q-card flat square>
              ...
              <div class="row">
                ...
                <div class="col-6">
                  <q-input
                    ...
                    :label="`${$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit}换算比例`"
                    :error="false"
                    lazy-rules
                    :rules="[
                      (val) => (val > -1 && val <= 100) || '请输入正确的' + $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit + '换算比例',
                      (val) => /^[0-9]+([.]\d{1,2})?$/.test(val) || '请输入正确的' + $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit + '换算比例',
                    ]"
                  >
                    <template v-slot:hint>
                     范围：大于等于0
              ...
              <div class="q-pa-sm">
                ...
                <div class=" q-mt-sm text-red">
                  ...
                  }}，选择商家优惠规则 8折（折扣百分比填 80），则该笔消费实际支付金额 80 {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_moneyUnit }}，优惠金额 20
                  {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_moneyUnit }}（ 100 - 100 × 80% = 20 ）
                </div>
            ...
            .then((res) => {
              if (res.code == 200) {
                this.$q.notify({
                  ...
                  position: 'top-right',
                  message: '成功',
                  caption: '新增' + this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit + '折扣设置成功',
                  color: 'green'
                })
    ...
    getItem () {
      this.$store.dispatch('integralset/getScene').then((res) => {
        if (res.code == 200) {
          ...
          const openPlugins = LocalStorage.getItem('oneyouka_store_local').openPlugins
