  ...
  <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
    ...
    <div class="col-shrink bg-white q-px-xs q-pb-md">
       <q-tabs v-model="activeName" class="bg-white q-mt-sm" active-color="primary" indicator-color="primary" align="left" content-class="text-grey-9" narrow-indicator dense>
        <q-tab name="oilConsume" v-if="authorityMeta('oilConsume')">
          <span>
            油品收银
          </span>
        </q-tab>
        ...
        <q-tab name="pointRecord" v-if="authorityMeta('integral')">
          <span>
             {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}记录
          </span>
        </q-tab>
        <q-tab name="moneyRecord" v-if="authorityMeta('balance')">
          <span>
            {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_balanceUnit }}记录
          </span>
        </q-tab>
    ...
    <div class="col-shrink bg-white full-height bg-grey-2 fit">
      <q-tab-panels v-model="activeName" animated transition-prev="fade" transition-next="fade" class="fit">
        <q-tab-panel class="q-ma-none q-pa-none fit" name="oilConsume">
          <oil-consumer-invoice :memberId="memberId" ref="oilConsumer"></oil-consumer-invoice>
        </q-tab-panel>
...
import IntegralRecord from './components/IntegralRecord.vue' // 积分记录
import BalanceDetails from './components/BalanceDetails.vue' // 储值记录
import ConsumerInvoice from './components/ConsumerInvoice.vue' // 消费列表
import ReturnRecord from './components/ReturnRecord.vue' // 退货列表
import OilConsumerInvoice from './components/OilConsumerInvoice.vue'
export default {
  name: 'invoice',
  props: ['memberId'],
  components: {
    ...
    searchBut () {
      switch (this.activeName) {
        case 'oilConsume':
          setTimeout(() => {
            this.$nextTick(() => {
              this.$refs.oilConsumer.getDatalist()
            })
          }, 100)
          break
        case 'consume':
