      ...
      >
        <q-form ref="consumeFormRef">
          ...
          <q-card-section>
            ...
            <div class="text-caption text-grey">单价（{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_moneyUnit }}/千克）</div>
          </q-card-section>
      ...
      try {
        ...
        this.serial_port.on('data', (data) => {
          ...
          switch (2) {
            case '1':
              // 大华计算格式
              that.chud_consumeNumber = this.$q_decimal(num / 1000000000).toFixed(2)
              break
            case '2':
              // 顶尖计算格式
              that.chud_consumeNumber = this.$q_decimal(num / 1000).toFixed(2)
              break
            default:
              // 其他通用计算格式
              that.chud_consumeNumber = this.$q_decimal(num / 1000).toFixed(2)
