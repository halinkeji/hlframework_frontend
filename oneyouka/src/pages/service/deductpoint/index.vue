  ...
  <q-page class="fit">
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-py-sm text-subtitle2">
        ...
        {{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}操作
      </div>
                ...
                <div class="row">
                  <div class="col-6 col-md-4 q-mt-md q-px-xs" v-for="item in fixedMoneyList">
                    <q-btn unelevated rounded color="green" class="q-px-md full-width" :disable="memberId == 0" @click="pointInput = item.fm_parameter">
                      {{ item.fm_parameter }}<span class="text-body1">{{ $q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_integralUnit }}</span>
                    </q-btn>
                  </div>
                </div>
              </q-card-section>
