<template>
  <div>
    <q-input outlined @input="changeNumber" dense v-model="params.data.cpb_quantity" />
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    changeNumber () {
      ...
      if (!regu.test(this.params.data.cpb_quantity)) {
        this.$q.notify({
          message: '错误',
          caption: '请输入正整数',
          color: 'red',
        ...
        const obj = {
          ...
          quantity: this.params.data.cpb_quantity,
