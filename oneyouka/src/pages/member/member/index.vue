    ...
    <div class="fit column no-wrap justify-between items-stretch content-stretch no-scroll">
      <div class="col-shrink bg-white q-px-xs q-pb-sm">
        ...
        >
          ...
          <q-tab name="3" label="未激活会员" v-if="authorityMeta('inactivated')"> </q-tab>
        </q-tabs>
      </div>
...
export default {
  ...
  mounted () {
    if (
      this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig &&
      this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_regBindFormField &&
      this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_regBindFormField > 0
    ) {
      this.getFieldData(this.$q.localStorage.getItem('oneyouka_store_local').storeBaseConfig.consume_base_regBindFormField)
    }
  },
  created () {
    this.getLevel()
